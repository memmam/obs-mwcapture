...
inline BOOLEAN IEC60958_ParseChannelStatus(IEC60958_CHANNEL_STATUS *pStatus,
					   BOOLEAN *pbLPCM,
					   DWORD *pdwSampleRate,
					   int *pnBitsPerSample)
{
	...
	if (pStatus->Professional.byProfessional) {
		...
		nBitsPerSample = IEC60958_GetBitsPerSample(
			pStatus->Professional.byWordLength);
	} else {
		bLPCM = !pStatus->Consumer.byNotLPCM;
		dwSampleRate = IEC60958C_GetSampleRate(pStatus);
		nBitsPerSample = IEC60958_GetBitsPerSample(
			pStatus->Consumer.byWordLength);
	}
