...
#include <math.h>
#define CHAR_BITS 8
void HevcBsInitWrite(ST_HEVC_BITSTREAMER *pstBs, unsigned char *pucStreamBuf,
		     unsigned int uiLen)
{
...
{
	int i;
	int m = static_cast<int>(log((long double)(value + 1)) /
				 log((long double)2)); // 前缀，也即是组号
	int info =
		static_cast<int>(value + 1 - pow(2.0, (int)m)); // 组内的偏移量
	for (i = 0; i < m; i++) {
...
void HevcBsInitRead(ST_HEVC_BITSTREAMER *pstBs, unsigned char *pucStreamBuf,
		    unsigned int uiLen, bool bCheckZero)
{
	pstBs->pucData = pucStreamBuf;
	pstBs->uiLen = uiLen;
	pstBs->uiIdx = 0;
...
int HevcGetBit(ST_HEVC_BITSTREAMER *pstBs)
{
	...
	if (pstBs->uiBits < 0) {
		...
		if (pstBs->uiIdx >= 2 && pstBs->bCheckZero == true) {
			if (pstBs->pucData[pstBs->uiIdx - 2] == 0 &&
			    pstBs->pucData[pstBs->uiIdx - 1] == 0 &&
			    pstBs->pucData[pstBs->uiIdx] == 3) {
				pstBs->uiIdx++;
			}
		}
	}
...
int HevcReplace(unsigned char *pucSrcData, unsigned int uiSrclen,
		unsigned char *pucDstData, unsigned int *pucDstlen)
{
	unsigned int i;
