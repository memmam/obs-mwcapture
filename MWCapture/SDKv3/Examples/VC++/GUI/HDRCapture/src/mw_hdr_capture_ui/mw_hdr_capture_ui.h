...
public:
	...
	static void callback_hdr_value_change(cmw_hdr_param_s t_param,
					      void *t_p_param)
	{
		if (t_p_param == NULL)
			return;
	...
	static DWORD WINAPI thread_cmd_process(LPVOID lpThreadParameter)
	{
		...
		if (lpThreadParameter != NULL) {
			CMWHDRCaptureUI *t_p_this =
				(CMWHDRCaptureUI *)lpThreadParameter;
...
protected:
	...
	unsigned char *setup_for_hdr(unsigned char **t_ppuc_stream_buf,
