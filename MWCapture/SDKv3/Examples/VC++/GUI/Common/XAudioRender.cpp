...
BOOL XAudioRender::Initialize()
{
	...
	if (FAILED(hr)) {
		...
		if (g_DllGetClassObject == NULL)
			g_DllGetClassObject = (Func)GetProcAddress(
				m_h_lib, "DllGetClassObject");
		if (g_DllGetClassObject != NULL) {
			IClassFactory *pClassFactory = NULL;
			HRESULT hr = g_DllGetClassObject(
				__uuidof(XAudio2), IID_IClassFactory,
				(LPVOID *)&pClassFactory);
			if (S_OK == hr) {
				hr = pClassFactory->CreateInstance(
					NULL, __uuidof(IXAudio2),
					(LPVOID *)&t_p_ixaudio2);
				if (S_OK == hr) {
					hr = t_p_ixaudio2->Initialize(
						m_ui_flags);
	...
	for (int i = 0; i < t_ui_count; i++) {
		...
		if (t_device_details.Role == GlobalDefaultDevice) {
			t_dw_channelmask =
				t_device_details.OutputFormat.dwChannelMask;
			break;
		}
	}
...
BOOL XAudioRender::CreateAudio(int t_n_samplerate, int t_n_channels,
			       int t_n_bitdepth)
{
	HRESULT hr = S_FALSE;
	WAVEFORMATEX t_fmt;
	...
	t_fmt.cbSize = 0;
	hr = m_p_xaudio2->CreateSourceVoice(&m_p_source_voice, &t_fmt, 0,
					    XAUDIO2_DEFAULT_FREQ_RATIO,
					    &m_voice_context, NULL, NULL);
	if (FAILED(hr)) {
		return FALSE;
	}
	...
	for (int i = 0; i < MAX_BUFFER_COUNT; i++) {
		m_p_buffer_arr[i] =
			new unsigned char[SAMPLES_PER_FRAME * 8 * 4];
	...
	m_arr_f_outputmatrix =
		new float[m_queue_destion_channels.size() * m_n_source_channels];
	for (int i = 0;
	     i < m_queue_destion_channels.size() * m_n_source_channels; i++) {
		m_arr_f_outputmatrix[i] = 0.0f;
	}
	for (int i = 0; i < m_n_source_channels; i++) {
		for (int j = 0; j < m_queue_destion_channels.size(); j++) {
			if (i == j) {
				m_arr_f_outputmatrix[j * m_n_source_channels +
						     i] = 1.0f;
			}
		}
	}
	m_p_source_voice->SetOutputMatrix(m_p_master_voice, m_n_source_channels,
					  m_queue_destion_channels.size(),
					  m_arr_f_outputmatrix,
					  XAUDIO2_COMMIT_NOW);
...
BOOL XAudioRender::SetOutPutMatrix(soutputcell *t_arr_output, int t_n_num)
{
	...
	for (int i = 0; i < t_n_num; i++) {
		...
		if (!(t_n_des >= 0 &&
		      t_n_des < m_queue_destion_channels.size())) {
			t_re = FALSE;
			continue;
		}
		...
		m_arr_f_outputmatrix[t_n_des * m_n_source_channels + t_n_sour] =
			t_f_vol;
		m_p_source_voice->SetOutputMatrix(
			m_p_master_voice, m_n_source_channels,
			m_queue_destion_channels.size(), m_arr_f_outputmatrix,
			XAUDIO2_COMMIT_NOW);
	}
