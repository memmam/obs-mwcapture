...
long CChartLogarithmicAxis::ValueToScreenStandard(double Value) const
{
	...
	Offset = (int)floor((log10(Value) - log10(m_MinValue)) *
			    GetAxisLenght() /
			    (log10(m_MaxValue) - log10(m_MinValue)));
	if (m_bIsHorizontal) {
		if (!m_bIsInverted)
...
double CChartLogarithmicAxis::ScreenToValue(long ScreenVal) const
{
	...
	return (pow(10.0, (AxisOffset * 1.0 / GetAxisLenght() *
			   (log10(m_MaxValue) - log10(m_MinValue))) +
				  log10(m_MinValue)));
}
...
void CChartLogarithmicAxis::GetScrollbarSteps(int &iTotalSteps,
					      int &iCurrentStep)
{
	...
	if ((m_MaxValue - m_MinValue) == 0 || (SeriesMax - SeriesMin) == 0 ||
		...
		iTotalSteps =
			(int)ceil(log(SeriesMax / SeriesMin) / log10(dStep));
		iCurrentStep =
			(int)(log(m_MinValue / SeriesMin) / log10(dStep));
	}
}
