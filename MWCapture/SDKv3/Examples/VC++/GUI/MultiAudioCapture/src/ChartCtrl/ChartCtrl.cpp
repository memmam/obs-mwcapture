...
COLORREF pSeriesColorTable[] = {RGB(255, 0, 0),   RGB(0, 150, 0),
				RGB(0, 0, 255),   RGB(255, 255, 0),
				RGB(0, 255, 255), RGB(255, 128, 0),
				RGB(128, 0, 128), RGB(128, 128, 0),
				RGB(255, 0, 255), RGB(64, 128, 128)};
...
void CChartCtrl::OnPaint()
{
	...
	CRect rect;
	GetClientRect(&rect);
	dc.BitBlt(0, 0, rect.Width(), rect.Height(), &m_BackgroundDC, 0, 0,
		  SRCCOPY);
...
CChartCtrl::CreateCrossHairCursor(bool bSecondaryHorizAxis,
				  bool bSecondaryVertAxis)
{
	...
	CChartCrossHairCursor *pNewCursor =
		new CChartCrossHairCursor(this, pHorizAxis, pVertAxis);
...
CChartDragLineCursor *CChartCtrl::CreateDragLineCursor(EAxisPos relatedAxis)
{
	...
	CChartDragLineCursor *pNewCursor =
		new CChartDragLineCursor(this, m_pAxes[relatedAxis]);
	m_mapCursors[pNewCursor->GetCursorId()] = pNewCursor;
	return pNewCursor;
}
...
int CChartCtrl::Create(CWnd *pParentWnd, const RECT &rect, UINT nID,
		       DWORD dwStyle)
{
	dwStyle |= WS_CLIPCHILDREN;
	int Result = CWnd::Create(CHARTCTRL_CLASSNAME, _T(""), dwStyle, rect,
				  pParentWnd, nID);
...
void CChartCtrl::SetBackGradient(COLORREF Col1, COLORREF Col2,
...
void CChartCtrl::RefreshCtrl()
{
	...
	if (!m_BackgroundDC.GetSafeHdc()) {
		...
		memBitmap.CreateCompatibleBitmap(&dc, ClientRect.Width(),
						 ClientRect.Height());
...
CChartCtrl::CreateCandlestickSerie(bool bSecondaryHorizAxis,
				   bool bSecondaryVertAxis)
{
...
void CChartCtrl::OnMouseMove(UINT nFlags, CPoint point)
{
	if (m_bRMouseDown && m_bPanEnabled) {
		if (point != m_PanAnchor) {
			EnableRefresh(false);
			if (m_pAxes[LeftAxis])
				m_pAxes[LeftAxis]->PanAxis(m_PanAnchor.y,
							   point.y);
			if (m_pAxes[RightAxis])
				m_pAxes[RightAxis]->PanAxis(m_PanAnchor.y,
							    point.y);
			if (m_pAxes[BottomAxis])
				m_pAxes[BottomAxis]->PanAxis(m_PanAnchor.x,
							     point.x);
			if (m_pAxes[TopAxis])
				m_pAxes[TopAxis]->PanAxis(m_PanAnchor.x,
							  point.x);
			RefreshCtrl();
			EnableRefresh(true);
	...
	if (pWnd != this) {
		CChartScrollBar *pScrollBar =
			dynamic_cast<CChartScrollBar *>(pWnd);
		if (pScrollBar)
	...
	if (m_bZoomEnabled) {
		if ((m_rectZoomArea.left > m_rectZoomArea.right) ||
			...
			if (m_pAxes[BottomAxis]) {
				...
				m_pAxes[BottomAxis]->SetZoomMinMax(MinVal,
								   MaxVal);
			}
			...
			if (m_pAxes[LeftAxis]) {
				if (m_pAxes[LeftAxis]->IsInverted()) {
					MaxVal =
						m_pAxes[LeftAxis]->ScreenToValue(
							m_rectZoomArea.bottom);
					MinVal =
						m_pAxes[LeftAxis]->ScreenToValue(
							m_rectZoomArea.top);
				} else {
				...
				m_pAxes[LeftAxis]->SetZoomMinMax(MinVal,
								 MaxVal);
			}
			...
			if (m_pAxes[TopAxis]) {
				if (m_pAxes[TopAxis]->IsInverted()) {
					MaxVal =
						m_pAxes[TopAxis]->ScreenToValue(
							m_rectZoomArea.left);
					MinVal =
						m_pAxes[TopAxis]->ScreenToValue(
							m_rectZoomArea.right);
				} else {
					MinVal =
						m_pAxes[TopAxis]->ScreenToValue(
							m_rectZoomArea.left);
			...
			if (m_pAxes[RightAxis]) {
				if (m_pAxes[RightAxis]->IsInverted()) {
					MaxVal =
						m_pAxes[RightAxis]->ScreenToValue(
							m_rectZoomArea.bottom);
					MinVal =
						m_pAxes[RightAxis]->ScreenToValue(
							m_rectZoomArea.top);
				...
				m_pAxes[RightAxis]->SetZoomMinMax(MinVal,
								  MaxVal);
			}
				...
				const CPoint &screenPoint) const
{
	if (m_pMouseListener) {
		...
		if (m_pTitles->IsPointInside(screenPoint))
			m_pMouseListener->OnMouseEventTitle(mouseEvent,
							    screenPoint);
		if (m_pLegend->IsPointInside(screenPoint))
			m_pMouseListener->OnMouseEventLegend(mouseEvent,
							     screenPoint);
		for (int i = 0; i < 4; i++) {
			if (m_pAxes[i] &&
			    m_pAxes[i]->IsPointInside(screenPoint))
				m_pMouseListener->OnMouseEventAxis(
					mouseEvent, screenPoint, m_pAxes[i]);
		}
		if (m_PlottingRect.PtInRect(screenPoint))
			m_pMouseListener->OnMouseEventPlotArea(mouseEvent,
							       screenPoint);
	}
...
{
	CChartScrollBar *pChartBar =
		dynamic_cast<CChartScrollBar *>(pScrollBar);
	if (pChartBar)
...
void CChartCtrl::Print(const TChartString &strTitle, CPrintDialog *pPrntDialog)
{
	...
	if (pPrntDialog == NULL) {
		...
		if (printDlg.DoModal() !=
		    IDOK) // Get printer settings from user
			return;
...
void CChartCtrl::OnBeginPrinting(CDC *pDC, CPrintInfo *pInfo)
{
	...
	m_LogicalPageSize.cx = ScreenPixelsPerInch.cx * m_PaperSize.cx /
			       PaperPixelsPerInch.cx * 3 / 4;
	m_LogicalPageSize.cy = ScreenPixelsPerInch.cy * m_PaperSize.cy /
			       PaperPixelsPerInch.cy * 3 / 4;
...
#if _MFC_VER > 0x0600
void CChartCtrl::SaveAsImage(const TChartString &strFilename, const CRect &rect,
			     int nBPP, REFGUID guidFileType)
{
	CImage chartImage;
