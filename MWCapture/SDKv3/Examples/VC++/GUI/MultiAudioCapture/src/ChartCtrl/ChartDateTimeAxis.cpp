...
void CChartDateTimeAxis::SetTickIncrement(bool bAuto, TimeInterval Interval,
					  int Multiplier)
{
	m_bAutoTicks = bAuto;
	if (!m_bAutoTicks) {
...
double CChartDateTimeAxis::GetFirstTickValue() const
{
	...
	if (m_bDiscrete) {
		...
		case tiMonth:
			...
			break;
		case tiYear:
			dtTick = AddMonthToDate(dtTick,
						-12 * m_iDTTickIntervalMult);
			break;
		}
	}
	return dRetVal;
...
bool CChartDateTimeAxis::GetNextTickValue(double dCurrentTick,
						...
						int iMonthsToAdd) const
{
	...
	newDate.SetDateTime(nYear, nMonths % 12 + 1, Date.GetDay(),
...
void CChartDateTimeAxis::RefreshTickIncrement()
{
	...
	int MaxTickNumber =
		(int)fabs((m_EndPos - m_StartPos) / PixelSpace * 1.0);
	if (MaxTickNumber == 0)
		MaxTickNumber = 1;
	...
	COleDateTimeSpan minTickInterval =
		(EndDate - StartDate) / MaxTickNumber;
...
double CChartDateTimeAxis::GetTickBeforeVal(double dValue) const
{
	...
	case tiSecond: {
		...
		totalSecs = (totalSecs / m_iDTTickIntervalMult) *
			    m_iDTTickIntervalMult;
		int Days = totalSecs / 86400; // 86400 seconds in one day
		int Hours =
			(totalSecs % 86400) / 3600; // 3600 seconds in one hour
		int Minutes = ((totalSecs % 86400) % 3600) /
			      60; // 60 seconds in one minute
	...
	case tiMinute: {
		...
		totalMinutes = (totalMinutes / m_iDTTickIntervalMult) *
			       m_iDTTickIntervalMult;
		int Days = totalMinutes / 1440; // 1440 minutes in one day
		int Hours =
			(totalMinutes % 1440) / 60; // 60 minutes in one hour
	...
	case tiHour: {
		...
		totalHours = (totalHours / m_iDTTickIntervalMult) *
			     m_iDTTickIntervalMult;
	...
	case tiDay: {
		...
		totalDays = (totalDays / m_iDTTickIntervalMult) *
			    m_iDTTickIntervalMult;
	...
	case tiMonth: {
		...
		int monthDiff =
			valueTime.GetMonth() - m_ReferenceTick.GetMonth();
		int totalMonths =
			((yearDiff * 12 + monthDiff) / m_iDTTickIntervalMult) *
			m_iDTTickIntervalMult;
		tickBefore = AddMonthToDate(m_ReferenceTick, totalMonths);
	} break;
	case tiYear: {
		int yearDiff = valueTime.GetYear() - m_ReferenceTick.GetYear();
		int year = ((yearDiff) / m_iDTTickIntervalMult) *
			   m_iDTTickIntervalMult;
