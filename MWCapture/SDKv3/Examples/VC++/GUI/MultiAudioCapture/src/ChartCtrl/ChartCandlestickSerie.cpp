...
CChartCandlestickSerie::CChartCandlestickSerie(CChartCtrl *pParent)
	: CChartSerieBase<SChartCandlestickPoint>(pParent),
	  m_iCandlestickWidth(7)
{
	m_bShadow = true;
}
...
void CChartCandlestickSerie::AddPoint(double XVal, double Low, double High,
...
void CChartCandlestickSerie::Draw(CDC *pDC)
{
	...
	if (pDC->GetSafeHdc()) {
		...
		for (m_uLastDrawnPoint; m_uLastDrawnPoint < GetPointsCount();
		     m_uLastDrawnPoint++) {
			SChartCandlestickPoint Point =
...
void CChartCandlestickSerie::DrawCandleStick(CDC *pDC,
					     SChartCandlestickPoint Point)
{
	int ScreenXVal = m_pHorizontalAxis->ValueToScreen(Point.XVal);
	...
	if (m_bShadow) {
		pDC->MoveTo(ScreenXVal + m_iShadowDepth,
			    ScreenOpen + m_iShadowDepth);
		pDC->LineTo(ScreenXVal + m_iShadowDepth,
			    ScreenHigh + m_iShadowDepth);
		pDC->MoveTo(ScreenXVal + m_iShadowDepth,
			    ScreenClose + m_iShadowDepth);
		pDC->LineTo(ScreenXVal + m_iShadowDepth,
			    ScreenLow + m_iShadowDepth);
	}
