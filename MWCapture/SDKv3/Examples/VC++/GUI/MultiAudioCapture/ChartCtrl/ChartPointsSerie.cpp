...
void CChartPointsSerie::Draw(CDC *pDC)
{
	...
	for (m_uLastDrawnPoint; m_uLastDrawnPoint < (int)GetPointsCount();
		...
		CRect PointRect;
		PointRect.SetRect(ScreenPoint.x - m_iXPointSize / 2,
				  ScreenPoint.y - m_iYPointSize / 2,
				  ScreenPoint.x + m_iXPointSize / 2,
				  ScreenPoint.y + m_iYPointSize / 2);
		CRect ShadowRect =
			PointRect + CSize(m_iShadowDepth, m_iShadowDepth);
		...
		case ptTriangle: {
			...
			TrPoints[2].x =
				PointRect.left +
				(int)fabs((PointRect.left - PointRect.right) /
					  2.0);
			...
			if (m_bShadow) {
				...
				for (int i = 0; i < 3; i++) {
					ShadowPoints[i] = TrPoints[i] +
							  CSize(m_iShadowDepth,
								m_iShadowDepth);
				}
...
void CChartPointsSerie::DrawLegend(CDC *pDC, const CRect &rectBitmap) const
{
	...
	if ((rectBitmap.Height() > m_iYPointSize) &&
	    (rectBitmap.Width() > m_iXPointSize)) {
		int XOffset = rectBitmap.left + rectBitmap.Width() / 2 -
			      m_iXPointSize / 2;
		int YOffset = rectBitmap.top + rectBitmap.Height() / 2 -
			      m_iYPointSize / 2;
	...
	case ptTriangle: {
		...
		TrPoints[2].x =
			PointRect.left +
			(int)fabs((PointRect.left - PointRect.right) / 2.0);
		TrPoints[2].y = PointRect.top;
