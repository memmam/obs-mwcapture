...
CChartGanttSerie::CChartGanttSerie(CChartCtrl *pParent)
	: CChartSerieBase<SChartGanttPoint>(pParent),
	  m_iBarWidth(10),
	  m_iBorderWidth(1),
...
CRect CChartGanttSerie::GetBarRectangle(unsigned uPointIndex) const
{
	...
	CRect PointRect(min(PointXStart, PointXEnd),
			min(PointYStart, PointYEnd),
			max(PointXStart, PointXEnd),
			max(PointYStart, PointYEnd));
	return PointRect;
}

void CChartGanttSerie::DrawBar(CDC *pDC, CBrush *pFillBrush,
			       CBrush *pBorderBrush, CRect BarRect)
{
	if (m_bShadow) {
