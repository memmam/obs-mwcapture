...
void CChartLineSerie::DrawAll(CDC *pDC)
{
	...
	if (m_iPenStyle != PS_SOLID) {
		...
		ShadowPen.CreatePen(PS_GEOMETRIC | m_iPenStyle, m_iLineWidth,
				    &lb);
	} else {
	...
	if (m_bSmooth) {
		...
		GetBezierControlPoints(uFirst, uLast, pKnots, pFirstControlPts,
				       pSecondControlPts);
		...
		for (unsigned n = 0; n < Count - 1; n++) {
			ValueToScreen(pKnots[n].X, pKnots[n].Y,
				      pBezierPts[index]);
			ValueToScreen(pFirstControlPts[n].X,
				      pFirstControlPts[n].Y,
				      pBezierPts[index + 1]);
			ValueToScreen(pSecondControlPts[n].X,
				      pSecondControlPts[n].Y,
				      pBezierPts[index + 2]);

			if (m_bShadow) {
				pShadowPts[index] = pBezierPts[index];
				pShadowPts[index].Offset(m_iShadowDepth,
							 m_iShadowDepth);
				pShadowPts[index + 1] = pBezierPts[index + 1];
				pShadowPts[index + 1].Offset(m_iShadowDepth,
							     m_iShadowDepth);
				pShadowPts[index + 2] = pBezierPts[index + 2];
				pShadowPts[index + 2].Offset(m_iShadowDepth,
							     m_iShadowDepth);
			}
			index += 3;
		}
		ValueToScreen(pKnots[Count - 1].X, pKnots[Count - 1].Y,
			      pBezierPts[index]);
		if (m_bShadow) {
			pShadowPts[index] = pBezierPts[index];
			pShadowPts[index].Offset(m_iShadowDepth,
...
void CChartLineSerie::Draw(CDC *pDC)
{
	...
	if (pDC->GetSafeHdc()) {
		...
		if (m_iPenStyle != PS_SOLID) {
			...
			lb.lbStyle = BS_SOLID;
			lb.lbColor = m_SerieColor;
			NewPen.CreatePen(PS_GEOMETRIC | m_iPenStyle,
					 m_iLineWidth, &lb);
		} else {
			NewPen.CreatePen(m_iPenStyle, m_iLineWidth,
					 m_SerieColor);
		}
		CPen *pOldPen;
		...
		for (m_uLastDrawnPoint;
		     m_uLastDrawnPoint < GetPointsCount() - 1;
		     m_uLastDrawnPoint++) {
				     ...
				     unsigned &uIndex) const
{
	...
	for (unsigned i = uFirst; i < uLast; i++) {
		...
		if (IsNearLine(ScreenPointOrig.x, ScreenPointOrig.y,
			       ScreenPointDest.x, ScreenPointDest.y,
...
bool CChartLineSerie::IsNearLine(long Axl, long Ayl, long Bxl, long Byl,
				 long Cxl, long Cyl) const
{
	double Ax = Axl;
	double Ay = Ayl;
	double Bx = Bxl;
	...
	if ((0 <= r) && (r <= 1)) {
		...
		if ((abs(Cx - Px) <= 3.0) && (abs(Cy - Py) <= 3.0)) {
			return true;
		}
	}
	return false;
