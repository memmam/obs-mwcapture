...
void CChartTitle::SetLineFont(int iLineIndex, int iPointSize,
...
void CChartTitle::Draw(CDC *pDC)
{
	...
	for (size_t i = 0; i < TitleCount; i++) {
		...
		int XPos = m_TitleRect.left +
			   (int)fabs((m_TitleRect.left - m_TitleRect.right) /
				     2.0) -
			   TextWidth / 2;
		...
		pDC->ExtTextOut(XPos, m_TitleRect.top + YPos, ETO_CLIPPED, NULL,
				m_StringArray[i].c_str(), NULL);
