			   ...
			   int nTexRef)
{
	...
	char *actual_path =
		sdkFindFilePath(pCtx->mModuleName.c_str(), exec_path);
	...
	if (pCtx->mModuleName.rfind(".ptx") != string::npos) {
		...
		cuStatus = cuModuleLoadDataEx(&pCtx->cuModule_,
					      ptx_source.c_str(), jitNumOptions,
				       ...
				       CUfunction *fpCudaKernel)
{
	...
	CUresult cuStatus = cuModuleGetFunction(
		&(pCtx->pCudaKernels_[idx].fpCuda), pCtx->cuModule_, func_name);
					...
					CUdeviceptr *pGlobalMem)
{
	...
	CUresult cuStatus =
		cuModuleGetGlobal(&(pCtx->pGlobalMem_[idx].devicePtr),
