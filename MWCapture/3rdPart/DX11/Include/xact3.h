...
static const XACTVOLUME XACTVOLUME_MAX =
	16777216.0f; // Maximum acceptable volume level (2^24) - matches XAudio2 max volume
...
static const DWORD XACT_FLAG_STOP_RELEASE =
	0x00000000; // Stop with release envelope (or as authored), for looping waves this acts as break loop.
...
static const DWORD XACT_STATE_PAUSED =
	0x00000040; // Paused (Can be combined with some of the other state flags above)
...
typedef struct XACT_RENDERER_DETAILS {
	WCHAR rendererID
		[XACT_RENDERER_ID_LENGTH]; // The string ID for the rendering device.
	WCHAR displayName
		[XACT_RENDERER_NAME_LENGTH]; // A friendly name suitable for display to a human.
...
#define XACT_ENGINE_LOOKAHEAD_DEFAULT \
	250 // Default look-ahead time of 250ms can be used during XACT engine initialization.
...
typedef struct XACT_RUNTIME_PARAMETERS {
	...
	IXAudio2MasteringVoice *
		pMasteringVoice; // Mastering voice to be used by the engine, if pXAudio2 is not NULL.
...
typedef struct XACT_CUE_PROPERTIES {
	CHAR friendlyName
		[XACT_CUE_NAME_LENGTH]; // Empty if the soundbank doesn't contain any friendly names
...
typedef struct XACT_SOUND_PROPERTIES {
	...
	XACT_TRACK_PROPERTIES arrTrackProperties
		[1]; // Array of active track properties (has numTracks number of elements)
...
typedef struct XACT_WAVE_PROPERTIES {
	char friendlyName
		[WAVEBANK_ENTRYNAME_LENGTH]; // Friendly name for the wave; empty if the wavebank doesn't contain friendly names.
...
static const XACTNOTIFICATIONTYPE XACTNOTIFICATIONTYPE_WAVEPLAY =
	13; // None, SoundBank, SoundBank & cue index, cue instance, WaveBank, wave instance
static const XACTNOTIFICATIONTYPE XACTNOTIFICATIONTYPE_WAVESTOP =
	14; // None, SoundBank, SoundBank & cue index, cue instance, WaveBank, wave instance
static const XACTNOTIFICATIONTYPE XACTNOTIFICATIONTYPE_WAVELOOPED =
	15; // None, SoundBank, SoundBank & cue index, cue instance, WaveBank, wave instance
...
__inline HRESULT __stdcall IXACT3SoundBank_Prepare(
	...
	XACTTIME timeOffset, __deref_out IXACT3Cue **ppCue)
{
	return pSoundBank->lpVtbl->Prepare(pSoundBank, nCueIndex, dwFlags,
					   timeOffset, ppCue);
}
...
__inline HRESULT __stdcall IXACT3SoundBank_Play(
	...
	XACTTIME timeOffset, __deref_opt_out IXACT3Cue **ppCue)
{
	return pSoundBank->lpVtbl->Play(pSoundBank, nCueIndex, dwFlags,
					timeOffset, ppCue);
}
...
DECLARE_INTERFACE(IXACT3WaveBank)
{
	...
	STDMETHOD_(XACTINDEX, GetWaveIndex)
	(THIS_ __in PCSTR szFriendlyName) PURE;
	STDMETHOD(GetWaveProperties)
	(THIS_ XACTINDEX nWaveIndex,
	 __out LPXACT_WAVE_PROPERTIES pWaveProperties) PURE;
	STDMETHOD(Prepare)
...
__inline HRESULT __stdcall IXACT3WaveBank_Play(__in IXACT3WaveBank *pWaveBank,
					       XACTINDEX nWaveIndex,
					       DWORD dwFlags,
					       DWORD dwPlayOffset,
					       XACTLOOPCOUNT nLoopCount,
					       __deref_out IXACT3Wave **ppWave)
{
	return pWaveBank->Play(nWaveIndex, dwFlags, dwPlayOffset, nLoopCount,
			       ppWave);
}
...
__inline HRESULT __stdcall IXACT3Wave_SetMatrixCoefficients(
	__in IXACT3Wave *pWave, UINT32 uSrcChannelCount,
	UINT32 uDstChannelCount, __in float *pMatrixCoefficients)
{
...
__inline HRESULT __stdcall IXACT3Cue_SetOutputVoiceMatrix(
	...
	__in_ecount(SourceChannels *DestinationChannels)
		const float *pLevelMatrix)
{
	return pCue->lpVtbl->SetOutputVoiceMatrix(pDestinationVoice,
						  SourceChannels,
						  DestinationChannels,
						  pLevelMatrix);
}
...
STDAPI IXACT3Engine_PrepareWave(__in IXACT3Engine *pEngine, DWORD dwFlags,
				...
				DWORD dwPlayOffset, XACTLOOPCOUNT nLoopCount,
				__deref_out IXACT3Wave **ppWave);
STDAPI IXACT3Engine_PrepareInMemoryWave(__in IXACT3Engine *pEngine,
					DWORD dwFlags, WAVEBANKENTRY entry,
					__in_opt DWORD *pdwSeekTable,
					__in_opt BYTE *pbWaveData,
					DWORD dwPlayOffset,
					XACTLOOPCOUNT nLoopCount,
					__deref_out IXACT3Wave **ppWave);
STDAPI IXACT3Engine_PrepareStreamingWave(
	...
	__in_opt DWORD *pdwSeekTable, DWORD dwPlayOffset,
...
DECLARE_INTERFACE_(IXACT3Engine, IUnknown)
{
	...
	STDMETHOD(GetRendererCount)
	(THIS_ __out XACTINDEX * pnRendererCount) PURE;
	...
	(THIS_ __in const XACT_RUNTIME_PARAMETERS *pParams) PURE;
	...
	(THIS_ __in const void *pvBuffer, DWORD dwSize, DWORD dwFlags,
	 DWORD dwAllocAttributes, __deref_out IXACT3SoundBank **ppSoundBank)
		PURE;
	...
	(THIS_ DWORD dwFlags, WAVEBANKENTRY entry,
	 __in_opt DWORD * pdwSeekTable, __in_opt BYTE * pbWaveData,
	 DWORD dwPlayOffset, XACTLOOPCOUNT nLoopCount,
	 __deref_out IXACT3Wave * *ppWave) PURE;
	...
	(THIS_ __in const XACT_NOTIFICATION_DESCRIPTION *pNotificationDesc)
		PURE;
	STDMETHOD(UnRegisterNotification)
	(THIS_ __in const XACT_NOTIFICATION_DESCRIPTION *pNotificationDesc)
		PURE;

	STDMETHOD_(XACTCATEGORY, GetCategory)
	(THIS_ __in PCSTR szFriendlyName) PURE;
	STDMETHOD(Stop)(THIS_ XACTCATEGORY nCategory, DWORD dwFlags) PURE;
	STDMETHOD(SetVolume)
	(THIS_ XACTCATEGORY nCategory, XACTVOLUME nVolume) PURE;
...
__inline HRESULT __stdcall IXACT3Engine_CreateSoundBank(
	...
	__deref_out IXACT3SoundBank **ppSoundBank)
{
	return pEngine->CreateSoundBank(pvBuffer, dwSize, dwFlags,
	...
	XACTLOOPCOUNT nLoopCount, __deref_out IXACT3Wave **ppWave)
{
	return pEngine->PrepareWave(dwFlags, szWavePath, wStreamingPacketSize,
				    dwAlignment, dwPlayOffset, nLoopCount,
				    ppWave);
}
...
__inline HRESULT __stdcall IXACT3Engine_PrepareInMemoryWave(
	...
	__deref_out IXACT3Wave **ppWave)
{
	return pEngine->PrepareInMemoryWave(dwFlags, entry, pdwSeekTable,
					    pbWaveData, dwPlayOffset,
					    nLoopCount, ppWave);
}
...
__inline HRESULT __stdcall IXACT3Engine_CreateSoundBank(
	...
	__deref_out IXACT3SoundBank **ppSoundBank)
{
	return pEngine->lpVtbl->CreateSoundBank(pEngine, pvBuffer, dwSize,
						dwFlags, dwAllocAttributes,
						ppSoundBank);
}
...
__inline HRESULT __stdcall IXACT3Engine_CreateInMemoryWaveBank(
	...
	__deref_out IXACT3WaveBank **ppWaveBank)
{
	return pEngine->lpVtbl->CreateInMemoryWaveBank(pEngine, pvBuffer,
						       dwSize, dwFlags,
						       dwAllocAttributes,
						       ppWaveBank);
}
...
__inline HRESULT __stdcall IXACT3Engine_CreateStreamingWaveBank(
	...
	__deref_out IXACT3WaveBank **ppWaveBank)
{
	return pEngine->lpVtbl->CreateStreamingWaveBank(pEngine, pParms,
							ppWaveBank);
}
	...
	XACTLOOPCOUNT nLoopCount, __deref_out IXACT3Wave **ppWave)
{
	return pEngine->lpVtbl->PrepareStreamingWave(
		pEngine, dwFlags, entry, streamingParams, dwAlignment,
		pdwSeekTable, dwPlayOffset, nLoopCount, ppWave);
}
...
__inline HRESULT __stdcall IXACT3Engine_RegisterNotification(
	...
	__in const XACT_NOTIFICATION_DESCRIPTION *pNotificationDesc)
{
	return pEngine->lpVtbl->RegisterNotification(pEngine,
						     pNotificationDesc);
}
...
__inline HRESULT __stdcall IXACT3Engine_UnRegisterNotification(
	...
	__in const XACT_NOTIFICATION_DESCRIPTION *pNotificationDesc)
{
	return pEngine->lpVtbl->UnRegisterNotification(pEngine,
						       pNotificationDesc);
}
	...
	DWORD dwCreationFlags, __deref_out IXACT3Engine **ppEngine)
{
	...
	BOOL debug = (dwCreationFlags & XACT_FLAG_API_DEBUG_MODE) ? TRUE
								  : FALSE;
	BOOL audition = (dwCreationFlags & XACT_FLAG_API_AUDITION_MODE) ? TRUE
									: FALSE;
	...
	if (!debug && !audition &&
		...
		if (RegQueryValueEx(key, XACT_DEBUGENGINE_REGISTRY_VALUE, NULL,
				    &type, (LPBYTE)&data,
	...
	hr = CoCreateInstance(audition ? __uuidof(XACTAuditionEngine)
				       : (debug ? __uuidof(XACTDebugEngine)
						: __uuidof(XACTEngine)),
	...
	if (FAILED(hr) && debug && !audition) {
		hr = CoCreateInstance(__uuidof(XACTEngine), NULL,
				      CLSCTX_INPROC_SERVER,
				      __uuidof(IXACT3Engine),
				      (void **)ppEngine);
	}
	...
	DWORD dwCreationFlags, __deref_out IXACT3Engine **ppEngine)
{
	...
	hr = CoCreateInstance(audition ? &CLSID_XACTAuditionEngine
				       : (debug ? &CLSID_XACTDebugEngine
						: &CLSID_XACTEngine),
			      NULL, CLSCTX_INPROC_SERVER, &IID_IXACT3Engine,
			      (void **)ppEngine);

	// If debug engine does not exist fallback to retail version
	if (FAILED(hr) && debug && !audition) {
		hr = CoCreateInstance(&CLSID_XACTEngine, NULL,
				      CLSCTX_INPROC_SERVER, &IID_IXACT3Engine,
				      (void **)ppEngine);
	}
...
#define XACTENGINE_E_EXPIRED \
	XACTENGINEERROR(     \
		0x003) // The engine has expired (demo or pre-release version)
...
#define XACTENGINE_E_INCALLBACK \
	XACTENGINEERROR(        \
		0x012) // Invalid call of method of function from callback
...
#define XACTENGINE_E_SEEKTIMEBEYONDWAVEEND \
	XACTENGINEERROR(                   \
		0x01a) // Time offset for seeking is beyond the wave end.
