...
DECLARE_INTERFACE_(ID3DX10Mesh, IUnknown)
{
	...
	STDMETHOD(GetIndexBuffer)
	(THIS_ ID3DX10MeshBuffer * *ppIndexBuffer) PURE;
	...
	(THIS_ D3DX10_ATTRIBUTE_RANGE * pAttribTable, UINT * pAttribTableSize)
		PURE;
	...
	STDMETHOD(GetAdjacencyBuffer)
	(THIS_ ID3DX10MeshBuffer * *ppAdjacency) PURE;

	STDMETHOD(SetPointRepData)(THIS_ CONST UINT * pPointReps) PURE;
	STDMETHOD(GetPointRepBuffer)
	(THIS_ ID3DX10MeshBuffer * *ppPointReps) PURE;
	...
	(THIS_ UINT AttribId, UINT InstanceCount, UINT StartInstanceLocation)
		PURE;

	STDMETHOD(GetDeviceVertexBuffer)
	(THIS_ UINT iBuffer, ID3D10Buffer * *ppVertexBuffer) PURE;
	STDMETHOD(GetDeviceIndexBuffer)
	(THIS_ ID3D10Buffer * *ppIndexBuffer) PURE;
};
...
DECLARE_INTERFACE_(ID3DX10SkinInfo, IUnknown)
{
	...
	STDMETHOD(RemapVertices)
	(THIS_ UINT NewVertexCount, UINT * pVertexRemap) PURE;
	...
	(THIS_ UINT BoneIndex, UINT InfluenceCount, UINT * pIndices,
	 float *pWeights) PURE;
	...
	STDMETHOD(Compact)
	(THIS_ UINT MaxPerVertexInfluences, UINT ScaleMode, float MinWeight)
		PURE;
