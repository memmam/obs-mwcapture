...
#define DSPASSERT(exp)                                        \
	if (!(exp)) {                                         \
		OutputDebugStringA("XDSP ASSERT: " #exp       \
				   ", {" __FUNCTION__ "}\n"); \
		__debugbreak();                               \
	}
#else
...
#pragma warning(push)
#pragma warning( \
	disable : 4328 4640) // disable "indirection alignment of formal parameter", "construction of local static object is not thread-safe" compile warnings
...
__forceinline void ButterflyDIT4_1(__inout XVECTORREF r1, __inout XVECTORREF i1)
{
	...
	XVECTOR rZrWiZiW = _mm_shuffle_ps(
		rTemp, iTemp,
		_MM_SHUFFLE(3, 2, 3, 2)); // [rTempZ|rTempW|iTempZ|iTempW]
	XVECTOR rZiWrZiW = _mm_shuffle_ps(
		rZrWiZiW, rZrWiZiW,
		_MM_SHUFFLE(3, 0, 3, 0)); // [rTempZ|iTempW|rTempZ|iTempW]
	XVECTOR iZrWiZrW = _mm_shuffle_ps(
		rZrWiZiW, rZrWiZiW,
	...
	r1 = _mm_add_ps(
		_mm_shuffle_ps(
			rTemp, rTemp,
			_MM_SHUFFLE(1, 0, 1,
				    0)), // [rTempX| rTempY| rTempX| rTempY] +
		_mm_xor_ps(rZiWrZiW,
			   vDFT4SignBits2)); // [rTempZ| iTempW|-rTempZ|-iTempW]
	i1 = _mm_add_ps(
		_mm_shuffle_ps(
			iTemp, iTemp,
			_MM_SHUFFLE(1, 0, 1,
				    0)), // [iTempX| iTempY| iTempX| iTempY] +
		_mm_xor_ps(iZrWiZrW,
			   vDFT4SignBits3)); // [iTempZ|-rTempW|-iTempZ| rTempW]
}
			...
			const UINT32 uCount = 1)
{
	...
	for (UINT32 uIndex = 0; uIndex < uCount; ++uIndex) {
		...
		XVECTOR oddsR =
			_mm_shuffle_ps(pR[0], pR[1], _MM_SHUFFLE(3, 1, 3, 1));
		XVECTOR evensR =
			_mm_shuffle_ps(pR[0], pR[1], _MM_SHUFFLE(2, 0, 2, 0));
		XVECTOR oddsI =
			_mm_shuffle_ps(pI[0], pI[1], _MM_SHUFFLE(3, 1, 3, 1));
		XVECTOR evensI =
			_mm_shuffle_ps(pI[0], pI[1], _MM_SHUFFLE(2, 0, 2, 0));
			 ...
			 const UINT32 uCount = 1)
{
	...
	for (UINT32 uIndex = 0; uIndex < uCount; ++uIndex) {
		ButterflyDIT4_4(pReal[uIndex * 4], pReal[uIndex * 4 + 1],
				...
				pImaginary[uIndex * 4 + 3], aUnityTableReal,
				aUnityTableImaginary, 1, TRUE);
	}
}
    ...
    const UINT32 uLength, const UINT32 uCount = 1)
{
	...
	const UINT32 uStride = uLength >>
			       4; // stride between butterfly elements
	...
	for (UINT32 uIndex = 0; uIndex < (uTotal_vectors >> 2); ++uIndex) {
		const UINT32 n = ((uIndex & uStrideInvMask) << 2) +
				 (uIndex & uStrideMask);
		ButterflyDIT4_4(
			pReal[n], pReal[n + uStride], pReal[n + uStride2],
			pReal[n + uStride3], pImaginary[n],
			pImaginary[n + uStride], pImaginary[n + uStride2],
	...
	if (uLength > 16 * 4) {
		FFT(pReal, pImaginary, pUnityTable + (uLength >> 1),
		    uLength >> 2, uCount * 4);
	...
	do {
		...
		for (UINT32 i = 0; i < 4; ++i) {
			for (UINT32 j = 0; j < uLength; ++j) {
				...
				pfUnityTable[uIndex] =
					cosf(FLOAT32(i) * FLOAT32(j) *
					     flStep); // real component
				pfUnityTable[uIndex + uLength * 4] =
					-sinf(FLOAT32(i) * FLOAT32(j) *
					      flStep); // imaginary component
			}
		}
		     ...
		     const UINT32 uLength)
{
	...
	for (UINT32 uIndex = 0; uIndex < (uLength >> 2); ++uIndex) {
		...
		XVECTOR vImaginary =
			_mm_mul_ps(pInputImaginary[uIndex], vOneOverLength);
			   ...
			   const UINT32 uChannelCount, const UINT32 uLog2Length)
{
	...
	if (uLength > 16) {
		for (UINT32 uChannel = 0; uChannel < uChannelCount;
		     ++uChannel) {
		  ...
		  const UINT32 uChannelCount, const UINT32 uLog2Length)
{
	...
	if (uChannelCount > 1) {
		...
		CopyMemory(pReal, vImaginaryTemp,
			   (uLength >> 2) * sizeof(XVECTOR));
	}
}
