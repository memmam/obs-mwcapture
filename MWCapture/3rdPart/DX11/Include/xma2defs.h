	...
	for (nBlock = 0; nBlock < nBlockCount; ++nBlock) {
		...
		if (nTotalSamplesSoFar > nDesiredSample) {
			...
			*pnSampleOffsetWithinBlock =
				nDesiredSample - nPreviousTotalSamples;
			return S_OK;
		}
...
)
{
	...
	for (;;) {
		...
		if (pCurrentPacket + XMA_BYTES_PER_PACKET >
		...
		if (nFrameCountSoFar + GetXmaPacketFrameCount(pCurrentPacket) >
			...
			while (nFramesToSkip--) {
				nFrameBitOffset += GetXmaFrameLengthInBits(
					pCurrentPacket, nFrameBitOffset);
			}
			...
			return (DWORD)(pCurrentPacket - pXmaData) * 8 +
			       nFrameBitOffset;
		}
		...
		pCurrentPacket += XMA_BYTES_PER_PACKET *
				  (GetXmaPacketSkipCount(pCurrentPacket) + 1);
	}
}
...
)
{
	...
	XMA2DEFS_ASSERT(pXmaData);
	XMA2DEFS_ASSERT(nXmaDataBytes % XMA_BYTES_PER_PACKET == 0);
	XMA2DEFS_ASSERT(nXmaDataBytes >=
			XMA_BYTES_PER_PACKET * (nStreamIndex + 1));
	...
	for (;;) {
		nBytesToNextPacket = XMA_BYTES_PER_PACKET *
				     (GetXmaPacketSkipCount(pLastPacket) + 1);
	...
	nFrameBitOffset = XMA_PACKET_HEADER_BITS +
			  GetXmaPacketFirstFrameOffsetInBits(pLastPacket);
	...
	while (--nFramesInLastPacket) {
		nFrameBitOffset +=
			GetXmaFrameLengthInBits(pLastPacket, nFrameBitOffset);
	}
...
)
{
	...
	DWORD nSubFrame = (nDesiredSample % XMA_SAMPLES_PER_FRAME) /
...
#define XMASWAP4BYTES(n)                                     \
	((DWORD)((n) >> 24 | (n) << 24 | ((n)&0xff00) << 8 | \
		 ((n)&0xff0000) >> 8))

	if (pXma2Format->wfx.wFormatTag == WAVE_FORMAT_XMA2) {
		return S_OK;
	} else if (XMASWAP2BYTES(pXma2Format->wfx.wFormatTag) ==
		   WAVE_FORMAT_XMA2) {
		pXma2Format->wfx.wFormatTag =
			XMASWAP2BYTES(pXma2Format->wfx.wFormatTag);
		pXma2Format->wfx.nChannels =
