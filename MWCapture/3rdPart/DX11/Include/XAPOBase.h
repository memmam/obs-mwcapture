...
#define XAPOBASE_DEFAULT_FORMAT_TAG \
	WAVE_FORMAT_IEEE_FLOAT // 32-bit float only, applies to WAVEFORMATEX.wFormatTag or WAVEFORMATEXTENSIBLE.SubFormat when used
...
#define XAPOBASE_DEFAULT_FORMAT_MIN_FRAMERATE \
	XAPO_MIN_FRAMERATE // minimum framerate, applies to WAVEFORMATEX.nSamplesPerSec
#define XAPOBASE_DEFAULT_FORMAT_MAX_FRAMERATE \
	XAPO_MAX_FRAMERATE // maximum framerate, applies to WAVEFORMATEX.nSamplesPerSec
#define XAPOBASE_DEFAULT_FORMAT_BITSPERSAMPLE \
	32 // 32-bit float only, applies to WAVEFORMATEX.wBitsPerSample and WAVEFORMATEXTENSIBLE.wValidBitsPerSample when used
...
#define XAPOASSERT(exp)                                       \
	if (!(exp)) {                                         \
		OutputDebugStringA("XAPO ASSERT: " #exp       \
				   ", {" __FUNCTION__ "}\n"); \
		__debugbreak();                               \
	}
#else
...
#pragma pack( \
	push, \
	8) // set packing alignment to ensure consistency across arbitrary build environments, and ensure synchronization variables used by Interlocked functionality are correctly aligned
...
private:
	const XAPO_REGISTRATION_PROPERTIES *
		m_pRegistrationProperties; // pointer to registration properties of the XAPO, set via constructor
...
protected:
	...
	//  void
	////
	void ProcessThru(__in void *pInputBuffer,
			 __inout FLOAT32 *pOutputBuffer, UINT32 FrameCount,
			 WORD InputChannelCount, WORD OutputChannelCount,
			 BOOL MixWithOutput);
...
public:
	...
	(REFIID riid, __deref_out_opt void **ppInterface)
	{
	...
	{
		ULONG uTmpReferenceCount =
			(ULONG)InterlockedDecrement(&m_lReferenceCount);
...
public:
	...
	CXAPOParametersBase(
		...
		BYTE *pParameterBlocks, UINT32 uParameterBlockByteSize,
		BOOL fProducer);
	...
	(__out_bcount(ParameterByteSize) void *pParameters,
	 UINT32 ParameterByteSize);
