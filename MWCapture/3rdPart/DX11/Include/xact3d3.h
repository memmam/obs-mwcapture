...
#pragma warning(push)
#pragma warning( \
	disable : 4701) // disable "local variable may be used without having been initialized" compile warning
					 ...
					 __in X3DAUDIO_HANDLE X3DInstance)
{
	...
	if (SUCCEEDED(hr)) {
		...
		hr = pEngine->GetGlobalVariable(xactSpeedOfSoundID,
						&nSpeedOfSound);
	}
	...
	if (SUCCEEDED(hr)) {
		...
		if (SUCCEEDED(hr)) {
			X3DAudioInitialize(wfxFinalMixFormat.dwChannelMask,
					   nSpeedOfSound, X3DInstance);
		}
	}
	return hr;
}
	...
	__inout X3DAUDIO_DSP_SETTINGS *pDSPSettings)
{
		...
		if (pEmitter->ChannelCount > 1 &&
			...
			case 2:
				pEmitter->pChannelAzimuths =
					(float *)&aStereoLayout[0];
				break;
			case 3:
				pEmitter->pChannelAzimuths =
					(float *)&a2Point1Layout[0];
				break;
			case 4:
				pEmitter->pChannelAzimuths =
					(float *)&aQuadLayout[0];
			...
			case 5:
				pEmitter->pChannelAzimuths =
					(float *)&a4Point1Layout[0];
				break;
			case 6:
				pEmitter->pChannelAzimuths =
					(float *)&a5Point1Layout[0];
				break;
			case 8:
				pEmitter->pChannelAzimuths =
					(float *)&a7Point1Layout[0];
				break;
			default:
				hr = E_FAIL;
				break;
	...
	if (SUCCEEDED(hr)) {
		static X3DAUDIO_DISTANCE_CURVE_POINT DefaultCurvePoints[2] = {
			0.0f, 1.0f, 1.0f, 1.0f};
		static X3DAUDIO_DISTANCE_CURVE DefaultCurve = {
			(X3DAUDIO_DISTANCE_CURVE_POINT *)&DefaultCurvePoints[0],
			2};
...
//  HResult error code
////
EXTERN_C HRESULT inline XACT3DApply(
	__in const X3DAUDIO_DSP_SETTINGS *pDSPSettings, __in IXACT3Cue *pCue)
{
	HRESULT hr = S_OK;
	...
	if (SUCCEEDED(hr)) {
		hr = pCue->SetMatrixCoefficients(
			pDSPSettings->SrcChannelCount,
			pDSPSettings->DstChannelCount,
			pDSPSettings->pMatrixCoefficients);
	}
	if (SUCCEEDED(hr)) {
		XACTVARIABLEINDEX xactDistanceID =
			pCue->GetVariableIndex("Distance");
	...
	if (SUCCEEDED(hr)) {
		...
		hr = pCue->SetVariable(xactDopplerID,
