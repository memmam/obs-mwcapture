...
typedef union WAVEBANKMINIWAVEFORMAT {
	...
	WORD BitsPerSample() const
	{
		...
		return (wBitsPerSample == WAVEBANKMINIFORMAT_BITDEPTH_16) ? 16
									  : 8;
	}
...
#define ADPCM_MINIWAVEFORMAT_BLOCKALIGN_CONVERSION_OFFSET 22
	...
	DWORD AvgBytesPerSec() const
	{
		...
		case WAVEBANKMINIFORMAT_TAG_ADPCM: {
			...
			dwReturn = blockAlign * nSamplesPerSec /
				   samplesPerAdpcmBlock;
		} break;

		case WAVEBANKMINIFORMAT_TAG_WMA: {
			DWORD dwBytesPerSecIndex = wBlockAlign >> 5;
			if (dwBytesPerSecIndex <
			    MAX_WMA_AVG_BYTES_PER_SEC_ENTRIES)
				dwReturn =
					aWMAAvgBytesPerSec[dwBytesPerSecIndex];
		} break;
		}
				  ...
				  DWORD dwAvgBytesPerSec) const
	{
		...
		if (dwBlockAlignIndex < MAX_WMA_BLOCK_ALIGN_ENTRIES) {
			...
			if (dwBytesPerSecIndex <
			    MAX_WMA_AVG_BYTES_PER_SEC_ENTRIES) {
				dwReturn = dwBlockAlignIndex |
					   (dwBytesPerSecIndex << 5);
			}
		}
	...
	void AdpcmFillCoefficientTable(ADPCMWAVEFORMAT *fmt) const
	{
		...
		static ADPCMCOEFSET aCoef[7] = {{256, 0},   {512, -256},
						{0, 0},     {192, 64},
						{240, 0},   {460, -208},
						{392, -232}};
