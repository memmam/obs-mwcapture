...
#define IM_OFFSETOF(_TYPE, _MEMBER) \
	((size_t) &                 \
	 (((_TYPE *)0)              \
		  ->_MEMBER)) // Offset of _MEMBER within _TYPE. Standardized as offsetof() in modern C++.
...
typedef int
	ImGuiDataType; // enum: a primary data type                // enum ImGuiDataType_
typedef int
	ImGuiDir; // enum: a cardinal direction               // enum ImGuiDir_
typedef int
	ImGuiCond; // enum: a condition for Set*()             // enum ImGuiCond_
typedef int
	ImGuiKey; // enum: a key identifier (ImGui-side enum) // enum ImGuiKey_
typedef int
	ImGuiNavInput; // enum: an input identifier for navigation // enum ImGuiNavInput_
typedef int
	ImGuiMouseCursor; // enum: a mouse cursor identifier          // enum ImGuiMouseCursor_
typedef int
	ImGuiStyleVar; // enum: a variable identifier for styling  // enum ImGuiStyleVar_
typedef int
	ImDrawCornerFlags; // flags: for ImDrawList::AddRect*() etc.   // enum ImDrawCornerFlags_
typedef int
	ImDrawListFlags; // flags: for ImDrawList                    // enum ImDrawListFlags_
typedef int
	ImFontAtlasFlags; // flags: for ImFontAtlas                   // enum ImFontAtlasFlags_
typedef int
...
typedef int
	ImGuiConfigFlags; // flags: for io.ConfigFlags                // enum ImGuiConfigFlags_
typedef int
	ImGuiComboFlags; // flags: for BeginCombo()                  // enum ImGuiComboFlags_
typedef int
...
typedef int
	ImGuiWindowFlags; // flags: for Begin*()                      // enum ImGuiWindowFlags_
...
IMGUI_API void ShowDemoWindow(
	bool *p_open =
		NULL); // create demo/test window (previously called ShowTestWindow). demonstrate most ImGui features. call this to learn about the library! try to make it always available in your application!
IMGUI_API void ShowMetricsWindow(
	bool *p_open =
		NULL); // create metrics window. display ImGui internals: draw commands (with individual draw calls and vertices), window list, basic internal state, etc.
...
IMGUI_API void SetNextWindowContentSize(
	const ImVec2 &
		size); // set next window content size (~ enforce the range of scrollbars). not including window decorations (title bar, menu bar, etc.). set an axis to 0.0f to leave it automatic. call before Begin()
...
IMGUI_API void SetCursorScreenPos(
	const ImVec2 &
		screen_pos); // cursor position in absolute screen coordinates [0..io.DisplaySize]
IMGUI_API void
...
IMGUI_API bool InvisibleButton(
	const char *str_id,
	const ImVec2 &
		size); // button behavior without the visuals, useful to build custom behaviors using the public api (along with IsItemActive, IsItemHovered, etc.)
...
IMGUI_API void TreePush(
	const char *
		str_id); // ~ Indent()+PushId(). Already called by TreeNode() when returning true, but you can call Push/Pop yourself for layout purpose
...
IMGUI_API void CaptureKeyboardFromApp(
	bool capture =
		true); // manually override io.WantCaptureKeyboard flag next frame (said flag is entirely left for your application to handle). e.g. force capture keyboard when your widget is being hovered.
IMGUI_API void CaptureMouseFromApp(
	bool capture =
		true); // manually override io.WantCaptureMouse flag next frame (said flag is entirely left for your application to handle).
...
enum ImGuiInputTextFlags_ {
	...
	ImGuiInputTextFlags_CallbackCompletion =
		1
		<< 6, // Call user function on pressing TAB (for completion handling)
...
enum ImGuiConfigFlags_ {
	...
	ImGuiConfigFlags_NavNoCaptureKeyboard =
		1
		<< 3, // Instruct navigation to not set the io.WantCaptureKeyboard flag with io.NavActive is set.
...
enum ImGuiBackendFlags_ {
	ImGuiBackendFlags_HasGamepad =
		1 << 0, // Back-end supports and has a connected gamepad.
	ImGuiBackendFlags_HasMouseCursors =
		1
		<< 1, // Back-end supports reading GetMouseCursor() to change the OS cursor shape.
...
enum ImGuiColorEditFlags_ {
	...
	ImGuiColorEditFlags_PickerHueBar =
		1
		<< 25, // [PickerMode] // ColorPicker: bar for Hue, rectangle for Sat/Value.
...
struct ImGuiIO {
	...
	ImGuiBackendFlags
		BackendFlags; // = 0                  // Set ImGuiBackendFlags_ enum. Set by imgui_impl_xxx files or custom back-end to communicate features supported by the back-end.
	...
	float NavInputs
		[ImGuiNavInput_COUNT]; // Gamepad inputs (keyboard keys will be auto-mapped and be written here by ImGui::NewFrame, all values will be cleared back to zero in ImGui::EndFrame)
...
{
	ImGuiIO &io = GetIO();
	SetNextWindowPos(ImVec2(io.DisplaySize.x * 0.5f,
...
struct ImDrawList {
	...
	IMGUI_API void
	AddRect(const ImVec2 &a, const ImVec2 &b, ImU32 col,
		float rounding = 0.0f,
	...
	IMGUI_API void AddImageQuad(ImTextureID user_texture_id,
				    ...
				    ImU32 col = 0xFFFFFFFF);
	IMGUI_API void
	AddImageRounded(ImTextureID user_texture_id, const ImVec2 &a,
...
struct ImFontAtlas {
	...
	IMGUI_API ImFont *AddFontFromMemoryTTF(
		...
		const ImWchar *glyph_ranges =
			NULL); // Note: Transfer ownership of 'ttf_data' to ImFontAtlas! Will be deleted after Build(). Set font_cfg->FontDataOwnedByAtlas to false to keep ownership.
	...
	IMGUI_API int AddCustomRectFontGlyph(
		ImFont *font, ImWchar id, int width, int height,
		float advance_x,
...
struct ImFont {
	...
	ImVector<float>
		IndexAdvanceX; //              // Sparse. Glyphs->AdvanceX in a directly indexable way (more cache-friendly, for CalcTextSize functions which are often bottleneck in large UI).
