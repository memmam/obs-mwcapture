...
static bool DragBehaviorT(ImGuiDataType data_type, TYPE *v, float v_speed,
			  const TYPE v_min, const TYPE v_max,
			  const char *format, float power);

template<typename TYPE, typename SIGNEDTYPE, typename FLOATTYPE>
static bool SliderBehaviorT(const ImRect &bb, ImGuiID id,
...
ImGuiStyle::ImGuiStyle()
{
	...
	ScrollbarSize =
		16.0f; // Width of the vertical scrollbar, Height of the horizontal scrollbar
	ScrollbarRounding =
...
bool ImTriangleContainsPoint(const ImVec2 &a, const ImVec2 &b, const ImVec2 &c,
			     const ImVec2 &p)
{
	bool b1 = ((p.x - b.x) * (a.y - b.y) - (p.y - b.y) * (a.x - b.x)) <
		  0.0f;
	bool b2 = ((p.x - c.x) * (b.y - c.y) - (p.y - c.y) * (b.x - c.x)) <
		  0.0f;
	bool b3 = ((p.x - a.x) * (c.y - a.y) - (p.y - a.y) * (c.x - a.x)) <
		  0.0f;
	return ((b1 == b2) && (b2 == b3));
}
...
ImU32 ImGui::GetColorU32(ImU32 col)
{
	...
	a = (ImU32)(a *
		    style_alpha); // We don't need to clamp 0..255 because Style.Alpha is in 0..1 range.
...
static void SetNavIDWithRectRel(ImGuiID id, int nav_layer,
...
void ImGui::ItemSize(const ImVec2 &size, float text_offset_y)
{
	...
	window->DC.CurrentLineHeight = window->DC.CurrentLineTextBaseOffset =
		0.0f;
...
static bool NavScoreItem(ImGuiNavMoveResult *result, ImRect cand)
{
	...
	const ImRect &curr =
		g.NavScoringRectScreen; // Current modified source rect (NB: we've applied Max.x = Min.x in NavUpdate() to inhibit the effect of having varied item width)
	g.NavScoringCount++;
...
static void NavRestoreLayer(int layer)
{
	...
	if (layer == 0 && g.NavWindow->NavLastIds[0] != 0)
		SetNavIDWithRectRel(g.NavWindow->NavLastIds[0], layer,
				  ...
				  const ImGuiID id)
{
	...
	if (g.NavId != id && !(item_flags & ImGuiItemFlags_NoNav)) {
		...
		bool new_best = NavScoreItem(result, nav_bb) &&
				g.NavMoveRequest;
#else
		bool new_best = g.NavMoveRequest &&
				  ...
				  bool tab_stop)
{
	...
	const bool allow_keyboard_focus =
		(window->DC.ItemFlags & (ImGuiItemFlags_AllowKeyboardFocus |
	...
	if (size.x <= 0.0f)
		size.x =
			(size.x == 0.0f)
				? default_x
				: ImMax(content_max.x -
						g.CurrentWindow->DC.CursorPos.x,
					4.0f) +
					  size.x;
	if (size.y <= 0.0f)
		size.y =
			(size.y == 0.0f)
				? default_y
				: ImMax(content_max.y -
						g.CurrentWindow->DC.CursorPos.y,
					4.0f) +
					  size.y;
	return size;
}
...
static ImVec2 NavCalcPreferredRefPos()
{
	...
	return ImFloor(ImClamp(
		...
		visible_rect
			.Max)); // ImFloor() is important because non-integer mouse position application in back-end might be lossy and result in undesirable non-zero delta.
}
...
static void ImGui::NavUpdateWindowing()
{
	...
	if (start_windowing_with_gamepad || start_windowing_with_keyboard)
		if (ImGuiWindow *window =
			    g.NavWindow
				    ? g.NavWindow
				    : FindWindowNavigable(g.Windows.Size - 1,
							  -INT_MAX, -1)) {
			g.NavWindowingTarget = window->RootWindowForTabbing;
			g.NavWindowingHighlightTimer =
	...
	if (g.NavWindowingTarget &&
		...
		g.NavWindowingHighlightAlpha = ImMax(
			...
			ImSaturate((g.NavWindowingHighlightTimer - 0.20f) /
				   0.05f));
	...
	if (g.NavWindowingTarget &&
		...
		g.NavWindowingHighlightAlpha = ImMax(
			g.NavWindowingHighlightAlpha,
			ImSaturate((g.NavWindowingHighlightTimer - 0.15f) /
				   0.04f)); // 1.0f
	...
	if (apply_toggle_layer && g.NavWindow) {
		...
		while ((new_nav_window->DC.NavLayerActiveMask & (1 << 1)) ==
			       0 &&
...
#undef NAV_MAP_KEY
	...
	if (g.NavMoveRequest &&
		...
		if (g.NavMoveResultOther.ID != 0 &&
		    g.NavMoveResultOther.Window->ParentWindow ==
			    g.NavWindow) // Maybe entering a flattened child? In this case solve the tie using the regular scoring rules
			if ((g.NavMoveResultOther.DistBox <
		...
		if (g.NavLayer == 0) {
			...
			if (result->Window->Flags &
			    ImGuiWindowFlags_ChildWindow)
				NavScrollToBringItemIntoView(
					result->Window->ParentWindow, rect_abs);

			// Estimate upcoming scroll so we can offset our result position so mouse position can be applied immediately after in NavUpdate()
			ImVec2 next_scroll =
				CalcNextScrollFromScrollTargetAndClamp(
					result->Window, false);
			result->RectRel.Translate(result->Window->Scroll -
						  next_scroll);
		}
	...
	if (g.NavMousePosDirty && g.NavIdIsAlive) {
		...
		if ((g.IO.ConfigFlags &
			...
			IM_ASSERT(!g.NavDisableHighlight &&
	...
	g.NavScoringRectScreen.Min.x =
		ImMin(g.NavScoringRectScreen.Min.x + 1.0f,
	...
	if (g.NavWindow) {
		...
		g.OverlayDrawList.AddText(NULL, 13.0f, p + ImVec2(8, -4), col,
					  buf);
	}
#endif
}
...
static void ImGui::UpdateMouseInputs()
{
	...
	for (int i = 0; i < IM_ARRAYSIZE(g.IO.MouseDown); i++) {
		...
		if (g.IO.MouseClicked[i]) {
			if (g.Time - g.IO.MouseClickedTime[i] <
...
void ImGui::NewFrameUpdateHoveredWindowAndCaptureFlags()
{
	...
	g.HoveredWindow = (g.MovingWindow &&
			   !(g.MovingWindow->Flags & ImGuiWindowFlags_NoInputs))
				  ? g.MovingWindow
				  : FindHoveredWindow();
	g.HoveredRootWindow = g.HoveredWindow ? g.HoveredWindow->RootWindow
					      : NULL;
...
void ImGui::NewFrame()
{
	...
	if (GetFrontMostPopupModal() != NULL)
		g.ModalWindowDarkeningRatio = ImMin(
			g.ModalWindowDarkeningRatio + g.IO.DeltaTime * 6.0f,
			1.0f);
	else
		...
		if (g.IO.MouseWheel != 0.0f) {
			if (g.IO.KeyCtrl && g.IO.FontAllowUserScaling) {
				...
				const float new_font_scale =
					ImClamp(window->FontWindowScale +
							g.IO.MouseWheel * 0.10f,
						0.50f, 2.50f);
				const float scale = new_font_scale /
						    window->FontWindowScale;
				window->FontWindowScale = new_font_scale;

				const ImVec2 offset =
					window->Size * (1.0f - scale) *
				...
				float scroll_amount =
					5 * scroll_window->CalcFontSize();
				scroll_amount = (float)(int)ImMin(
					scroll_amount,
					(scroll_window->ContentsRegionRect
						 .GetHeight() +
					 scroll_window->WindowPadding.y *
						 2.0f) *
						0.67f);
	...
	if (g.ActiveId == 0 && g.NavWindow != NULL && g.NavWindow->Active &&
		...
		if (g.NavId != 0 && g.NavIdTabCounter != INT_MAX)
			...
			g.NavWindow->FocusIdxTabRequestNext = g.IO.KeyShift ? -1
									    : 0;
	}
					   ...
					   const char *line)
{
	...
	if (sscanf(line, "Pos=%f,%f", &x, &y) == 2)
		...
		settings->Size =
			ImMax(ImVec2(x, y), GImGui->Style.WindowMinSize);
				    ...
				    ImGuiWindow *window)
{
	...
	if (window->Active) {
		...
		for (int i = 0; i < count; i++) {
			...
			if (child->Active)
				AddWindowToSortedBuffer(out_sorted_windows,
							child);
		}
	}
}
			  ...
			  ImDrawData *out_draw_data)
{
	...
	out_draw_data->CmdLists = (draw_lists->Size > 0) ? draw_lists->Data
							 : NULL;
	...
	for (int n = 0; n < draw_lists->Size; n++) {
		out_draw_data->TotalVtxCount +=
			draw_lists->Data[n]->VtxBuffer.Size;
		out_draw_data->TotalIdxCount +=
			draw_lists->Data[n]->IdxBuffer.Size;
	}
}
			     ...
			     ->Appearing) // Unless we just made a window/popup appear
		{
			...
			if (g.IO.MouseClicked[0]) {
				if (g.HoveredRootWindow != NULL) {
					...
					SetActiveID(g.HoveredWindow->MoveId,
						    g.HoveredWindow);
					g.NavDisableHighlight = true;
					g.ActiveIdClickOffset =
						g.IO.MousePos -
						g.HoveredRootWindow->Pos;
					if (!(g.HoveredWindow->Flags &
			...
			if (g.IO.MouseClicked[1]) {
				...
				for (int i = g.Windows.Size - 1;
					...
					if (window == g.HoveredWindow)
						hovered_window_above_modal =
							true;
				}
				ClosePopupsOverWindow(hovered_window_above_modal
							      ? g.HoveredWindow
							      : modal);
			}
		}
...
void ImGui::Render()
{
	...
	if (window_to_render_front_most &&
	    window_to_render_front_most->Active &&
	...
	if (g.IO.MouseDrawCursor &&
	    g.IO.Fonts->GetMouseCursorTexData(g.MouseCursor, &offset, &size,
					      &uv[0], &uv[2])) {
		...
		g.OverlayDrawList.AddImage(
			tex_id, pos, pos + size * sc, uv[2], uv[3],
			IM_COL32(0, 0, 0, 255)); // Black border
		g.OverlayDrawList.AddImage(
			tex_id, pos, pos + size * sc, uv[0], uv[1],
			    ...
			    const char *text_end = NULL)
{
	...
	const bool log_new_line = ref_pos &&
				  (ref_pos->y > window->DC.LogLinePosY + 1);
	if (ref_pos)
		...
		if (line_end != NULL &&
			...
			if (log_new_line || !is_first_line)
				ImGui::LogText(IM_NEWLINE "%*s%.*s",
					       tree_depth * 4, "", char_count,
					       text_remaining);
			else
				ImGui::LogText(" %.*s", char_count,
					       text_remaining);
		}
...
static ImGuiWindow *FindHoveredWindow()
{
	...
	for (int i = g.Windows.Size - 1; i >= 0; i--) {
		...
		ImRect bb(window->OuterRectClipped.Min -
...
static bool IsKeyPressedMap(ImGuiKey key, bool repeat)
{
	...
	return (key_index >= 0) ? ImGui::IsKeyPressed(key_index, repeat)
				: false;
}
...
ImVec2 ImGui::GetMousePosOnOpeningCurrentPopup()
{
	...
	if (g.CurrentPopupStack.Size > 0)
		return g.OpenPopupStack[g.CurrentPopupStack.Size - 1]
			.OpenMousePos;
...
ImVec2 ImGui::GetMouseDragDelta(int button, float lock_threshold)
{
	...
	if (g.IO.MouseDown[button])
		if (g.IO.MouseDragMaxDistanceSqr[button] >=
		    lock_threshold * lock_threshold)
			return g.IO.MousePos -
			       g.IO.MouseClickedPos
				       [button]; // Assume we can only get active with left-mouse button (at the moment).
...
void ImGui::ClosePopupsOverWindow(ImGuiWindow *ref_window)
{
	...
	if (ref_window) {
		for (n = 0; n < g.OpenPopupStack.Size; n++) {
			...
			for (int m = n; m < g.OpenPopupStack.Size && !has_focus;
			     m++)
				has_focus =
...
static ImVec2 FindBestWindowPosForPopupEx(
	...
	ImGuiPopupPositionPolicy policy = ImGuiPopupPositionPolicy_Default)
{
	...
	if (policy == ImGuiPopupPositionPolicy_ComboBox) {
		const ImGuiDir dir_prefered_order[ImGuiDir_COUNT] = {
			ImGuiDir_Down, ImGuiDir_Right, ImGuiDir_Left,
			ImGuiDir_Up};
		for (int n = (*last_dir != ImGuiDir_None) ? -1 : 0;
		     n < ImGuiDir_COUNT; n++) {
			const ImGuiDir dir = (n == -1) ? *last_dir
...
static ImVec2 CalcSizeAutoFit(ImGuiWindow *window, const ImVec2 &size_contents)
{
	...
	if (window->Flags & ImGuiWindowFlags_Tooltip) {
		...
		ImVec2 size_auto_fit = ImClamp(
			size_contents, style.WindowMinSize,
			ImMax(style.WindowMinSize,
			      g.IO.DisplaySize -
				      g.Style.DisplaySafeAreaPadding * 2.0f));
...
{
	return ImMax(0.0f, window->SizeContents.x - (window->SizeFull.x -
						     window->ScrollbarSizes.x));
}

static float GetScrollMaxY(ImGuiWindow *window)
{
	return ImMax(0.0f, window->SizeContents.y - (window->SizeFull.y -
						     ...
						     bool snap_on_edges)
{
	...
	if (window->ScrollTarget.y < FLT_MAX) {
		...
		scroll.y =
			target_y -
			(1.0f - cr_y) * (window->TitleBarHeight() +
				      ...
				      ImU32 resize_grip_col[4])
{
	...
	for (int resize_grip_n = 0; resize_grip_n < resize_grip_count;
		...
		ImRect resize_rect(corner,
				   corner + grip.InnerDir * grip_hover_size);
	...
	for (int border_n = 0; border_n < resize_border_count; border_n++) {
		...
		if (held) {
			...
			if (border_n == 0) {
				...
				border_target.y = (g.IO.MousePos.y -
			...
			if (border_n == 3) {
				border_posn = ImVec2(0, 0);
				border_target.x = (g.IO.MousePos.x -
...
bool ImGui::Begin(const char *name, bool *p_open, ImGuiWindowFlags flags)
{
	...
	if (flags & ImGuiWindowFlags_Popup) {
		ImGuiPopupRef &popup_ref =
			g.OpenPopupStack[g.CurrentPopupStack.Size];
	...
	if (first_begin_of_the_frame) {
		...
		if (!(flags & ImGuiWindowFlags_ChildWindow)) {
				    ...
				    0.0f) // Ignore zero-sized display explicitly to avoid losing positions if a window manager reports zero-sized window when initializing or minimizing.
			{
				...
				window->Pos = ImMax(window->Pos + window->Size,
						    padding) -
					      window->Size;
				window->Pos = ImMin(window->Pos,
		...
		const int resize_grip_count =
			(flags & ImGuiWindowFlags_ResizeFromAnySide) ? 2
								     : 1; // 4
		const float grip_draw_size = (float)(int)ImMax(
			g.FontSize * 1.35f,
			window->WindowRounding + 1.0f + g.FontSize * 0.2f);
		...
		if ((flags & ImGuiWindowFlags_ChildWindow) &&
			...
			PushClipRect(viewport_rect.Min, viewport_rect.Max,
				     true);
		...
		if ((flags & ImGuiWindowFlags_Modal) != 0 &&
			...
			window->DrawList->AddRectFilled(
				viewport_rect.Min, viewport_rect.Max,
		...
		if (window->Collapsed) {
			...
			if (window_border_size > 0.0f)
				window->DrawList->AddRect(
					...
					GetColorU32(ImGuiCol_Border),
					window_rounding, ImDrawCornerFlags_All,
					window_border_size);
			if (border_held != -1) {
				ImRect border =
					GetBorderRect(window, border_held,
						      grip_draw_size, 0.0f);
				window->DrawList->AddLine(
		...
		window->ContentsRegionRect.Min.x = window->Pos.x -
						   window->Scroll.x +
		...
		if (!(flags & ImGuiWindowFlags_NoTitleBar)) {
			...
			if (!(flags & ImGuiWindowFlags_NoCollapse)) {
				...
				ImRect bb(window->Pos + style.FramePadding +
						  ImVec2(1, 1),
			...
			RenderTextClipped(text_r.Min, text_r.Max, name, NULL,
		  ...
		  float bg_alpha_override, ImGuiWindowFlags flags)
{
	...
	if (size_first_use.x != 0.0f || size_first_use.y != 0.0f)
		ImGui::SetNextWindowSize(size_first_use,
...
void ImGui::Scrollbar(ImGuiLayoutType direction)
{
	...
	float other_scrollbar_size_w = other_scrollbar ? style.ScrollbarSize
						       : 0.0f;
	const ImRect window_rect = window->Rect();
	const float border_size = window->WindowBorderSize;
	ImRect bb =
		horizontal ? ImRect(window->Pos.x + border_size,
	...
	if (held && grab_h_norm < 1.0f) {
		...
		float mouse_pos_v = horizontal ? g.IO.MousePos.x
					       : g.IO.MousePos.y;
		...
		if (seek_absolute)
			*click_delta_to_grab_center_v = clicked_v_norm -
							grab_v_norm -
							grab_h_norm * 0.5f;
	}
	...
	if (horizontal)
		grab_rect = ImRect(
			...
			ImMin(ImLerp(bb.Min.x, bb.Max.x, grab_v_norm) +
				      grab_h_pixels,
			      window_rect.Max.x),
			bb.Max.y);
	else
		grab_rect = ImRect(
			bb.Min.x, ImLerp(bb.Min.y, bb.Max.y, grab_v_norm),
			bb.Max.x,
			ImMin(ImLerp(bb.Min.y, bb.Max.y, grab_v_norm) +
				      grab_h_pixels,
			      window_rect.Max.y));
...
void ImGui::SetCurrentFont(ImFont *font)
{
	...
	g.Font = font;
	g.FontBaseSize =
		g.IO.FontGlobalScale * g.Font->FontSize * g.Font->Scale;
...
void ImGui::PushStyleVar(ImGuiStyleVar idx, float val)
{
	...
	IM_ASSERT(
		0); // Called function with wrong-type? Variable is not a float.
}
...
static void SetWindowScrollX(ImGuiWindow *window, float new_scroll_x)
{
	window->DC.CursorMaxPos.x +=
		window->Scroll
			.x; // SizeContents is generally computed based on CursorMaxPos which is affected by scroll position, so we need to apply our change to it.
...
static void SetWindowScrollY(ImGuiWindow *window, float new_scroll_y)
{
	window->DC.CursorMaxPos.y +=
		window->Scroll
			.y; // SizeContents is generally computed based on CursorMaxPos which is affected by scroll position, so we need to apply our change to it.
...
void ImGui::SetKeyboardFocusHere(int offset)
{
	...
	ImGuiWindow *window = GetCurrentWindow();
	window->FocusIdxAllRequestNext =
		window->FocusIdxAllCounter + 1 + offset;
	window->FocusIdxTabRequestNext = INT_MAX;
}
...
void ImGui::TextUnformatted(const char *text, const char *text_end)
{
	...
	if (text_end - text > 2000 && !wrap_enabled) {
		...
		if (text_pos.y <= clip_rect.Max.y) {
			...
			if (!g.LogEnabled) {
				int lines_skippable =
					(int)((clip_rect.Min.y - text_pos.y) /
					      line_height);
			...
			if (line < text_end) {
				...
				while (line < text_end) {
					...
					const ImVec2 line_size = CalcTextSize(
						line, line_end, false);
					text_size.x =
						ImMax(text_size.x, line_size.x);
...
void ImGui::AlignTextToFramePadding()
{
	...
	window->DC.CurrentLineHeight =
		ImMax(window->DC.CurrentLineHeight,
...
bool ImGui::ArrowButton(const char *str_id, ImGuiDir dir)
{
	...
	const ImRect bb(window->DC.CursorPos,
			window->DC.CursorPos + ImVec2(sz, sz));
	ItemSize(bb);
	if (!ItemAdd(bb, id))
		return false;
...
bool ImGui::TreeNodeBehaviorIsOpen(ImGuiID id, ImGuiTreeNodeFlags flags)
{
	...
	if (g.NextTreeNodeOpenCond != 0) {
		...
		is_open = storage->GetInt(id, (flags &
			     ...
			     const char *label, const char *label_end)
{
	...
	bool hovered, held,
		pressed = ButtonBehavior(interact_bb, id, &hovered, &held,
					 button_flags);
	if (!(flags & ImGuiTreeNodeFlags_Leaf)) {
		bool toggled = false;
		if (pressed) {
	...
	if (display_frame) {
		...
		RenderArrow(frame_bb.Min +
				    ImVec2(padding.x, text_base_offset_y),
		...
		if (g.LogEnabled) {
			...
			LogRenderedText(&text_pos, log_suffix + 1,
					log_suffix + 3);
		} else {
			RenderTextClipped(text_pos, frame_bb.Max, label,
					  label_end, &label_size);
				       ...
				       const void *data_ptr, const char *format)
{
	if (data_type == ImGuiDataType_S32 ||
	    data_type ==
		    ImGuiDataType_U32) // Signedness doesn't matter when pushing the argument
		return ImFormatString(buf, buf_size, format,
				      *(const ImU32 *)data_ptr);
	if (data_type == ImGuiDataType_S64 ||
	    data_type ==
		    ImGuiDataType_U64) // Signedness doesn't matter when pushing the argument
		return ImFormatString(buf, buf_size, format,
			    ...
			    void *arg1, const void *arg2)
{
	...
	case ImGuiDataType_U32:
		if (op == '+')
			*(unsigned int *)output = *(const unsigned int *)arg1 +
						  *(const ImU32 *)arg2;
		else if (op == '-')
			*(unsigned int *)output = *(const unsigned int *)arg1 -
						  *(const ImU32 *)arg2;
					   ...
					   void *data_ptr, const char *format)
{
	...
	if (value_changed)
		return DataTypeApplyOpFromText(
			data_buf, g.InputTextState.InitialText.begin(),
...
static inline TYPE RoundScalarWithFormat(const char *format,
...
static inline float
SliderBehaviorCalcRatioFromValue(ImGuiDataType data_type, TYPE v, TYPE v_min,
				 TYPE v_max, float power, float linear_zero_pos)
{
	if (v_min == v_max)
		return 0.0f;
...
template<typename TYPE, typename SIGNEDTYPE, typename FLOATTYPE>
static bool
ImGui::SliderBehaviorT(const ImRect &bb, ImGuiID id, ImGuiDataType data_type,
		       ...
		       const char *format, float power, ImGuiSliderFlags flags)
{
	...
	const float slider_sz =
		is_horizontal ? (bb.GetWidth() - grab_padding * 2.0f)
	...
	const float slider_usable_pos_min =
		(is_horizontal ? bb.Min.x : bb.Min.y) + grab_padding +
		grab_sz * 0.5f;
	const float slider_usable_pos_max =
		(is_horizontal ? bb.Max.x : bb.Max.y) - grab_padding -
		grab_sz * 0.5f;
	...
	if (g.ActiveId == id) {
		...
		if (g.ActiveIdSource == ImGuiInputSource_Mouse) {
			...
			if (g.NavActivatePressedId == id &&
				...
				clicked_t = SliderBehaviorCalcRatioFromValue<
					TYPE, FLOATTYPE>(data_type, *v, v_min,
							 v_max, power,
							 linear_zero_pos);
		...
		if (set_new_value) {
			...
			if (is_power) {
				...
				if (clicked_t < linear_zero_pos) {
					...
					float a = 1.0f -
						  (clicked_t / linear_zero_pos);
					a = ImPow(a, power);
					v_new = ImLerp(ImMin(v_max, (TYPE)0),
						       v_min, a);
				} else {
					// Positive: rescale to the positive range before powering
					float a;
					if (ImFabs(linear_zero_pos - 1.0f) >
					    1.e-6f)
						a = (clicked_t -
						     linear_zero_pos) /
						    (1.0f - linear_zero_pos);
						...
						a = clicked_t;
					a = ImPow(a, power);
					v_new = ImLerp(ImMax(v_min, (TYPE)0),
						       v_max, a);
				}
			} else {
				// Linear slider
				if (is_decimal) {
					...
					FLOATTYPE v_new_off_f =
						(v_max - v_min) * clicked_t;
					TYPE v_new_off_floor =
						(TYPE)(v_new_off_f);
			...
			v_new = RoundScalarWithFormat<TYPE, SIGNEDTYPE>(
			 ...
			 const char *format, float power)
{
	...
	if (tab_focus_requested || (hovered && g.IO.MouseClicked[0]) ||
		...
		SetFocusID(id, window);
		FocusWindow(window);
		g.ActiveIdAllowNavDirFlags = (1 << ImGuiDir_Up) |
					     (1 << ImGuiDir_Down);
	...
	if (start_text_input ||
		...
		return InputScalarAsWidgetReplacement(frame_bb, id, label,
						      data_type, v, format);

	// Actual slider behavior + render grab
	ItemSize(total_bb, style.FramePadding.y);
	const bool value_changed = SliderBehavior(frame_bb, id, data_type, v,
			  ...
			  const void *v_max, const char *format, float power)
{
	...
	// Actual slider behavior + render grab
	bool value_changed = SliderBehavior(frame_bb, id, data_type, v, v_min,
					    v_max, format, power,
			  ...
			  const char *format, float power)
{
	...
	for (int i = 0; i < components; i++) {
		PushID(i);
		value_changed |= SliderScalar("##v", data_type, v, v_min, v_max,
					      format, power);
...
template<typename TYPE, typename SIGNEDTYPE, typename FLOATTYPE>
static bool ImGui::DragBehaviorT(ImGuiDataType data_type, TYPE *v,
				 float v_speed, const TYPE v_min,
				 ...
				 float power)
{
	...
	if (is_power) {
		...
		v_cur = v_min +
			(TYPE)ImPow(ImSaturate((float)v_new_norm_curved),
				    power) *
				(v_max - v_min);
	...
	// Round to user desired precision based on format string
	v_cur = RoundScalarWithFormat<TYPE, SIGNEDTYPE>(format, data_type,
							v_cur);
	...
	if (is_power) {
		FLOATTYPE v_cur_norm_curved = ImPow(
			(FLOATTYPE)(v_cur - v_min) / (FLOATTYPE)(v_max - v_min),
			(FLOATTYPE)1.0f / power);
		g.DragCurrentAccum -= (float)(v_cur_norm_curved -
			 ...
			 const char *format, float power)
{
	...
	case ImGuiDataType_Double:
		return DragBehaviorT<double, double, double>(
			...
			v_max ? *(const double *)v_max : DBL_MAX, format,
			power);
	case ImGuiDataType_COUNT:
		break;
	}
		       ...
		       const char *format, float power)
{
	...
	ItemSize(total_bb, style.FramePadding.y);
	const bool value_changed = DragBehavior(id, data_type, v, v_speed,
						v_min, v_max, format, power);
			...
			const void *v_max, const char *format, float power)
{
	...
	for (int i = 0; i < components; i++) {
		PushID(i);
		value_changed |= DragScalar("##v", data_type, v, v_speed, v_min,
					    v_max, format, power);
		SameLine(0, g.Style.ItemInnerSpacing.x);
		PopID();
		PopItemWidth();
		   ...
		   ImVec2 graph_size)
{
	...
	if (values_count > 0) {
		...
		for (int n = 0; n < res_w; n++) {
			...
			const float v1 =
				values_getter(data, (v1_idx + values_offset +
						     1) % values_count);
			const ImVec2 tp1 =
				     ...
				     void *user_data)
{
	...
	if (c >= 0xE000 &&
	    c <= 0xF8FF) // Filter private Unicode range. I don't imagine anybody would want to input them. GLFW on OSX seems to send private characters for special keys like arrow keys.
		return false;
			...
			ImGuiTextEditCallback callback, void *user_data)
{
	...
	const bool focus_requested_by_code =
		focus_requested && (window->FocusIdxAllCounter ==
				    window->FocusIdxAllRequestCurrent);
	...
	if (focus_requested || user_clicked || user_scrolled ||
		...
		if (g.ActiveId != id) {
			...
			edit_state.CurLenW = ImTextStrFromUtf8(
				edit_state.Text.Data, edit_state.Text.Size, buf,
				NULL, &buf_end);
			edit_state.CurLenA =
				(int)(buf_end -
			...
			if (recycle_state) {
				...
				stb_textedit_initialize_state(
					&edit_state.StbState, !is_multiline);
				if (!is_multiline && focus_requested_by_code)
					select_all = true;
			}
			if (flags & ImGuiInputTextFlags_AlwaysInsertMode)
				edit_state.StbState.insert_mode = true;
			if (!is_multiline && (focus_requested_by_tab ||
					      (user_clicked && io.KeyCtrl)))
				select_all = true;
		}
		SetActiveID(id, window);
		SetFocusID(id, window);
	...
	if (g.ActiveId == id) {
		...
		if (select_all ||
			...
			edit_state.OnKeyPressed(STB_TEXTEDIT_K_WORDRIGHT |
			   ...
			   !edit_state.SelectedAllMouseLock) {
			if (hovered) {
				stb_textedit_click(&edit_state,
						   &edit_state.StbState,
						   mouse_x, mouse_y);
				edit_state.CursorAnimReset();
			}
		...
		if (io.InputCharacters[0]) {
			...
			bool ignore_inputs = (io.KeyCtrl && !io.KeyAlt) ||
					     (is_osx && io.KeySuper);
			if (!ignore_inputs && is_editable &&
			    !user_nav_input_start)
				for (int n = 0;
				     n < IM_ARRAYSIZE(io.InputCharacters) &&
	...
	if (g.ActiveId == id && !g.ActiveIdIsJustActivated &&
		...
		const bool is_osx_shift_shortcut = is_osx && io.KeySuper &&
		...
		const bool is_ctrl_key_only = io.KeyCtrl && !io.KeyShift &&
					      !io.KeyAlt && !io.KeySuper;
		const bool is_shift_key_only = io.KeyShift && !io.KeyCtrl &&
					       !io.KeyAlt && !io.KeySuper;
		...
		if (IsKeyPressedMap(ImGuiKey_LeftArrow)) {
			...
			edit_state.OnKeyPressed(STB_TEXTEDIT_K_BACKSPACE |
						k_mask);
			...
			if (io.SetClipboardTextFn) {
				const int ib =
					edit_state.HasSelection()
						? ImMin(edit_state.StbState
								.select_start,
							edit_state.StbState
								.select_end)
						: 0;
				const int ie =
					edit_state.HasSelection()
						? ImMax(edit_state.StbState
								.select_start,
							edit_state.StbState
								.select_end)
						: edit_state.CurLenW;
				edit_state.TempTextBuffer.resize((ie - ib) * 4 +
								 1);
	...
	if (g.ActiveId == id) {
		if (cancel_edit) {
			...
			if (is_editable &&
			    strncmp(buf, edit_state.InitialText.Data,
				    buf_size) != 0) {
				ImStrncpy(buf, edit_state.InitialText.Data,
					  buf_size);
				value_changed = true;
			}
		}
		...
		if (apply_edit_back_to_user_buffer) {
			...
			if (is_editable) {
				edit_state.TempTextBuffer.resize(
					edit_state.Text.Size * 4);
				ImTextStrToUtf8(edit_state.TempTextBuffer.Data,
						edit_state.TempTextBuffer.Size,
			...
			if (is_editable &&
				...
				ImStrncpy(buf, edit_state.TempTextBuffer.Data,
					  buf_size);
				value_changed = true;
			}
		}
	...
	if (g.ActiveId == id || is_currently_scrolling) {
...

		{
			...
			searches_input_ptr[0] =
				text_begin + edit_state.StbState.cursor;
			...
			for (const ImWchar *s = text_begin; *s != 0; s++)
				if (*s == '\n') {
					line_count++;
					if (searches_result_line_number[0] ==
						    -1 &&
					    s >= searches_input_ptr[0]) {
						searches_result_line_number[0] =
							line_count;
						if (--searches_remaining <= 0)
							break;
					}
					if (searches_result_line_number[1] ==
						    -1 &&
					    s >= searches_input_ptr[1]) {
						searches_result_line_number[1] =
							line_count;
						if (--searches_remaining <= 0)
							break;
					}
			...
			cursor_offset.x =
				InputTextCalcTextSizeW(
					...
					.x;
			cursor_offset.y =
				searches_result_line_number[0] * g.FontSize;
			...
			if (is_multiline)
				text_size =
					ImVec2(size.x, line_count * g.FontSize);
		}
			...
			if (!(flags & ImGuiInputTextFlags_NoHorizontalScroll)) {
				...
				if (cursor_offset.x < edit_state.ScrollX)
					edit_state.ScrollX = (float)(int)ImMax(
						...
						cursor_offset.x -
							scroll_increment_x);
				else if (cursor_offset.x - size.x >=
					 edit_state.ScrollX)
					edit_state.ScrollX =
						(float)(int)(cursor_offset.x -
							     size.x +
							     scroll_increment_x);
			} else {
				edit_state.ScrollX = 0.0f;
		...
		if (edit_state.StbState.select_start !=
			...
			ImVec2 rect_pos = render_pos + select_start_offset -
					  render_scroll;
		...
		ImVec2 cursor_screen_pos =
			render_pos + cursor_offset - render_scroll;
	...
	if (step != NULL) {
		...
		if (InputText(
			...
			value_changed = DataTypeApplyOpFromText(
				buf, g.InputTextState.InitialText.Data,
		...
		if (ButtonEx("-", ImVec2(button_size, button_size),
			...
			DataTypeApplyOp(data_type, '-', data_ptr, data_ptr,
					g.IO.KeyCtrl && step_fast ? step_fast
								  : step);
			value_changed = true;
		}
		SameLine(0, style.ItemInnerSpacing.x);
		     ...
		     ImGuiInputTextFlags extra_flags)
{
	...
	const char *format =
		(extra_flags & ImGuiInputTextFlags_CharsHexadecimal) ? "%08X"
								     : "%d";
			 ...
			 ImGuiInputTextFlags extra_flags)
{
	...
	for (int i = 0; i < components; i++) {
		PushID(i);
		value_changed |= InputScalar("##v", data_type, v, step,
					     step_fast, format, extra_flags);
		SameLine(0, g.Style.ItemInnerSpacing.x);
		PopID();
		PopItemWidth();
		v = (void *)((char *)v + type_size);
		       ...
		       ImGuiComboFlags flags)
{
	...
	const float arrow_size = (flags & ImGuiComboFlags_NoArrowButton)
					 ...
					 : GetFrameHeight();
	const ImVec2 label_size = CalcTextSize(label, NULL, true);
	const float w = (flags & ImGuiComboFlags_NoPreview) ? arrow_size
							    : CalcItemWidth();
	const ImRect frame_bb(
		window->DC.CursorPos,
		window->DC.CursorPos +
	...
	if (!(flags & ImGuiComboFlags_NoArrowButton)) {
		window->DrawList->AddRectFilled(
			...
			frame_bb.Max,
			GetColorU32((popup_open || hovered)
					    ? ImGuiCol_ButtonHovered
					    : ImGuiCol_Button),
			style.FrameRounding,
			(w <= arrow_size) ? ImDrawCornerFlags_All
					  : ImDrawCornerFlags_Right);
		RenderArrow(ImVec2(frame_bb.Max.x - arrow_size +
					   style.FramePadding.y,
				   frame_bb.Min.y + style.FramePadding.y),
			    ImGuiDir_Down);
	}
	...
	if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
		RenderTextClipped(frame_bb.Min + style.FramePadding,
				  value_bb.Max, preview_value, NULL, NULL,
				  ImVec2(0.0f, 0.0f));
	if (label_size.x > 0)
		RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x,
				  frame_bb.Min.y + style.FramePadding.y),
	...
	if (ImGuiWindow *popup_window = FindWindowByName(name))
		if (popup_window->WasActive) {
			...
			ImVec2 size_expected = CalcSizeAfterConstraint(
				popup_window,
				CalcSizeAutoFit(popup_window, size_contents));
		  ...
		  int items_count, int popup_max_height_in_items)
{
	...
	if (popup_max_height_in_items != -1 &&
	    !g.NextWindowData.SizeConstraintCond) {
		float popup_max_height = CalcMaxPopupHeightFromItemCount(
			popup_max_height_in_items);
		  ...
		  const char *items_separated_by_zeros, int height_in_items)
{
	...
	bool value_changed = Combo(label, current_item,
		     ...
		     bool enabled)
{
	...
	if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
		...
		ImVec2 shortcut_size = shortcut ? CalcTextSize(shortcut, NULL)
						: ImVec2(0.0f, 0.0f);
		...
		if (shortcut_size.x > 0.0f) {
			PushStyleColor(ImGuiCol_Text,
				       g.Style.Colors[ImGuiCol_TextDisabled]);
			RenderText(pos + ImVec2(window->MenuColumns.Pos[1] +
							extra_w,
						0.0f),
				   shortcut, NULL, false);
			PopStyleColor();
		...
		if (selected)
			RenderCheckMark(
				pos + ImVec2(window->MenuColumns.Pos[2] +
						     extra_w +
						     g.FontSize * 0.40f,
...
bool ImGui::BeginMainMenuBar()
{
	...
	bool is_open = Begin("##MainMenuBar", NULL, window_flags) &&
		       BeginMenuBar();
	PopStyleVar(2);
...
bool ImGui::BeginMenuBar()
{
	...
	ImRect clip_rect(
		...
		ImFloor(ImMax(bar_rect.Min.x,
			      bar_rect.Max.x - window->WindowRounding) +
			0.5f),
...
bool ImGui::BeginMenu(const char *label, bool enabled)
{
	...
	const bool hovered = enabled &&
			     ItemHoverable(window->DC.LastItemRect, id);
	if (menuset_is_open)
	    ...
	    ImGuiLayoutType_Vertical) // (window->Flags & (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
	{
		...
		if (g.HoveredWindow == window &&
			...
			if (ImGuiWindow *next_window =
				...
				tc.y = ta.y +
				       ImMin((tc.y + extra) - ta.y, +100.0f);
				moving_within_opened_triangle =
					ImTriangleContainsPoint(ta, tb, tc,
								g.IO.MousePos);
				//window->DrawList->PushClipRectFullScreen(); window->DrawList->AddTriangleFilled(ta, tb, tc, moving_within_opened_triangle ? IM_COL32(0,128,0,128) : IM_COL32(128,0,0,128)); window->DrawList->PopClipRect(); // Debug
			}
		}

		want_close = (menu_is_open && !hovered &&
			      g.HoveredWindow == window &&
			      g.HoveredIdPreviousFrame != 0 &&
			      g.HoveredIdPreviousFrame != id &&
			      !moving_within_opened_triangle);
		want_open = (!menu_is_open && hovered &&
			     !moving_within_opened_triangle) ||
			...
			ImGuiColorEditFlags flags, ImVec2 size)
{
	...
	if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col.w < 1.0f) {
		...
		RenderColorRectWithAlphaCheckerboard(
			...
			bb_inner.Max, GetColorU32(col), grid_step,
...
void ImGui::ColorEditOptionsPopup(const float *col, ImGuiColorEditFlags flags)
{
	...
	if (BeginPopup("Copy")) {
		...
		if (Selectable(buf))
			SetClipboardText(buf);
		if (flags & ImGuiColorEditFlags_NoAlpha)
			ImFormatString(buf, IM_ARRAYSIZE(buf), "0x%02X%02X%02X",
				       cr, cg, cb);
		else
				    ...
				    const float *ref_col)
{
	...
	if (allow_opt_picker) {
		ImVec2 picker_size(
			g.FontSize * 8,
			ImMax(g.FontSize * 8 - (ImGui::GetFrameHeight() +
	...
	if (!(flags & ImGuiColorEditFlags_NoSmallPreview)) {
		...
		if (BeginPopup("picker")) {
			...
			ImGuiColorEditFlags picker_flags =
				...
				ImGuiColorEditFlags_NoLabel |
				ImGuiColorEditFlags_AlphaPreviewHalf;
			PushItemWidth(square_sz *
				      12.0f); // Use 256 + bar sizes?
			value_changed |= ColorPicker4("##picker", col,
						      picker_flags,
						      &g.ColorPickerRef.x);
			PopItemWidth();
			EndPopup();
				       ...
				       ImVec2 half_sz, float bar_w)
{
	...
	RenderArrow(draw_list, ImVec2(pos.x + bar_w - half_sz.x - 1, pos.y),
		    ...
		    IM_COL32_BLACK);
	RenderArrow(draw_list, ImVec2(pos.x + bar_w - half_sz.x, pos.y),
			 ...
			 ImGuiColorEditFlags flags, const float *ref_col)
{
		...
		if (IsItemActive()) {
			...
			if (ImTriangleContainsPoint(
				...
				ImVec2 current_off_unrotated =
					ImRotate(current_off, cos_hue_angle,
						 sin_hue_angle);
				if (!ImTriangleContainsPoint(
	...
	if (flags & ImGuiColorEditFlags_PickerHueWheel) {
		...
		for (int n = 0; n < 6; n++) {
			...
			draw_list->PathStroke(IM_COL32_WHITE, false,
					      wheel_thickness);
		...
		draw_list->AddCircle(hue_cursor_pos, hue_cursor_rad,
				     IM_COL32_WHITE, hue_cursor_segments);
		...
		draw_list->AddTriangle(tra, trb, trc,
		...
		RenderFrameBorder(picker_pos,
				  picker_pos + ImVec2(sv_picker_size,
						      sv_picker_size),
				  0.0f);
		sv_cursor_pos.x = ImClamp(
			(float)(int)(picker_pos.x +
				     ImSaturate(S) * sv_picker_size + 0.5f),
			picker_pos.x + 2,
	...
	if (alpha_bar) {
		...
		RenderColorRectWithAlphaCheckerboard(bar1_bb.Min, bar1_bb.Max,
						     ...
						     ImVec2(0.0f, 0.0f));
		draw_list->AddRectFilledMultiColor(
			bar1_bb.Min, bar1_bb.Max, col32_no_alpha,
			col32_no_alpha, col32_no_alpha & ~IM_COL32_A_MASK,
			col32_no_alpha & ~IM_COL32_A_MASK);
		float bar1_line_y =
			(float)(int)(picker_pos.y +
				     (1.0f - alpha) * sv_picker_size + 0.5f);
...
void ImGui::Separator()
{
	...
	if ((flags & (ImGuiSeparatorFlags_Horizontal |
		      ImGuiSeparatorFlags_Vertical)) == 0)
...
void ImGui::BeginGroup()
{
	...
	window->DC.GroupOffsetX = window->DC.CursorPos.x - window->Pos.x -
				  window->DC.ColumnsOffsetX;
...
void ImGui::SetColumnOffset(int column_index, float offset)
{
	...
	if (preserve_width)
		SetColumnOffset(column_index + 1,
				offset + ImMax(g.Style.ColumnsMinSpacing,
					       width));
}
			 ...
			 ImGuiColumnsFlags flags)
{
	...
	columns->MinX = window->DC.IndentX -
			g.Style.ItemSpacing.x; // Lock our horizontal range
	columns->MaxX = ImMax(content_region_width - window->Scroll.x,
			      columns->MinX + 1.0f);
	...
	if (!(columns->Flags & ImGuiColumnsFlags_NoBorder) &&
		...
		for (int n = 1; n < columns->Count; n++) {
			...
			if (!(columns->Flags & ImGuiColumnsFlags_NoResize)) {
				ButtonBehavior(column_rect, column_id, &hovered,
					       &held);
				if (hovered || held)
					g.MouseCursor =
						ImGuiMouseCursor_ResizeEW;
...
void ImGui::Indent(float indent_w)
{
	ImGuiContext &g = *GImGui;
	ImGuiWindow *window = GetCurrentWindow();
	window->DC.IndentX += (indent_w != 0.0f) ? indent_w
...
void ImGui::ShowMetricsWindow(bool *p_open)
{
	if (ImGui::Begin("ImGui Metrics", p_open)) {
		...
		ImGui::Text("%d allocations",
			    (int)GImAllocatorActiveAllocationsCount);
		ImGui::Checkbox(
			"Show clipping rectangles when hovering draw commands",
		...
		struct Funcs {
			...
			static void
			NodeWindows(ImVector<ImGuiWindow *> &windows,
				    const char *label)
			{
					       ...
					       const char *label)
			{
				if (!ImGui::TreeNode(
					...
					return;
				ImGuiWindowFlags flags = window->Flags;
				NodeDrawList(window, window->DrawList,
					     "DrawList");
				ImGui::BulletText(
				...
				if (window->ColumnsStorage.Size > 0 &&
					...
					for (int n = 0;
					     n < window->ColumnsStorage.Size;
					     n++) {
						const ImGuiColumnsSet *columns =
							&window->ColumnsStorage
								 [n];
						if (ImGui::TreeNode(
							    (void *)(uintptr_t)
								    columns->ID,
		...
		if (ImGui::TreeNode("DrawList", "Active DrawLists (%d)",
			...
			for (int i = 0; i < g.DrawDataBuilder.Layers[0].Size;
			     i++)
				Funcs::NodeDrawList(
					NULL, g.DrawDataBuilder.Layers[0][i],
					"DrawList");
			ImGui::TreePop();
		}
		...
		if (ImGui::TreeNode("Internal state")) {
			const char *input_source_names[] = {"None", "Mouse",
							    "Nav",
							    "NavKeyboard",
							    "NavGamepad"};
			...
			ImGui::Text(
				...
				g.ActiveIdTimer, g.ActiveIdAllowOverlap,
