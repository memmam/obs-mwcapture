...
void ImGui::ShowDemoWindow(bool *p_open)
{
	...
	if (ImGui::CollapsingHeader("Widgets")) {
		if (ImGui::TreeNode("Basic")) {
			...
			for (int i = 0; i < 7; i++) {
				...
				ImGui::PushStyleColor(
					ImGuiCol_Button,
					(ImVec4)ImColor::HSV(i / 7.0f, 0.6f,
							     0.6f));
				ImGui::PushStyleColor(
					ImGuiCol_ButtonHovered,
					(ImVec4)ImColor::HSV(i / 7.0f, 0.7f,
							     0.7f));
				ImGui::PushStyleColor(
					ImGuiCol_ButtonActive,
					(ImVec4)ImColor::HSV(i / 7.0f, 0.8f,
							     0.8f));
				ImGui::Button("Click");
...

			{
				...
				ImGui::InputFloat("input float", &f0, 0.01f,
						  1.0f);

				static double d0 = 999999.00000001;
				ImGui::InputDouble("input double", &d0, 0.01f,
						   1.0f, "%.8f");

				static float f1 = 1.e10f;
				ImGui::InputFloat("input scientific", &f1, 0.0f,
						  0.0f, "%e");
				ImGui::SameLine();
				ShowHelpMarker(
...

			{
				...
				ImGui::DragInt("drag int 0..100", &i2, 1, 0,
					       100, "%d%%");

				static float f1 = 1.00f, f2 = 0.0067f;
				ImGui::DragFloat("drag float", &f1, 0.005f);
				ImGui::DragFloat("drag small float", &f2,
						 0.0001f, 0.0f, 0.0f,
						 "%.06f ns");
			}

			{
				...
				ImGui::SliderFloat("slider float", &f1, 0.0f,
						   1.0f, "ratio = %.3f");
				ImGui::SliderFloat("slider float (curve)", &f2,
						   -10.0f, 10.0f, "%.4f", 2.0f);
		...
		if (ImGui::TreeNode("Trees")) {
			...
			if (ImGui::TreeNode(
				...
				static bool align_label_with_current_x_position =
					false;
				ImGui::Checkbox(
		...
		if (ImGui::TreeNode("Text")) {
			...
			if (ImGui::TreeNode("Word Wrapping")) {
				...
				ImGui::SliderFloat("Wrap width", &wrap_width,
						   -20, 600, "%.0f");
		...
		if (ImGui::TreeNode("Combo")) {
			...
			const char *items[] = {"AAAA", "BBBB",   "CCCC",
					       ...
					       "JJJJ", "KKKK",   "LLLLLLL",
					       "MMMM", "OOOOOOO"};
			static const char *item_current = items
		...
		if (ImGui::TreeNode("Selectables")) {
			...
			if (ImGui::TreeNode("Basic")) {
				static bool selection[5] = {false, true, false,
							    false, false};
				ImGui::Selectable("1. I am selectable",
						  &selection[0]);
				ImGui::Selectable("2. I am selectable",
						  &selection[1]);
				ImGui::Text("3. I am not selectable");
				ImGui::Selectable("4. I am selectable",
						  &selection[3]);
				if (ImGui::Selectable(
			...
			if (ImGui::TreeNode(
				...
				static bool selection[5] = {false, false, false,
							    false, false};
				for (int n = 0; n < 5; n++) {
					char buf[32];
					sprintf(buf, "Object %d", n);
			...
			if (ImGui::TreeNode("Grid")) {
				static bool selected[16] = {true,  false, false,
							    false, false, true,
							    false, false, false,
							    false, true,  false,
							    false, false, false,
		...
		if (ImGui::TreeNode("Plots Widgets")) {
			...
			ImGui::PlotLines("Lines", values, IM_ARRAYSIZE(values),
					 ...
					 ImVec2(0, 80));
			ImGui::PlotHistogram("Histogram", arr,
					     IM_ARRAYSIZE(arr), 0, NULL, 0.0f,
					     1.0f, ImVec2(0, 80));
			...
			ImGui::PopItemWidth();
			ImGui::SameLine();
			ImGui::SliderInt("Sample count", &display_count, 1,
					 400);
			float (*func)(void *, int) =
				(func_type == 0) ? Funcs::Sin : Funcs::Saw;
			ImGui::PlotLines("Lines", func, NULL, display_count, 0,
					 NULL, -1.0f, 1.0f, ImVec2(0, 80));
			ImGui::PlotHistogram("Histogram", func, NULL,
					     display_count, 0, NULL, -1.0f,
					     1.0f, ImVec2(0, 80));
			ImGui::Separator();
			...
			float progress_saturated = (progress < 0.0f) ? 0.0f
						   : (progress > 1.0f)
							   ...
							   : progress;
			char buf[32];
			sprintf(buf, "%d/%d", (int)(progress_saturated * 1753),
				1753);
		...
		if (ImGui::TreeNode("Color/Picker Widgets")) {
			...
			ImGui::Checkbox("With Half Alpha Preview",
					&alpha_half_preview);
			...
			if (!saved_palette_inited)
				for (int n = 0; n < IM_ARRAYSIZE(saved_palette);
				     n++) {
			...
			bool open_popup = ImGui::ColorButton("MyColor##3b",
							     color, misc_flags);
			ImGui::SameLine();
			...
			ImGui::ColorPicker4("MyColor##4", (float *)&color,
					    flags,
		...
		if (ImGui::TreeNode("Range Widgets")) {
			...
			ImGui::DragFloatRange2("range", &begin, &end, 0.25f,
					       0.0f, 100.0f, "Min: %.1f %%",
					       "Max: %.1f %%");
			ImGui::DragIntRange2("range int (no bounds)", &begin_i,
					     &end_i, 5, 0, 0, "Min: %d units",
					     "Max: %d units");
			ImGui::TreePop();
		}
		...
		if (ImGui::TreeNode("Data Types")) {
			...
			ImGui::DragScalar("drag float", ImGuiDataType_Float,
					  &f32_v, 0.005f, &f32_zero, &f32_one,
					  "%f", 1.0f);
			...
			ImGui::DragScalar("drag double", ImGuiDataType_Double,
					  &f64_v, 0.0005f, &f64_zero, NULL,
					  "%.10f grams", 1.0f);
			ImGui::DragScalar("drag double ^2",
					  ImGuiDataType_Double, &f64_v, 0.0005f,
					  &f64_zero, &f64_one, "0 < %.10f < 1",
					  2.0f);

			ImGui::Text("Sliders");
			ImGui::SliderScalar("slider s32 low", ImGuiDataType_S32,
					    &s32_v, &s32_zero, &s32_fifty,
					    "%d");
			ImGui::SliderScalar("slider s32 high",
					    ImGuiDataType_S32, &s32_v,
					    &s32_hi_a, &s32_hi_b, "%d");
			ImGui::SliderScalar("slider s32 full",
					    ImGuiDataType_S32, &s32_v, &s32_min,
					    &s32_max, "%d");
			ImGui::SliderScalar("slider u32 low", ImGuiDataType_U32,
					    &u32_v, &u32_zero, &u32_fifty,
					    "%u");
			ImGui::SliderScalar("slider u32 high",
					    ImGuiDataType_U32, &u32_v,
					    &u32_hi_a, &u32_hi_b, "%u");
			ImGui::SliderScalar("slider u32 full",
					    ImGuiDataType_U32, &u32_v, &u32_min,
					    &u32_max, "%u");
			ImGui::SliderScalar("slider s64 low", ImGuiDataType_S64,
					    &s64_v, &s64_zero, &s64_fifty,
					    "%I64d");
			ImGui::SliderScalar("slider s64 high",
					    ImGuiDataType_S64, &s64_v,
					    &s64_hi_a, &s64_hi_b, "%I64d");
			ImGui::SliderScalar("slider s64 full",
					    ImGuiDataType_S64, &s64_v, &s64_min,
					    &s64_max, "%I64d");
			ImGui::SliderScalar("slider u64 low", ImGuiDataType_U64,
					    &u64_v, &u64_zero, &u64_fifty,
					    "%I64u ms");
			ImGui::SliderScalar("slider u64 high",
					    ImGuiDataType_U64, &u64_v,
					    &u64_hi_a, &u64_hi_b, "%I64u ms");
			ImGui::SliderScalar("slider u64 full",
					    ImGuiDataType_U64, &u64_v, &u64_min,
					    &u64_max, "%I64u ms");
			ImGui::SliderScalar("slider float low",
					    ImGuiDataType_Float, &f32_v,
					    &f32_zero, &f32_one);
			...
			ImGui::InputScalar("input s32", ImGuiDataType_S32,
					   &s32_v,
					   inputs_step ? &s32_one : NULL, NULL,
					   "%d");
			ImGui::InputScalar(
				"input s32 hex", ImGuiDataType_S32, &s32_v,
				inputs_step ? &s32_one : NULL, NULL, "%08X",
				ImGuiInputTextFlags_CharsHexadecimal);
			ImGui::InputScalar("input u32", ImGuiDataType_U32,
					   &u32_v,
					   inputs_step ? &u32_one : NULL, NULL,
					   "%u");
			ImGui::InputScalar(
			...
			ImGui::InputScalar("input u64", ImGuiDataType_U64,
					   &u64_v,
					   inputs_step ? &u64_one : NULL);
			ImGui::InputScalar("input float", ImGuiDataType_Float,
					   &f32_v,
					   inputs_step ? &f32_one : NULL);
			ImGui::InputScalar("input double", ImGuiDataType_Double,
					   &f64_v,
		...
		if (ImGui::TreeNode("Multi-component Widgets")) {
			...
			ImGui::DragFloat2("drag float2", vec4f, 0.01f, 0.0f,
					  1.0f);
			...
			ImGui::DragFloat3("drag float3", vec4f, 0.01f, 0.0f,
					  1.0f);
			...
			ImGui::DragFloat4("drag float4", vec4f, 0.01f, 0.0f,
					  1.0f);
		...
		if (ImGui::TreeNode("Vertical Sliders")) {
			...
			ImGui::VSliderInt("##int", ImVec2(18, 160), &int_value,
					  0, 5);
			ImGui::SameLine();
			...
			for (int i = 0; i < 7; i++) {
				...
				ImGui::PushStyleColor(
					ImGuiCol_FrameBg,
					(ImVec4)ImColor::HSV(i / 7.0f, 0.5f,
							     0.5f));
				ImGui::PushStyleColor(
				...
				ImGui::PushStyleColor(
					ImGuiCol_SliderGrab,
					(ImVec4)ImColor::HSV(i / 7.0f, 0.9f,
							     0.9f));
				ImGui::VSliderFloat("##v", ImVec2(18, 160),
						    &values[i], 0.0f, 1.0f, "");
			...
			for (int i = 0; i < 4; i++) {
				...
				ImGui::PushStyleVar(ImGuiStyleVar_GrabMinSize,
						    40);
				ImGui::VSliderFloat("##v", ImVec2(40, 160),
						    &values[i], 0.0f, 1.0f,
						    "%.2f\nsec");
				ImGui::PopStyleVar();
				ImGui::PopID();
	...
	if (ImGui::CollapsingHeader("Layout")) {
		if (ImGui::TreeNode("Child regions")) {
			...
			ImGui::Checkbox("Disable Mouse Wheel",
					&disable_mouse_wheel);
		...
		if (ImGui::TreeNode("Widgets Width")) {
			...
			ImGui::PushItemWidth(
				ImGui::GetContentRegionAvailWidth() * 0.5f);
		...
		if (ImGui::TreeNode("Groups")) {
			...
			const float values[5] = {0.5f, 0.20f, 0.80f, 0.60f,
						 0.25f};
			ImGui::PlotHistogram("##values", values,
					     IM_ARRAYSIZE(values), 0, NULL,
					     0.0f, 1.0f, size);
		...
		if (ImGui::TreeNode("Scrolling")) {
			...
			ImGui::PushItemWidth(100);
			ImGui::SameLine(130);
			track |= ImGui::DragInt("##line", &track_line, 0.25f, 0,
						99, "Line = %d");
			bool scroll_to = ImGui::Button("Scroll To Pos");
			ImGui::SameLine(130);
			scroll_to |= ImGui::DragInt("##pos_y", &scroll_to_px,
						    1.00f, 0, 9999,
						    "Y = %d px");
			ImGui::PopItemWidth();
			if (scroll_to)
		...
		if (ImGui::TreeNode("Horizontal Scrolling")) {
			...
			for (int line = 0; line < lines; line++) {
				...
				for (int n = 0; n < num_buttons; n++) {
					...
					ImGui::PushStyleColor(
						ImGuiCol_Button,
						(ImVec4)ImColor::HSV(hue, 0.6f,
								     0.6f));
					ImGui::PushStyleColor(
						ImGuiCol_ButtonHovered,
						(ImVec4)ImColor::HSV(hue, 0.7f,
								     0.7f));
					ImGui::PushStyleColor(
						ImGuiCol_ButtonActive,
						(ImVec4)ImColor::HSV(hue, 0.8f,
								     0.8f));
					ImGui::Button(
						label,
						ImVec2(40.0f + sinf((float)(line +
									    n)) *
								       20.0f,
						       0.0f));
					ImGui::PopStyleColor(3);
		...
		if (ImGui::TreeNode("Clipping")) {
			...
			ImGui::DragFloat2("size", (float *)&size, 0.5f, 0.0f,
					  200.0f, "%.0f");
			ImGui::TextWrapped("(Click and drag)");
			ImVec2 pos = ImGui::GetCursorScreenPos();
			ImVec4 clip_rect(pos.x, pos.y, pos.x + size.x,
					 pos.y + size.y);
	...
	if (ImGui::CollapsingHeader("Popups & Modal windows")) {
		if (ImGui::TreeNode("Popups")) {
			...
			ImGui::SameLine();
			ImGui::TextUnformatted(selected_fish == -1
						       ? "<None>"
						       : names[selected_fish]);
			...
			if (ImGui::BeginPopup("toggle")) {
				...
				if (ImGui::BeginPopup("another popup")) {
					for (int i = 0; i < IM_ARRAYSIZE(names);
					     i++)
						ImGui::MenuItem(names[i], "",
								&toggles[i]);
					if (ImGui::BeginMenu("Sub-menu")) {
		...
		if (ImGui::TreeNode("Context menus")) {
			...
			ImGui::Text("Value = %.3f (<-- right-click here)",
				    value);
			if (ImGui::BeginPopupContextItem("item context menu")) {
				...
				if (ImGui::Selectable("Set to PI"))
					value = 3.1415f;
				ImGui::PushItemWidth(-1);
				ImGui::DragFloat("##Value", &value, 0.1f, 0.0f,
						 0.0f);
				ImGui::PopItemWidth();
				ImGui::EndPopup();
			}
		...
		if (ImGui::TreeNode("Modals")) {
			...
			if (ImGui::BeginPopupModal("Stacked 1")) {
				...
				ImGui::Combo(
					"Combo", &item,
					"aaaa\0bbbb\0cccc\0dddd\0eeee\0\0");
				static float color[4] = {0.4f, 0.7f, 0.0f,
							 0.5f};
				ImGui::ColorEdit4(
					"color",
...
void ImGui::ShowStyleEditor(ImGuiStyle *ref)
{
	...
	if (ImGui::TreeNode("Rendering")) {
		...
		ImGui::DragFloat("Curve Tessellation Tolerance",
				 &style.CurveTessellationTol, 0.02f, 0.10f,
				 FLT_MAX, NULL, 2.0f);
	...
	if (ImGui::TreeNode("Settings")) {
		...
		ImGui::SliderFloat2("ItemInnerSpacing",
				    (float *)&style.ItemInnerSpacing, 0.0f,
				    20.0f, "%.0f");
		...
		ImGui::SliderFloat("ScrollbarSize", &style.ScrollbarSize, 1.0f,
				   20.0f, "%.0f");
		ImGui::SliderFloat("GrabMinSize", &style.GrabMinSize, 1.0f,
				   20.0f, "%.0f");
		...
		ImGui::SliderFloat("FrameBorderSize", &style.FrameBorderSize,
				   0.0f, 1.0f, "%.0f");
		ImGui::Text("Rounding");
		ImGui::SliderFloat("WindowRounding", &style.WindowRounding,
				   0.0f, 14.0f, "%.0f");
	...
	if (fonts_opened) {
		...
		for (int i = 0; i < atlas->Fonts.Size; i++) {
			...
			if (font_details_opened) {
				...
				ImGui::PopFont();
				ImGui::DragFloat(
					"Font scale", &font->Scale, 0.005f,
					0.3f, 2.0f,
					"%.1f"); // Scale only this font
				ImGui::InputFloat("Font offset",
						  &font->DisplayOffset.y, 1, 1,
						  0);
				ImGui::SameLine();
				ShowHelpMarker(
					...
					for (int base = 0; base < 0x10000;
						...
						if (count > 0 &&
							...
							float cell_spacing =
								style.ItemSpacing
									.y;
							ImVec2 base_pos = ImGui::
								GetCursorScreenPos();
							ImDrawList *draw_list = ImGui::
								GetWindowDrawList();
							for (int n = 0; n < 256;
							     n++) {
								ImVec2 cell_p1(
		...
		ImGui::DragFloat("this window scale", &window_scale, 0.005f,
				 0.3f, 2.0f, "%.1f"); // scale only this window
		ImGui::DragFloat("global scale",
				 &ImGui::GetIO().FontGlobalScale, 0.005f, 0.3f,
...
static void ShowExampleAppConstrainedResize(bool *p_open)
{
	struct CustomConstraints // Helper functions to demonstrate programmatic constraints
	{
		...
		static void Step(ImGuiSizeCallbackData *data)
		{
			...
			data->DesiredSize = ImVec2(
				...
				(int)(data->DesiredSize.y / step + 0.5f) *
					step);
		}
	};
	...
	ImGuiWindowFlags flags = auto_resize ? ImGuiWindowFlags_AlwaysAutoResize
					     : 0;
...
static void ShowExampleAppSimpleOverlay(bool *p_open)
{
	...
	static int corner = 0;
	ImVec2 window_pos =
		ImVec2((corner & 1) ? ImGui::GetIO().DisplaySize.x - DISTANCE
				    : DISTANCE,
		       (corner & 2) ? ImGui::GetIO().DisplaySize.y - DISTANCE
				    : DISTANCE);
...
static void ShowExampleAppCustomRendering(bool *p_open)
{
	...
	ImGui::ColorEdit3("Color", &col.x);
	{
		...
		for (int n = 0; n < 2; n++) {
			...
			draw_list->AddRect(ImVec2(x, y), ImVec2(x + sz, y + sz),
					   col32, 0.0f, ImDrawCornerFlags_All,
					   curr_thickness);
			x += sz + spacing;
			draw_list->AddRect(ImVec2(x, y), ImVec2(x + sz, y + sz),
					   col32, 10.0f, ImDrawCornerFlags_All,
					   curr_thickness);
			x += sz + spacing;
			draw_list->AddRect(ImVec2(x, y), ImVec2(x + sz, y + sz),
					   col32, 10.0f,
	...
	ImGui::Separator();
	{
		...
		for (int i = 0; i < points.Size - 1; i += 2)
			draw_list->AddLine(
				ImVec2(canvas_pos.x + points[i].x,
	...
	void Draw(const char *title, bool *p_open = NULL)
	{
		...
		if (Filter.IsActive()) {
			...
			for (int line_no = 0; line != NULL; line_no++) {
				...
				line = line_end && line_end[1] ? line_end + 1
							       : NULL;
			}
		} else {
...
static void ShowExampleAppLog(bool *p_open)
{
	...
	if (time - last_time >= 0.20f && !ImGui::GetIO().KeyCtrl) {
		const char *random_words[] = {"system", "info",  "warning",
					      "error",  "fatal", "notice",
					      "log"};
...
static void ShowExampleAppPropertyEditor(bool *p_open)
{
	...
	struct funcs {
		static void ShowDummyObject(const char *prefix, int uid)
		{
			...
			bool node_open =
				ImGui::TreeNode("Object", "%s_%u", prefix, uid);
			ImGui::NextColumn();
			ImGui::AlignTextToFramePadding();
			...
			if (node_open) {
				...
				for (int i = 0; i < 8; i++) {
					ImGui::PushID(
						i); // Use field index as identifier.
					if (i < 2) {
						ShowDummyObject("Child",
								424242);
					} else {
