...
STDMETHODIMP
CBaseReferenceClock::NonDelegatingQueryInterface(REFIID riid,
...
#ifdef DXMPERF
	PERFLOG_CTOR(pName ? pName : L"CBaseReferenceClock",
		...
		*phr = E_OUTOFMEMORY;
...
#ifdef PERF
		m_idGetSystemTime =
			MSR_REGISTER(TEXT("CBaseReferenceClock::GetTime"));
#endif
		...
		if (!pShed) {
			...
			if (m_hThread) {
				...
				*phr = E_FAIL;
				EXECUTE_ASSERT(
					CloseHandle(m_pSchedule->GetEvent()));
				delete m_pSchedule;
				m_pSchedule = NULL;
			}
		}
...
STDMETHODIMP CBaseReferenceClock::GetTime(__out REFERENCE_TIME *pTime)
{
	...
	if (pTime) {
		...
		*pTime = m_rtLastGotTime;
		Unlock();
		MSR_INTEGER(m_idGetSystemTime,
			    LONG((*pTime) / (UNITS / MILLISECONDS)));
	...
	__out DWORD_PTR *pdwAdviseCookie) // where your cookie goes
{
	...
	*pdwAdviseCookie = 0;
	...
	if (lRefTime <= 0 || lRefTime == MAX_TIME) {
		...
		*pdwAdviseCookie = m_pSchedule->AddAdvisePacket(
			lRefTime, 0, HANDLE(hEvent), FALSE);
	...
	__out DWORD_PTR *pdwAdviseCookie) // where your cookie goes
{
	...
	*pdwAdviseCookie = 0;
	...
	if (StartTime > 0 && PeriodTime > 0 && StartTime != MAX_TIME) {
		*pdwAdviseCookie = m_pSchedule->AddAdvisePacket(
			StartTime, PeriodTime, HANDLE(hSemaphore), TRUE);
		hr = *pdwAdviseCookie ? NOERROR : E_OUTOFMEMORY;
	} else
		hr = E_INVALIDARG;
...
REFERENCE_TIME CBaseReferenceClock::GetPrivateTime()
{
	...
	{
		m_rtPrivateTime +=
			Int32x32To64(UNITS / MILLISECONDS,
				     (DWORD)(dwTime - m_dwPrevSystemTime));
...
STDMETHODIMP CBaseReferenceClock::SetTimeDelta(const REFERENCE_TIME &TimeDelta)
{
	...
	DbgLog((LOG_TIMING, Severity < 0 ? 0 : Severity,
		...
		Severity, usDelta,
		DWORD(ConvertToMilliseconds(m_rtPrivateTime)),
...
{
	return DWORD(
		reinterpret_cast<CBaseReferenceClock *>(p)->AdviseThread());
}
...
HRESULT CBaseReferenceClock::AdviseThread()
{
	...
	while (!m_bAbort) {
		...
		DbgLog((LOG_TIMING, 3,
			TEXT("CBaseRefClock::AdviseThread() Delay: %lu ms"),
			dwWait));
		...
		dwWait = (llWait > REFERENCE_TIME(UINT_MAX)) ? UINT_MAX
							     : DWORD(llWait);
	};
	return NOERROR;
}
...
)
{
	...
	if (0 == timerResolution) {
		...
		TIMECAPS tc;
		DWORD dwMinResolution =
			(TIMERR_NOERROR == timeGetDevCaps(&tc, sizeof(tc)))
				? tc.wPeriodMin
				: 1;
		DWORD dwResolution =
			max(dwMinResolution, DWORD(timerResolution / 10000));
