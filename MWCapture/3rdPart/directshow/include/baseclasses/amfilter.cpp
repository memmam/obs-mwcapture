...
{
	return CoCreateInstance(CLSID_MemoryAllocator, 0, CLSCTX_INPROC_SERVER,
			 ...
			 __deref_out IUnknown **ppPassThru)
{
	*ppPassThru = NULL;
	IUnknown *pUnkSeek;
	HRESULT hr = CoCreateInstance(CLSID_SeekingPassThru, pAgg,
	...
	hr = pUnkSeek->QueryInterface(IID_ISeekingPassThru,
				      (void **)&pPassThru);
	if (FAILED(hr)) {
		pUnkSeek->Release();
		return hr;
...
STDMETHODIMP
CBaseMediaFilter::NonDelegatingQueryInterface(REFIID riid,
...
CBaseFilter::CBaseFilter(__in_opt LPCTSTR pName, __inout_opt LPUNKNOWN pUnk,
			 __in CCritSec *pLock, REFCLSID clsid)
	: CUnknown(pName, pUnk),
	  m_pLock(pLock),
	  m_clsid(clsid),
...
CBaseFilter::CBaseFilter(__in_opt LPCTSTR pName, __in_opt LPUNKNOWN pUnk,
			 __in CCritSec *pLock, REFCLSID clsid,
			 __inout HRESULT *phr)
	: CUnknown(pName, pUnk),
	  m_pLock(pLock),
...
#ifdef UNICODE
CBaseFilter::CBaseFilter(__in_opt LPCSTR pName, __in_opt LPUNKNOWN pUnk,
			 __in CCritSec *pLock, REFCLSID clsid)
	: CUnknown(pName, pUnk),
	  m_pLock(pLock),
	  m_clsid(clsid),
...
#ifdef DXMPERF
	PERFLOG_STOP(m_pName ? m_pName : L"CBaseFilter", (IBaseFilter *)this,
		     m_State);
#endif // DXMPERF
...
#ifdef DXMPERF
	PERFLOG_PAUSE(m_pName ? m_pName : L"CBaseFilter", (IBaseFilter *)this,
		      m_State);
#endif // DXMPERF
...
#ifdef DXMPERF
	PERFLOG_RUN(m_pName ? m_pName : L"CBaseFilter", (IBaseFilter *)this,
		    tStart, m_State);
#endif // DXMPERF
...
CBaseFilter::QueryFilterInfo(__out FILTER_INFO *pInfo)
{
	...
	if (m_pName) {
		(void)StringCchCopyW(pInfo->achName, NUMELMS(pInfo->achName),
				     m_pName);
	} else {
		pInfo->achName[0] = L'\0';
	}
...
STDMETHODIMP
CBaseFilter::JoinFilterGraph(__inout_opt IFilterGraph *pGraph,
...
#ifdef DXMPERF
	PERFLOG_JOINGRAPH(m_pName ? m_pName : L"CBaseFilter",
			  (IBaseFilter *)this, pGraph);
#endif // DXMPERF
...
HRESULT
CBaseFilter::NotifyEvent(long EventCode, LONG_PTR EventParam1,
...
CBaseFilter::ReconnectPin(IPin *pPin, __in_opt AM_MEDIA_TYPE const *pmt)
{
	...
	if (m_pGraph != NULL) {
		HRESULT hr = m_pGraph->QueryInterface(IID_IFilterGraph2,
						      (void **)&pGraph2);
		if (SUCCEEDED(hr)) {
...
CEnumMediaTypes::CEnumMediaTypes(__in CBasePin *pPin,
	...
	: m_Position(0), m_pPin(pPin), m_cRef(1)
{
		      ...
		      __out ULONG *pcFetched) // actual count passed
{
	...
	ValidateReadWritePtr(ppMediaTypes,
			     cMediaTypes * sizeof(AM_MEDIA_TYPE *));
	/* Check we are still in sync with the pin */
	if (AreWeOutOfSync() == TRUE) {
		return VFW_E_ENUM_OUT_OF_SYNC;
	}
...
CEnumMediaTypes::Skip(ULONG cMediaTypes)
{
	...
	CMediaType cmt;
	return S_OK == m_pPin->GetMediaType(m_Position - 1, &cmt) ? S_OK
								  : S_FALSE;
}
...
CBasePin::CBasePin(__in_opt LPCTSTR pObjectName, __in CBaseFilter *pFilter,
    ...
    */
	...
	if (pName) {
		...
		if (SUCCEEDED(hr)) {
			...
			if (m_pName) {
				(void)StringCchCopyW(m_pName, cchName + 1,
						     pName);
			}
		}
	}
...
#ifdef UNICODE
CBasePin::CBasePin(__in_opt LPCSTR pObjectName, __in CBaseFilter *pFilter,
		   __in CCritSec *pLock, __inout HRESULT *phr,
		   __in_opt LPCWSTR pName, PIN_DIRECTION dir)
	: CUnknown(pObjectName, NULL),
	  m_pFilter(pFilter),
...
void CBasePin::DisplayPinInfo(IPin *pReceivePin)
{
	...
	if (DbgCheckModuleLevel(LOG_TRACE, CONNECT_TRACE_LEVEL)) {
		...
		if (FAILED(QueryPinInfo(&ConnectPinInfo))) {
			StringCchCopyW(ConnectPinInfo.achName,
		...
		if (FAILED(pReceivePin->QueryPinInfo(&ReceivePinInfo))) {
			StringCchCopyW(ReceivePinInfo.achName,
		...
		DbgLog((LOG_TRACE, CONNECT_TRACE_LEVEL,
			TEXT("Trying to connect Pins :")));
		DbgLog((LOG_TRACE, CONNECT_TRACE_LEVEL, TEXT("    <%ls>"),
			ConnectPinInfo.achName));
		DbgLog((LOG_TRACE, CONNECT_TRACE_LEVEL, TEXT("    <%ls>"),
			ReceivePinInfo.achName));
	}
}
#endif
...
void CBasePin::DisplayTypeInfo(IPin *pPin, const CMediaType *pmt)
{
	...
	if (DbgCheckModuleLevel(LOG_TRACE, CONNECT_TRACE_LEVEL)) {
		DbgLog((LOG_TRACE, CONNECT_TRACE_LEVEL,
			TEXT("Trying media type:")));
		DbgLog((LOG_TRACE, CONNECT_TRACE_LEVEL,
			TEXT("    major type:  %hs"), GuidNames[*pmt->Type()]));
		DbgLog((LOG_TRACE, CONNECT_TRACE_LEVEL,
			TEXT("    sub type  :  %hs"),
				...
				IEnumMediaTypes *pEnum)
{
	...
	for (;;) {
		...
		hr = pEnum->Next(1, (AM_MEDIA_TYPE **)&pMediaType,
				 &ulMediaCount);
		if (hr != S_OK) {
			if (S_OK == hrFailure) {
		...
		if (pMediaType &&
		    ((pmt == NULL) || pMediaType->MatchesPartial(pmt))) {

			hr = AttemptConnection(pReceivePin, pMediaType);

			// attempt to remember a specific error code
			if (FAILED(hr) && SUCCEEDED(hrFailure) &&
			    (hr != E_FAIL) && (hr != E_INVALIDARG) &&
			    (hr != VFW_E_TYPE_NOT_ACCEPTED)) {
				hrFailure = hr;
			}
		} else {
...
)
{
...
#ifdef DXMPERF
			PERFLOG_RXCONNECT(pConnector, (IPin *)this, NOERROR,
					  pmt);
#endif // DXMPERF
	...
	DbgLog((LOG_TRACE, CONNECT_TRACE_LEVEL,
		TEXT("Failed to set the media type or failed to complete the connection.")));
	m_Connected->Release();
	m_Connected = NULL;
...
HRESULT
CBaseOutputPin::DecideAllocator(IMemInputPin *pPin,
				__deref_out IMemAllocator **ppAlloc)
{
	HRESULT hr = NOERROR;
	*ppAlloc = NULL;
				  ...
				  DWORD dwFlags)
{
	if (m_pAllocator != NULL) {
		return m_pAllocator->GetBuffer(ppSample, pStartTime, pEndTime,
					       dwFlags);
	} else {
		return E_NOINTERFACE;
	}
...
#ifdef DXMPERF
	PERFLOG_DELIVER(m_pName ? m_pName : L"CBaseOutputPin", (IPin *)this,
			(IPin *)m_pInputPin, pSample, &m_mt);
#endif // DXMPERF
...
HRESULT
CBaseOutputPin::DeliverNewSegment(REFERENCE_TIME tStart, REFERENCE_TIME tStop,
				  double dRate)
{
	if (m_Connected == NULL) {
		return VFW_E_NOT_CONNECTED;
...
#ifdef UNICODE
CBaseInputPin::CBaseInputPin(__in LPCSTR pObjectName, __in CBaseFilter *pFilter,
			     __in CCritSec *pLock, __inout HRESULT *phr,
...
#ifdef DXMPERF
	PERFLOG_RECEIVE(m_pName ? m_pName : L"CBaseInputPin",
			(IPin *)m_Connected, (IPin *)this, pSample, &m_mt);
#endif // DXMPERF

	/* Check for IMediaSample2 */
	IMediaSample2 *pSample2;
	if (SUCCEEDED(pSample->QueryInterface(IID_IMediaSample2,
					      (void **)&pSample2))) {
		hr = pSample2->GetProperties(sizeof(m_SampleProps),
					     (PBYTE)&m_SampleProps);
		pSample2->Release();
		if (FAILED(hr)) {
			return hr;
...
STDMETHODIMP
CBaseInputPin::ReceiveMultiple(__in_ecount(nSamples) IMediaSample **pSamples,
...
CBaseInputPin::PassNotify(Quality &q)
{
	...
	DbgLog((LOG_TRACE, 3,
		TEXT("Passing Quality notification through transform")));
	if (m_pQSink != NULL) {
		...
		if (m_Connected) {
			m_Connected->QueryInterface(IID_IQualityControl,
						    (void **)&pIQC);
...
{
	if (riid == IID_IMediaSample || riid == IID_IMediaSample2 ||
	    riid == IID_IUnknown) {
...
CMediaSample::Release()
{
	...
	DbgLog((LOG_MEMORY, 3, TEXT("    Unknown %X ref-- = %d"), this,
		m_cRef));
...
STDMETHODIMP
CMediaSample::SetTime(__in_opt REFERENCE_TIME *pTimeStart,
...
STDMETHODIMP
CMediaSample::SetMediaTime(__in_opt LONGLONG *pTimeStart,
					    ...
					    const BYTE *pbProperties)
{
	...
	if (CONTAINS_FIELD(AM_SAMPLE2_PROPERTIES, cbData, cbProperties)) {
		...
		if (CONTAINS_FIELD(AM_SAMPLE2_PROPERTIES, dwSampleFlags,
				   cbProperties)) {
			/*  Check the flags */
            ...
            */
			if ((pProps->dwSampleFlags & AM_SAMPLE_TIMEVALID) &&
			    ...
			    !CONTAINS_FIELD(AM_SAMPLE2_PROPERTIES, tStop,
					    cbProperties)) {
				return E_INVALIDARG;
			}
		}
		/*  NB - can't SET the pointer or size */
		if (CONTAINS_FIELD(AM_SAMPLE2_PROPERTIES, pbBuffer,
				   cbProperties)) {

			/*  Check pbBuffer */
			if (pProps->pbBuffer != 0 &&
			    pProps->pbBuffer != m_pBuffer) {
				return E_INVALIDARG;
			}
		}
		if (CONTAINS_FIELD(AM_SAMPLE2_PROPERTIES, cbBuffer,
				   cbProperties)) {

			/*  Check cbBuffer */
			if (pProps->cbBuffer != 0 &&
		...
		if (CONTAINS_FIELD(AM_SAMPLE2_PROPERTIES, cbBuffer,
				   cbProperties) &&
		    CONTAINS_FIELD(AM_SAMPLE2_PROPERTIES, lActual,
				   cbProperties)) {
		...
		if (CONTAINS_FIELD(AM_SAMPLE2_PROPERTIES, pMediaType,
				   cbProperties)) {
			...
			if (pProps->dwSampleFlags & AM_SAMPLE_TYPECHANGED) {
				...
				pMediaType =
					CreateMediaType(pProps->pMediaType);
				if (pMediaType == NULL) {
					return E_OUTOFMEMORY;
				}
			}
		...
		if (CONTAINS_FIELD(AM_SAMPLE2_PROPERTIES, dwStreamId,
				   cbProperties)) {
		...
		if (CONTAINS_FIELD(AM_SAMPLE2_PROPERTIES, dwSampleFlags,
			...
			if (CONTAINS_FIELD(AM_SAMPLE2_PROPERTIES,
					   dwTypeSpecificFlags, cbProperties)) {
				m_dwTypeSpecificFlags =
					pProps->dwTypeSpecificFlags;
			}
		}

		if (CONTAINS_FIELD(AM_SAMPLE2_PROPERTIES, lActual,
				   cbProperties)) {
			/*  Set lActual */
			m_lActual = pProps->lActual;
		}

		if (CONTAINS_FIELD(AM_SAMPLE2_PROPERTIES, tStop,
				   cbProperties)) {

			/*  Set the times */
			m_End = pProps->tStop;
		}
		if (CONTAINS_FIELD(AM_SAMPLE2_PROPERTIES, tStart,
				   cbProperties)) {
...
#ifdef UNICODE
CDynamicOutputPin::CDynamicOutputPin(__in_opt LPCSTR pObjectName,
				     __in CBaseFilter *pFilter,
				     __in CCritSec *pLock, __inout HRESULT *phr,
				     __in_opt LPCWSTR pName)
	: CBaseOutputPin(pObjectName, pFilter, pLock, phr, pName),
...
{
	m_hUnblockOutputPinEvent = ::CreateEvent(
		NULL,  // The event will have the default security descriptor.
...
STDMETHODIMP
CDynamicOutputPin::NonDelegatingQueryInterface(REFIID riid,
					       __deref_out void **ppv)
{
...
{
	HANDLE hNotifyCallerPinBlockedEvent = ::CreateEvent(
		NULL,  // The event will have the default security attributes.
...
HRESULT CDynamicOutputPin::AsynchronousBlockOutputPin(
...
HRESULT CDynamicOutputPin::StartUsingOutputPin(void)
{
	...
	while (BLOCKED == m_BlockState) {
		...
		HANDLE ahWaitEvents[] = {m_hUnblockOutputPinEvent,
					 m_hStopEvent};
		DWORD dwNumWaitEvents = sizeof(ahWaitEvents) / sizeof(HANDLE);

		DWORD dwReturnValue = ::WaitForMultipleObjects(
			dwNumWaitEvents, ahWaitEvents, FALSE, INFINITE);
...
void CDynamicOutputPin::StopUsingOutputPin(void)
{
	...
	if ((m_dwNumOutstandingOutputPinUsers == 0) &&
	    (NOT_BLOCKED != m_BlockState)) {
		BlockOutputPin();
	}
	...
	if ((NULL == m_hStopEvent) || (NULL == m_pGraphConfig)) {
		DbgBreak(ERROR
			 : CDynamicOutputPin::Active()
					   failed because m_pGraphConfig and
				   m_hStopEvent were not initialized
					   .Call SetConfigInfo()
						   to initialize them.);
		return E_FAIL;
	}
...
HRESULT CDynamicOutputPin::ChangeMediaTypeHelper(const CMediaType *pmt)
{
	...
	if (NULL != m_pInputPin) {
		...
		hr = m_pInputPin->NotifyAllocator(m_pAllocator,
...
HRESULT CDynamicOutputPin::DynamicReconnect(const CMediaType *pmt)
{
	...
	HRESULT hr = m_pGraphConfig->Reconnect(
		this, NULL, pmt, NULL, m_hStopEvent,
...
void CDynamicOutputPin::AssertValid(void)
{
	...
	ASSERT((NOT_BLOCKED == m_BlockState) || (PENDING == m_BlockState) ||
	       (BLOCKED == m_BlockState));

	// m_hNotifyCallerPinBlockedEvent is only needed when a block operation cannot complete
	// immediately.
	ASSERT(((NULL == m_hNotifyCallerPinBlockedEvent) &&
		(PENDING != m_BlockState)) ||
	       ((NULL != m_hNotifyCallerPinBlockedEvent) &&
		(PENDING == m_BlockState)));
	...
	ASSERT(((0 != m_dwNumOutstandingOutputPinUsers) &&
		(BLOCKED != m_BlockState)) ||
	       ((0 == m_dwNumOutstandingOutputPinUsers) &&
		(NOT_BLOCKED != m_BlockState)) ||
	       ((0 == m_dwNumOutstandingOutputPinUsers) &&
		(NOT_BLOCKED == m_BlockState)));
}
...
CBaseAllocator::CBaseAllocator(__in_opt LPCTSTR pName,
			       ...
			       BOOL bEvent, BOOL fEnableReleaseCallback)
	: CUnknown(pName, pUnk),
	  m_lAllocated(0),
	  m_bChanged(FALSE),
	  m_bCommitted(FALSE),
...
STDMETHODIMP
CBaseAllocator::SetProperties(__in ALLOCATOR_PROPERTIES *pRequest,
			      __out ALLOCATOR_PROPERTIES *pActual)
{
	...
	if (pRequest->cbAlign != 1) {
		DbgLog((LOG_ERROR, 2,
			TEXT("Alignment requested was 0x%x, not 1"),
			pRequest->cbAlign));
...
void CBaseAllocator::CSampleList::Remove(__inout CMediaSample *pSample)
{
	...
	for (pSearch = &m_List; *pSearch != NULL;
...
CMemAllocator::CMemAllocator(__in_opt LPCTSTR pName, __inout_opt LPUNKNOWN pUnk,
			     __inout HRESULT *phr)
	: CBaseAllocator(pName, pUnk, phr, TRUE, TRUE), m_pBuffer(NULL)
{
}

#ifdef UNICODE
CMemAllocator::CMemAllocator(__in_opt LPCSTR pName, __inout_opt LPUNKNOWN pUnk,
			     __inout HRESULT *phr)
	: CBaseAllocator(pName, pUnk, phr, TRUE, TRUE), m_pBuffer(NULL)
{
}
...
/* This sets the size and count of the required samples. The memory isn't
   ...
   to change the buffering, the memory will be released in Commit() */
STDMETHODIMP
CMemAllocator::SetProperties(__in ALLOCATOR_PROPERTIES *pRequest,
			     __out ALLOCATOR_PROPERTIES *pActual)
{
	CheckPointer(pActual, E_POINTER);
	ValidateReadWritePtr(pActual, sizeof(ALLOCATOR_PROPERTIES));
	...
	if ((-pRequest->cbAlign & pRequest->cbAlign) != pRequest->cbAlign) {
		DbgLog((LOG_ERROR, 1,
			TEXT("Alignment requested 0x%x not a power of 2!"),
			pRequest->cbAlign));
	}
	/*  Check the alignment requested */
	if (pRequest->cbAlign == 0 ||
		...
		DbgLog((LOG_ERROR, 1,
			TEXT("Invalid alignment 0x%x requested - granularity = 0x%x"),
    ...
    */
	...
	for (; m_lAllocated < m_lCount; m_lAllocated++, pNext += lAlignedSize) {
		...
		pSample = new CMediaSample(
			NAME("Default memory media sample"), this, &hr,
		...
		if (SUCCEEDED(hr)) {
			...
			for (UINT m1 = 0; m1 < psetupdata->nPins; m1++) {
				...
				if (SUCCEEDED(hr)) {
					...
					//
					DbgLog((LOG_TRACE, 3,
						TEXT("= = register filter pin types")));
					for (UINT m2 = 0;
					     m2 <
					     psetupdata->lpPin[m1].nMediaTypes;
					     m2++) {
						hr = pIFM->RegisterPinType(
							*(psetupdata->clsID),
							psetupdata->lpPin[m1]
								.strName,
							*(psetupdata->lpPin[m1]
