...
// the derived class.
CSource::CSource(__in_opt LPCTSTR pName, __inout_opt LPUNKNOWN lpunk,
		 CLSID clsid)
...
#ifdef UNICODE
CSource::CSource(__in_opt LPCSTR pName, __inout_opt LPUNKNOWN lpunk,
		 CLSID clsid)
	: CBaseFilter(pName, lpunk, &m_cStateLock, clsid),
	  m_iPins(0),
	  m_paStreams(NULL)
...
CSourceStream::CSourceStream(__in_opt LPCTSTR pObjectName, __inout HRESULT *phr,
			     __inout CSource *ps, __in_opt LPCWSTR pPinName)
...
#ifdef UNICODE
CSourceStream::CSourceStream(__in_opt LPCSTR pObjectName, __inout HRESULT *phr,
			     __inout CSource *ps, __in_opt LPCWSTR pPinName)
	: CBaseOutputPin(pObjectName, ps, ps->pStateLock(), phr, pPinName),
	  m_pFilter(ps)
{
...
DWORD CSourceStream::ThreadProc(void)
{
	...
	do {
		...
		if (com != CMD_INIT) {
			DbgLog((LOG_ERROR, 1,
				TEXT("Thread expected init command")));
			Reply((DWORD)E_UNEXPECTED);
		}
	} while (com != CMD_INIT);

	DbgLog((LOG_TRACE, 1,
		TEXT("CSourceStream worker thread initializing")));

	hr = OnThreadCreate(); // perform set up tasks
	if (FAILED(hr)) {
		DbgLog((LOG_ERROR, 1,
			TEXT("CSourceStream::OnThreadCreate failed. Aborting thread.")));
		OnThreadDestroy();
	...
	do {
		...
		default:
			DbgLog((LOG_ERROR, 1,
				TEXT("Unknown command %d received!"), cmd));
	...
	if (FAILED(hr)) {
		DbgLog((LOG_ERROR, 1,
			TEXT("CSourceStream::OnThreadDestroy failed. Exiting thread.")));
		return 1;
	}
	...
	do {
		while (!CheckRequest(&com)) {
			...
			if (hr == S_OK) {
				...
				if (hr != S_OK) {
					DbgLog((LOG_TRACE, 2,
						TEXT("Deliver() returned %08x; stopping"),
						hr));
					return S_OK;
				}
				...
				pSample->Release();
				DbgLog((LOG_ERROR, 1,
					TEXT("Error %08lX from FillBuffer!!!"),
					hr));
				DeliverEndOfStream();
