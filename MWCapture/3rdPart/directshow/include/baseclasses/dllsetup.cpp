...
EliminateSubKey(HKEY hkey, LPCTSTR strSubKey)
{
	...
	ASSERT(lreturn == ERROR_SUCCESS || lreturn == ERROR_FILE_NOT_FOUND ||
	...
	if (ERROR_SUCCESS == lreturn) {
		...
		for (;;) {
			...
			lreturn = RegEnumKeyEx(hk, 0, Buffer, &dw, NULL, NULL,
					       NULL, &ft);
...
STDAPI
AMovieSetupRegisterServer(CLSID clsServer, LPCWSTR szDescription,
			  LPCWSTR szFileName,
			  ...
			  LPCWSTR szServerType = L"InprocServer32")
{
	...
	//
	HKEY hkey;
	(void)StringCchPrintf(achTemp, NUMELMS(achTemp), TEXT("CLSID\\%ls"),
			      szCLSID);
	...
	(void)StringCchPrintf(achTemp, NUMELMS(achTemp), TEXT("%ls"),
			      szDescription);
	lreturn = RegSetValue(hkey, (LPCTSTR)NULL, REG_SZ, achTemp,
			      sizeof(achTemp));
	if (ERROR_SUCCESS != lreturn) {
		RegCloseKey(hkey);
		return AmHresultFromWin32(lreturn);
	...
	(void)StringCchPrintf(achTemp, NUMELMS(achTemp), TEXT("%ls"),
			      szFileName);
	lreturn = RegSetValue(hsubkey, (LPCTSTR)NULL, REG_SZ, (LPCTSTR)achTemp,
	...
	(void)StringCchPrintf(achTemp, NUMELMS(achTemp), TEXT("%ls"),
			      szThreadingModel);
	lreturn = RegSetValueEx(hsubkey, TEXT("ThreadingModel"), 0L, REG_SZ,
				(CONST BYTE *)achTemp,
				sizeof(TCHAR) * (lstrlen(achTemp) + 1));
...
AMovieSetupUnregisterServer(CLSID clsServer)
{
	...
	(void)StringCchPrintf(achBuffer, NUMELMS(achBuffer), TEXT("CLSID\\%ls"),
			      szCLSID);
...
AMovieDllRegisterServer2(BOOL bRegister)
{
	...
	if (SUCCEEDED(hr)) {
		...
		if (FAILED(hr)) {
			DbgLog((LOG_TRACE, 2,
				TEXT("- trying IFilterMapper instead")));
		...
		if (SUCCEEDED(hr)) {
			...
			for (int i = 0; i < g_cTemplates; i++) {
				...
				if (NULL != pT->m_pAMovieSetup_Filter) {
					...
					if (pIFM2) {
						hr = AMovieSetupRegisterFilter2(
							pT->m_pAMovieSetup_Filter,
							pIFM2, bRegister);
					} else {
						hr = AMovieSetupRegisterFilter(
							pT->m_pAMovieSetup_Filter,
							pIFM, bRegister);
					}
				}
...
AMovieDllRegisterServer(void)
{
	...
	for (int i = 0; i < g_cTemplates; i++) {
		...
		hr = AMovieSetupRegisterServer(
			*(pT->m_ClsID), (LPCWSTR)pT->m_Name, achFileName);
