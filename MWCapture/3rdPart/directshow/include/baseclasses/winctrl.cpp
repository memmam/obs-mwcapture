...
BOOL WINAPI PossiblyEatMessage(HWND hwndDrain, UINT uMsg, WPARAM wParam,
			       LPARAM lParam)
{
...
STDMETHODIMP CBaseControlWindow::put_Caption(__in BSTR strCaption)
{
	...
	WideCharToMultiByte(CP_ACP, 0, strCaption, -1, Caption, CAPTION, NULL,
			    NULL);
...
STDMETHODIMP CBaseControlWindow::put_WindowStyleEx(long WindowStyleEx)
{
	...
	if (GetWindowLong(m_hwnd, GWL_EXSTYLE) & WS_EX_TOPMOST) {
		if ((WindowStyleEx & WS_EX_TOPMOST) == 0) {
			SendMessage(m_hwnd, m_ShowStageTop, (WPARAM)FALSE,
				    (LPARAM)0);
		}
	}
...
STDMETHODIMP CBaseControlWindow::put_WindowStyle(long WindowStyle)
{
	...
	if ((WindowStyle & WS_DISABLED) || (WindowStyle & WS_ICONIC) ||
	    (WindowStyle & WS_MAXIMIZE) || (WindowStyle & WS_MINIMIZE) ||
	    (WindowStyle & WS_HSCROLL) || (WindowStyle & WS_VSCROLL)) {
...
HRESULT CBaseControlWindow::DoSetWindowStyle(long Style, long WindowLong)
{
	...
	if (GetParent(m_hwnd)) {
		...
		MapWindowPoints(HWND_DESKTOP, GetParent(m_hwnd),
...
STDMETHODIMP
CBaseControlWindow::SetWindowPosition(long Left, long Top, long Width,
				      long Height)
{
	CheckConnected(m_pPin, VFW_E_NOT_CONNECTED);
	BOOL bSuccess;
...
STDMETHODIMP
CBaseControlWindow::GetWindowPosition(__out long *pLeft, __out long *pTop,
...
STDMETHODIMP
CBaseControlWindow::GetRestorePosition(__out long *pLeft, __out long *pTop,
				       __out long *pWidth, __out long *pHeight)
{
	// Should check the pointers are not NULL
	...
	if (SystemParametersInfo(SPI_GETWORKAREA, 0, &WorkArea, FALSE) ==
	    TRUE) {
...
STDMETHODIMP
CBaseControlWindow::GetMinIdealImageSize(__out long *pWidth,
					 __out long *pHeight)
{
	CheckPointer(pWidth, E_POINTER);
	CheckPointer(pHeight, E_POINTER);
...
STDMETHODIMP
CBaseControlWindow::GetMaxIdealImageSize(__out long *pWidth,
					 __out long *pHeight)
{
	CheckPointer(pWidth, E_POINTER);
	CheckPointer(pHeight, E_POINTER);
...
STDMETHODIMP
CBaseControlVideo::get_AvgTimePerFrame(__out REFTIME *pAvgTimePerFrame)
{
...
STDMETHODIMP CBaseControlVideo::GetVideoPaletteEntries(
...
STDMETHODIMP
CBaseControlVideo::SetSourcePosition(long Left, long Top, long Width,
				     long Height)
{
	CheckConnected(m_pPin, VFW_E_NOT_CONNECTED);
	CAutoLock cInterfaceLock(m_pInterfaceLock);
...
STDMETHODIMP
CBaseControlVideo::GetSourcePosition(__out long *pLeft, __out long *pTop,
				     __out long *pWidth, __out long *pHeight)
{
...
STDMETHODIMP
CBaseControlVideo::SetDestinationPosition(long Left, long Top, long Width,
					  long Height)
{
	CheckConnected(m_pPin, VFW_E_NOT_CONNECTED);
	CAutoLock cInterfaceLock(m_pInterfaceLock);
...
STDMETHODIMP
CBaseControlVideo::GetDestinationPosition(__out long *pLeft, __out long *pTop,
					  __out long *pWidth,
					  __out long *pHeight)
{
	// Should check the pointers are not NULL
...
STDMETHODIMP
CBaseControlVideo::get_DestinationLeft(__out long *pDestinationLeft)
{
...
STDMETHODIMP
CBaseControlVideo::get_DestinationWidth(__out long *pDestinationWidth)
{
...
STDMETHODIMP
CBaseControlVideo::get_DestinationHeight(__out long *pDestinationHeight)
{
...
HRESULT CBaseControlVideo::CopyImage(
	IMediaSample *pMediaSample, __in VIDEOINFOHEADER *pVideoInfo,
	__inout long *pBufferSize,
	...
	__in RECT *pSourceRect)
{
	...
	if (pMediaSample == NULL || pSourceRect == NULL || pVideoInfo == NULL ||
	    pVideoImage == NULL || pBufferSize == NULL) {
	...
	LONG ScanLine =
		(pVideoInfo->bmiHeader.biBitCount / 8) * WIDTH(pSourceRect);
	...
	pCurrentImage +=
		pSourceRect->left * (pVideoInfo->bmiHeader.biBitCount / 8);
