...
void CBaseRenderer::DisplayRendererState()
{
	...
	DbgLog((LOG_TIMING, 1, TEXT("Current media sample %x"),
		m_pMediaSample));
	...
	DbgLog((LOG_TIMING, 1, TEXT("End of stream timer %x"),
		m_EndOfStreamTimer));

	DbgLog((LOG_TIMING, 1, TEXT("Deliver time %s"),
	...
	DbgLog((LOG_TIMING, 1, TEXT("Last run time %s"),
	...
	DbgLog((LOG_TIMING, 1, TEXT("Clock time %s"),
		CDisp((LONGLONG)CurrentTime.m_time)));

	DbgLog((LOG_TIMING, 1, TEXT("Time difference %dms"),
		Offset.Millisecs()));
	...
	m_pMediaSample->GetTime((REFERENCE_TIME *)&StartTime,
				(REFERENCE_TIME *)&EndTime);
	DbgLog((LOG_TIMING, 1,
		TEXT("Next sample stream times (Start %d End %d ms)"),
...
HRESULT CBaseRenderer::WaitForRenderTime()
{
	...
	while (Result == WAIT_TIMEOUT) {
		Result = WaitForMultipleObjects(2, WaitObjects, FALSE,
						RENDER_TIMEOUT);
...
HRESULT CBaseRenderer::BreakConnect()
{
	...
	if (m_State != State_Stopped &&
	    !m_pInputPin->CanReconnectWhenActive()) {
...
HRESULT CBaseRenderer::SendEndOfStream()
{
	...
	m_EndOfStreamTimer =
		CompatibleTimeSetEvent((UINT)Delay,        // Period of timer
...
CRendererInputPin::CRendererInputPin(__inout CBaseRenderer *pRenderer,
	...
	: CBaseInputPin(NAME("Renderer pin"), pRenderer,
...
STDMETHODIMP CRendererInputPin::Receive(IMediaSample *pSample)
{
	...
	if (FAILED(hr)) {
...

		{
			...
			// handles the errors.
			if (!IsStopped() && !IsFlushing() &&
			    !m_pRenderer->m_bAbort && !m_bRunTimeError) {
				...
				{
					CAutoLock alRendererLock(
						&m_pRenderer->m_RendererLock);
...
void CBaseVideoRenderer::RecordFrameLateness(int trLate, int trFrame)
{
	...
	if (m_cFramesDrawn > 2) {
		int tFrame =
			trFrame / 10000; // convert to mSec else it overflows
...
void CBaseVideoRenderer::OnRenderEnd(IMediaSample *pMediaSample)
{
	...
	int tr =
		(timeGetTime() - m_tRenderStart) * 10000; // convert mSec->UNITS
	if (tr < m_trRenderAvg * 2 || tr < 2 * m_trRenderLast) {
		// DO_MOVING_AVG(m_trRenderAvg, tr);
		m_trRenderAvg =
			(tr + (AVGPERIOD - 1) * m_trRenderAvg) / AVGPERIOD;
	}
	m_trRenderLast = tr;
	ThrottleWait();
} // OnRenderEnd
					...
					REFERENCE_TIME trRealStream)
{
	...
	if (m_trFrameAvg < 0) {
		...
		if (m_trWaitAvg >= m_trFrameAvg) {
			...
			if (m_trFrameAvg + 20000 > m_trWaitAvg) {
				q.Proportion = 1000 * (m_trFrameAvg /
						       (m_trFrameAvg + 20000 -
							m_trWaitAvg));
			} else {
	...
	if (m_pQSink == NULL) {
		...
		hr = pOutputPin->QueryInterface(IID_IQualityControl,
						(void **)&pQC);
		if (SUCCEEDED(hr)) {
			m_pQSink = pQC;
		}
...
HRESULT
CBaseVideoRenderer::ShouldDrawSampleNow(IMediaSample *pMediaSample,
					...
					__inout REFERENCE_TIME *ptrEnd)
{
	...
	MSR_INTEGER(m_idTimeStamp,
		    (int)((*ptrStart) >> 32));        // high order 32 bits
	...
	BOOL bJustDroppedFrame =
		(m_bSupplierHandlingQuality
		 ...
		 && (S_OK ==
		     pMediaSample->IsDiscontinuity()) // he just dropped one
		 ) ||
	...
	if (trLate > 0) {
		...
		m_trEarliness =
			trLate; // Things have slipped of their own accord
	} else {
		m_trEarliness =
			m_trEarliness - m_trEarliness / 8; // graceful slide
	}
	...
	int trFrame;
	{
		REFERENCE_TIME tr = trRealStream -
				    m_trLastDraw; // Cd be large - 4 min pause!
		if (tr > 10000000) {
	...
	if (
		...
		|| (m_bSupplierHandlingQuality ? (trLate <= trDuration * 4)
		...
		if (bPlayASAP) {
			...
			m_trWaitAvg =
				(m_trWaitAvg * (AVGPERIOD - 1)) / AVGPERIOD;

			// Assume that we draw it immediately.  Update inter-frame stats
			m_trFrameAvg =
				(trFrame + m_trFrameAvg * (AVGPERIOD - 1)) /
				AVGPERIOD;
#ifndef PERF
			...
			if (m_trEarliness > trLate) {
				m_trEarliness =
					trLate; // if we are actually early, this is neg
			}
			...
			Result =
				Delay <= 0
					? S_OK
					: S_FALSE; // OK = draw now, FALSE = wait
			...
			if (Delay > 0) {
				...
				iAccuracy = TimeDiff(*ptrStart -
						     m_trRememberStampForPerf);
			} else {
				// Report lateness based on playing it *now*.
				iAccuracy =
					trTrueLate; // trRealStream-RememberStampForPerf;
			}
...
STDMETHODIMP
CBaseVideoRenderer::get_FramesDroppedInRenderer(__out int *pcFramesDropped)
{
...
int isqrt(int x)
{
	...
	if (x > 0x40000000) {
		...
		while (s * s <
		       x) {        // loop cannot possible go more than 31 times
...
//  statistics
//
HRESULT CBaseVideoRenderer::GetStdDev(int nSamples, __out int *piResult,
				      LONGLONG llSumSq, LONGLONG iTot)
{
...
STDMETHODIMP CBaseVideoRenderer::get_Jitter(__out int *piJitter)
{
	...
	return GetStdDev(m_cFramesDrawn - 2, piJitter, m_iSumSqFrameTime,
			 m_iSumFrameTime);
} // get_Jitter
...
STDMETHODIMP
CBaseVideoRenderer::NonDelegatingQueryInterface(REFIID riid,
						__deref_out VOID **ppv)
{
...
STDMETHODIMP
CBaseVideoRenderer::JoinFilterGraph(__inout_opt IFilterGraph *pGraph,
