...
const LPCTSTR pKeyNames[] = {
	...
	TEXT("CUSTOM1"), TEXT("CUSTOM2"), TEXT("CUSTOM3"),
	TEXT("CUSTOM4"), TEXT("CUSTOM5")};
	...
	for (lKeyPos = 0; lKeyPos < iMAXLEVELS; lKeyPos++) {
		...
		if (lReturn != ERROR_SUCCESS || dwKeyType != REG_DWORD) {
			...
			if (lReturn != ERROR_SUCCESS) {
				DbgLog((LOG_ERROR, 1,
					TEXT("Could not create subkey %s"),
					pKeyNames[lKeyPos]));
				dwKeyValue = 0;
			}
		}
...
#ifdef UNICODE
		CHAR szDest[2048];
		WideCharToMultiByte(CP_ACP, 0, psz, -1, szDest, NUMELMS(szDest),
				    0, 0);
...
void WINAPI DbgInitLogTo(HKEY hKey)
{
	...
	if (szFile[0] != 0) {
		if (!lstrcmpi(szFile, TEXT("Console"))) {
		...
		} else if (szFile[0] && lstrcmpi(szFile, TEXT("Debug")) &&
			...
			if (INVALID_HANDLE_VALUE == m_hOutput &&
				...
				if (SUCCEEDED(DbgUniqueProcessName(
					    szFile, uniqueName))) {
					m_hOutput = CreateFile(
						uniqueName, GENERIC_WRITE,
						FILE_SHARE_READ, NULL,
						OPEN_ALWAYS,
						FILE_ATTRIBUTE_NORMAL, NULL);
				}
...
void WINAPI DbgInitGlobalSettings(bool fTakeMax)
{
	...
	(void)StringCchPrintf(szInfo, NUMELMS(szInfo), TEXT("%s\\%s"), pBaseKey,
			      pGlobalKey);
	...
	if (lReturn != ERROR_SUCCESS) {
		...
		if (lReturn != ERROR_SUCCESS) {
			DbgLog((LOG_ERROR, 1,
				TEXT("Could not access GLOBAL module key")));
		}
		return;
	}
...
void WINAPI DbgInitModuleSettings(bool fTakeMax)
{
	...
	if (lReturn != ERROR_SUCCESS) {
		...
		if (lReturn != ERROR_SUCCESS) {
			DbgLog((LOG_ERROR, 1,
				TEXT("Could not access module key")));
		}
		return;
	}
...
INT MessageBoxOtherThread(HWND hwnd, LPCTSTR szTitle, LPCTSTR szMessage,
			  DWORD dwFlags)
{
...
void WINAPI DbgAssert(LPCTSTR pCondition, LPCTSTR pFileName, INT iLine)
{
	if (g_fUseKASSERT) {
		...
		(void)StringCchPrintf(
			szInfo, NUMELMS(szInfo),
			TEXT("%s \nAt line %d of %s\nContinue? (Cancel to debug)"),
			pCondition, iLine, pFileName);

		INT MsgId = MessageBoxOtherThread(
			NULL, szInfo, TEXT("ASSERT Failed"),
...
void WINAPI DbgBreakPoint(LPCTSTR pCondition, LPCTSTR pFileName, INT iLine)
{
	if (g_fUseKASSERT) {
		...
		INT MsgId = MessageBoxOtherThread(
			NULL, szInfo, TEXT("Hard coded break point"),
			MB_SYSTEMMODAL | MB_ICONHAND | MB_YESNOCANCEL |
				MB_SETFOREGROUND);
		switch (MsgId) {
		case IDNO: /* Kill the application */
			  ...
			  __format_string LPCTSTR szFormatString, ...)
{
	...
	HRESULT hr = StringCchVPrintf(szBreakPointMessage,
				      NUMELMS(szBreakPointMessage),
				      szFormatString, va);
...
BOOL WINAPI DbgCheckModuleLevel(DWORD Type, DWORD Level)
{
	...
	if ((Type & ((1 << iMAXLEVELS) - 1))) {
		...
		for (LONG lKeyPos = 0; lKeyPos < iMAXLEVELS; lKeyPos++) {
			if (Type & Mask) {
				if (Level <=
				    (m_Levels[lKeyPos] & ~LOG_FORCIBLY_SET)) {
					return TRUE;
				}
			}
			Mask <<= 1;
		       ...
		       ...)
{
	...
	(void)StringCchPrintf(szInfo, NUMELMS(szInfo),
			      TEXT("%s(tid %x) %8d : "), m_ModuleName,
			      GetCurrentThreadId(),
			      timeGetTime() - dwTimeOffset);

	CHAR szInfoA[2000];
	WideCharToMultiByte(CP_ACP, 0, szInfo, -1, szInfoA, NUMELMS(szInfoA), 0,
			    0);

	(void)StringCchVPrintfA(szInfoA + lstrlenA(szInfoA),
				NUMELMS(szInfoA) - lstrlenA(szInfoA), pFormat,
				va);
	(void)StringCchCatA(szInfoA, NUMELMS(szInfoA), "\r\n");

	WCHAR wszOutString[2000];
	MultiByteToWideChar(CP_ACP, 0, szInfoA, -1, wszOutString,
...
{
	DbgLog((LOG_ERROR, 0,
		TEXT("Assertion FAILED (%hs) at line %d in file %hs"),
		pCondition, iLine, pFileName));
	DebugBreak();
}
...
{
	DbgLog((LOG_ERROR, 0,
		TEXT("Assertion FAILED (%s) at line %d in file %s"), pCondition,
		iLine, pFileName));
	DebugBreak();
}
				       ...
				       LPCWSTR wszObjectName)
{
	...
	if (wszObjectName) {
		DbgLog((LOG_MEMORY, 2,
			TEXT("Object created   %d (%ls) %d Active"),
			pObject->m_dwCookie, wszObjectName, m_dwObjectCount));
	} else {
		DbgLog((LOG_MEMORY, 2,
			TEXT("Object created   %d (%hs) %d Active"),
...
BOOL WINAPI DbgRegisterObjectDestruction(DWORD dwCookie)
{
	...
	if (pObject->m_wszName) {
		DbgLog((LOG_MEMORY, 2,
			TEXT("Object destroyed %d (%ls) %d Active"),
			pObject->m_dwCookie, pObject->m_wszName,
			m_dwObjectCount));
	} else {
		DbgLog((LOG_MEMORY, 2,
			TEXT("Object destroyed %d (%hs) %d Active"),
			pObject->m_dwCookie, pObject->m_szName,
			m_dwObjectCount));
	}
				       ...
				       BOOL bWaitAll)
{
	DWORD dwWaitResult;
	do {
		dwWaitResult = WaitForMultipleObjects(nCount, lpHandles,
						      bWaitAll, dwWaitTimeout);
		ASSERT((DWORD)(dwWaitResult - WAIT_OBJECT_0) <
...
CDisp::CDisp(LONGLONG ll, int Format)
{
	...
	case CDISP_HEX:
	default:
		(void)StringCchPrintf(m_String, NUMELMS(m_String),
				      TEXT("0x%X%8.8X"), li.HighPart,
				      li.LowPart);
	}
};
...
CDisp::CDisp(CRefTime llTime)
{
	...
	if (llTime >= llDiv) {
		(void)StringCchPrintf(m_String + lstrlen(m_String),
				      NUMELMS(m_String) - lstrlen(m_String),
				      TEXT("%d days "), (LONG)(llTime / llDiv));
		llTime = llTime % llDiv;
	}
	...
	if (llTime >= llDiv) {
		(void)StringCchPrintf(m_String + lstrlen(m_String),
				      NUMELMS(m_String) - lstrlen(m_String),
				      TEXT("%d mins "), (LONG)(llTime / llDiv));
		llTime = llTime % llDiv;
	}
	(void)StringCchPrintf(m_String + lstrlen(m_String),
			      NUMELMS(m_String) - lstrlen(m_String),
			      TEXT("%d.%3.3d sec"), (LONG)llTime / 10000000,
...
CDisp::CDisp(IPin *pPin)
{
...
#ifndef UNICODE
		WideCharToMultiByte(GetACP(), 0, pi.achName,
				    lstrlenW(pi.achName) + 1, str, MAX_PIN_NAME,
				    NULL, NULL);
#else
	...
	(void)StringCchPrintf(m_pString, lstrlen(str) + 64, TEXT("%hs(%s)"),
			      GuidNames[clsid], str);
}
...
#else
				(void)StringCchPrintf(m_pString, len, "%S",
						      fi.achName);
#endif
			}
		}
...
{
	(void)StringCchPrintf(m_String, NUMELMS(m_String), TEXT("%d.%03d"),
			      (int)d, (int)((d - (int)d) * 1000));
}
...
void WINAPI DisplayType(LPCTSTR label, const AM_MEDIA_TYPE *pmtIn)
{
	...
	DbgLog((LOG_TRACE, 2, TEXT("%s  M type %hs  S type %hs"), label,
		GuidNames[pmtIn->majortype], GuidNames[pmtIn->subtype]));
	DbgLog((LOG_TRACE, 5, TEXT("Subtype description %s"),
	...
	if (pmtIn->bFixedSizeSamples) {
		DbgLog((LOG_TRACE, 5, TEXT("Sample size %d"),
			pmtIn->lSampleSize));
	} else {
	...
	if (pmtIn->formattype == FORMAT_VideoInfo) {
		...
		VIDEOINFOHEADER *pVideoInfo =
			(VIDEOINFOHEADER *)pmtIn->pbFormat;
	...
	if (pmtIn->formattype == FORMAT_VideoInfo2) {
		...
		VIDEOINFOHEADER2 *pVideoInfo2 =
			(VIDEOINFOHEADER2 *)pmtIn->pbFormat;
		...
		if ((pmtIn->subtype != MEDIASUBTYPE_MPEG1Packet) &&
			...
			DbgLog((LOG_TRACE, 2, TEXT("wFormatTag %u"),
				pwfx->wFormatTag));
			DbgLog((LOG_TRACE, 2, TEXT("nChannels %u"),
				pwfx->nChannels));
			DbgLog((LOG_TRACE, 2, TEXT("nSamplesPerSec %lu"),
				pwfx->nSamplesPerSec));
			DbgLog((LOG_TRACE, 2, TEXT("nAvgBytesPerSec %lu"),
				pwfx->nAvgBytesPerSec));
			DbgLog((LOG_TRACE, 2, TEXT("nBlockAlign %u"),
				pwfx->nBlockAlign));
			DbgLog((LOG_TRACE, 2, TEXT("wBitsPerSample %u"),
				pwfx->wBitsPerSample));

			/* PCM uses a WAVEFORMAT and does not have the extra size field */

			if (pmtIn->cbFormat >= sizeof(WAVEFORMATEX)) {
				DbgLog((LOG_TRACE, 2, TEXT("cbSize %u"),
					pwfx->cbSize));
			}
		} else {
		}
...
{
	DbgLog((LOG_TRACE, 5, TEXT("Size of BITMAPINFO structure %d"),
		pbmi->biSize));
	if (pbmi->biCompression < 256) {
		DbgLog((LOG_TRACE, 2, TEXT("%dx%dx%d bit  (%d)"), pbmi->biWidth,
			pbmi->biHeight, pbmi->biBitCount, pbmi->biCompression));
	} else {
		DbgLog((LOG_TRACE, 2, TEXT("%dx%dx%d bit '%4.4hs'"),
			pbmi->biWidth, pbmi->biHeight, pbmi->biBitCount,
			&pbmi->biCompression));
	}

	DbgLog((LOG_TRACE, 2, TEXT("Image size %d"), pbmi->biSizeImage));
	DbgLog((LOG_TRACE, 5, TEXT("Planes %d"), pbmi->biPlanes));
	DbgLog((LOG_TRACE, 5, TEXT("X Pels per metre %d"),
		pbmi->biXPelsPerMeter));
	DbgLog((LOG_TRACE, 5, TEXT("Y Pels per metre %d"),
...
void DisplayRECT(LPCTSTR szLabel, const RECT &rc)
{
	DbgLog((LOG_TRACE, 5, TEXT("%s (Left %d Top %d Right %d Bottom %d)"),
		szLabel, rc.left, rc.top, rc.right, rc.bottom));
}
