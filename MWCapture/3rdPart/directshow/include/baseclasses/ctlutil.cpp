...
typedef HRESULT(STDAPICALLTYPE *LPLOADTYPELIB)(
	const OLECHAR FAR *szFile, __deref_out ITypeLib FAR *FAR *pptlib);
...
STDMETHODIMP
CBaseDispatch::GetTypeInfo(REFIID riid, UINT itinfo, LCID lcid,
			   __deref_out ITypeInfo **pptinfo)
{
	...
	*pptinfo = NULL;
	...
	if (NULL == m_pti) {
		...
		hr = (*lpfnLoadRegTypeLib)(LIBID_QuartzTypeLib, 1,
					   0, // version 1.0
					   lcid, &ptlib);
...
STDMETHODIMP
CBaseDispatch::GetIDsOfNames(REFIID riid,
...
STDMETHODIMP
CMediaControl::GetTypeInfo(UINT itinfo, LCID lcid,
			   __deref_out ITypeInfo **pptinfo)
{
...
STDMETHODIMP
CMediaControl::GetIDsOfNames(REFIID riid,
			     __in_ecount(cNames) LPOLESTR *rgszNames,
			     UINT cNames, LCID lcid,
			     __out_ecount(cNames) DISPID *rgdispid)
{
	return m_basedisp.GetIDsOfNames(IID_IMediaControl, rgszNames, cNames,
					lcid, rgdispid);
}

STDMETHODIMP
CMediaControl::Invoke(DISPID dispidMember, REFIID riid, LCID lcid, WORD wFlags,
		      ...
		      __out_opt EXCEPINFO *pexcepinfo, __out_opt UINT *puArgErr)
{
	...
	hr = pti->Invoke((IMediaControl *)this, dispidMember, wFlags,
...
STDMETHODIMP
CMediaEvent::GetTypeInfo(UINT itinfo, LCID lcid,
			 __deref_out ITypeInfo **pptinfo)
{
...
STDMETHODIMP
CMediaEvent::GetIDsOfNames(REFIID riid, __in_ecount(cNames) LPOLESTR *rgszNames,
			   UINT cNames, LCID lcid,
			   __out_ecount(cNames) DISPID *rgdispid)
{
	return m_basedisp.GetIDsOfNames(IID_IMediaEvent, rgszNames, cNames,
					lcid, rgdispid);
}

STDMETHODIMP
CMediaEvent::Invoke(DISPID dispidMember, REFIID riid, LCID lcid, WORD wFlags,
		    ...
		    __out_opt EXCEPINFO *pexcepinfo, __out_opt UINT *puArgErr)
{
	...
	hr = pti->Invoke((IMediaEvent *)this, dispidMember, wFlags, pdispparams,
...
STDMETHODIMP
CMediaPosition::GetTypeInfo(UINT itinfo, LCID lcid,
			    __deref_out ITypeInfo **pptinfo)
{
	return m_basedisp.GetTypeInfo(IID_IMediaPosition, itinfo, lcid,
				      pptinfo);
}

STDMETHODIMP
CMediaPosition::GetIDsOfNames(REFIID riid,
			      __in_ecount(cNames) LPOLESTR *rgszNames,
			      UINT cNames, LCID lcid,
			      __out_ecount(cNames) DISPID *rgdispid)
{
	return m_basedisp.GetIDsOfNames(IID_IMediaPosition, rgszNames, cNames,
					lcid, rgdispid);
}

STDMETHODIMP
CMediaPosition::Invoke(DISPID dispidMember, REFIID riid, LCID lcid, WORD wFlags,
		       __in DISPPARAMS *pdispparams,
		       __out_opt VARIANT *pvarResult,
		       ...
		       __out_opt UINT *puArgErr)
{
	...
	hr = pti->Invoke((IMediaPosition *)this, dispidMember, wFlags,
			 pdispparams, pvarResult, pexcepinfo, puArgErr);
...
CPosPassThru::CPosPassThru(__in_opt LPCTSTR pName, __in_opt LPUNKNOWN pUnk,
			   __inout HRESULT *phr, IPin *pPin)
	: CMediaPosition(pName, pUnk), m_pPin(pPin)
{
	if (pPin == NULL) {
		*phr = E_POINTER;
		return;
				...
				__in_opt const GUID *pSourceFormat)
{
	...
	hr = pMS->ConvertTimeFormat(pTarget, pTargetFormat, Source,
				    pSourceFormat);
	pMS->Release();
	return hr;
}
...
STDMETHODIMP
CPosPassThru::SetPositions(__inout_opt LONGLONG *pCurrent, DWORD CurrentFlags,
...
STDMETHODIMP
CPosPassThru::GetPositions(__out_opt LONGLONG *pCurrent,
...
CPosPassThru::GetCurrentPosition(__out LONGLONG *pCurrent)
{
	...
	if (SUCCEEDED(hr))
		...
		hr = GetSeekingLongLong(&IMediaSeeking::GetCurrentPosition,
					pCurrent);
	return hr;
}
...
STDMETHODIMP
CPosPassThru::GetAvailable(__out_opt LONGLONG *pEarliest,
...
HRESULT
CRendererPosPassThru::GetMediaTime(__out LONGLONG *pStartTime,
				   __out_opt LONGLONG *pEndTime)
{
	...
	HRESULT hr = ConvertTimeFormat(pStartTime, 0, m_StartMedia,
				       &TIME_FORMAT_MEDIA_TIME);
	if (pEndTime && SUCCEEDED(hr)) {
		hr = ConvertTimeFormat(pEndTime, 0, m_EndMedia,
				       &TIME_FORMAT_MEDIA_TIME);
	}
	return hr;
}
...
CSourceSeeking::CSourceSeeking(__in_opt LPCTSTR pName, __in_opt LPUNKNOWN pUnk,
			       __inout HRESULT *phr, __in CCritSec *pLock)
	: CUnknown(pName, pUnk), m_pLock(pLock), m_rtStart((long)0)
{
	m_rtStop = _I64_MAX / 2;
					  ...
					  __in_opt const GUID *pSourceFormat)
{
	...
	if (pTargetFormat == 0 || *pTargetFormat == TIME_FORMAT_MEDIA_TIME) {
		if (pSourceFormat == 0 ||
		    *pSourceFormat == TIME_FORMAT_MEDIA_TIME) {
			*pTarget = Source;
			return S_OK;
		}
	}
...
CSourcePosition::CSourcePosition(__in_opt LPCTSTR pName,
				 __in_opt LPUNKNOWN pUnk, __inout HRESULT *phr,
				 __in CCritSec *pLock)
...
STDMETHODIMP
CBasicAudio::GetTypeInfo(UINT itinfo, LCID lcid,
			 __deref_out ITypeInfo **pptinfo)
{
...
STDMETHODIMP
CBasicAudio::GetIDsOfNames(REFIID riid, __in_ecount(cNames) LPOLESTR *rgszNames,
			   UINT cNames, LCID lcid,
			   __out_ecount(cNames) DISPID *rgdispid)
{
	return m_basedisp.GetIDsOfNames(IID_IBasicAudio, rgszNames, cNames,
					lcid, rgdispid);
}

STDMETHODIMP
CBasicAudio::Invoke(DISPID dispidMember, REFIID riid, LCID lcid, WORD wFlags,
		    __in DISPPARAMS *pdispparams, __out_opt VARIANT *pvarResult,
		    __out_opt EXCEPINFO *pexcepinfo, __out_opt UINT *puArgErr)
{
	// this parameter is a dead leftover from an earlier interface
	...
	hr = pti->Invoke((IBasicAudio *)this, dispidMember, wFlags, pdispparams,
			 pvarResult, pexcepinfo, puArgErr);
...
STDMETHODIMP
CBaseVideoWindow::NonDelegatingQueryInterface(REFIID riid,
					      __deref_out void **ppv)
{
	ValidateReadWritePtr(ppv, sizeof(PVOID));
...
STDMETHODIMP
CBaseVideoWindow::GetTypeInfo(UINT itinfo, LCID lcid,
			      __deref_out ITypeInfo **pptinfo)
{
...
STDMETHODIMP
CBaseVideoWindow::GetIDsOfNames(REFIID riid,
				__in_ecount(cNames) LPOLESTR *rgszNames,
				UINT cNames, LCID lcid,
				__out_ecount(cNames) DISPID *rgdispid)
{
	return m_basedisp.GetIDsOfNames(IID_IVideoWindow, rgszNames, cNames,
					lcid, rgdispid);
}

STDMETHODIMP
CBaseVideoWindow::Invoke(DISPID dispidMember, REFIID riid, LCID lcid,
			 ...
			 __out_opt UINT *puArgErr)
{
	...
	hr = pti->Invoke((IVideoWindow *)this, dispidMember, wFlags,
			 pdispparams, pvarResult, pexcepinfo, puArgErr);
...
STDMETHODIMP
CBaseBasicVideo::NonDelegatingQueryInterface(REFIID riid,
					     __deref_out void **ppv)
{
	ValidateReadWritePtr(ppv, sizeof(PVOID));
...
STDMETHODIMP
CBaseBasicVideo::GetTypeInfo(UINT itinfo, LCID lcid,
			     __deref_out ITypeInfo **pptinfo)
{
...
STDMETHODIMP
CBaseBasicVideo::GetIDsOfNames(REFIID riid,
			       __in_ecount(cNames) LPOLESTR *rgszNames,
			       UINT cNames, LCID lcid,
			       __out_ecount(cNames) DISPID *rgdispid)
{
	return m_basedisp.GetIDsOfNames(IID_IBasicVideo, rgszNames, cNames,
					lcid, rgdispid);
}

STDMETHODIMP
CBaseBasicVideo::Invoke(DISPID dispidMember, REFIID riid, LCID lcid,
			WORD wFlags, __in DISPPARAMS *pdispparams,
			__out_opt VARIANT *pvarResult,
			__out_opt EXCEPINFO *pexcepinfo,
			__out_opt UINT *puArgErr)
{
	...
	hr = pti->Invoke((IBasicVideo *)this, dispidMember, wFlags, pdispparams,
			 pvarResult, pexcepinfo, puArgErr);
...
CDispParams::CDispParams(UINT nArgs, __in_ecount(nArgs) VARIANT *pArgs,
	...
	for (UINT i = 0; i < cArgs; i++) {
		...
		case VT_BSTR:
			...
			if ((PVOID)rgvarg[i].bstrVal != NULL) {
				OLECHAR *pch = rgvarg[i].bstrVal -
					       (sizeof(WORD) / sizeof(OLECHAR));
				delete pch;
			}
			break;
...
CDeferredCommand::CDeferredCommand(
	__inout CCmdQueue *pQ, __in_opt LPUNKNOWN pUnk, __inout HRESULT *phr,
	__in LPUNKNOWN pUnkExecutor, REFTIME time, __in GUID *iid,
	long dispidMethod, short wFlags, long nArgs,
	__in_ecount(nArgs) VARIANT *pDispParams, __out VARIANT *pvarResult,
...
CDeferredCommand::Invoke()
{
	...
	EXCEPINFO expinfo;
	UINT uArgErr;
	m_hrResult = pti->Invoke(pInterface, GetMethod(), GetFlags(),
				 GetParams(), GetResult(), &expinfo, &uArgErr);
	       ...
	       __out VARIANT *pvarResult, __out short *puArgErr, BOOL bStream)
{
	...
	*ppCmd = NULL;
	...
	pCmd = new CDeferredCommand(this,
				    ...
				    time, iid, dispidMethod, wFlags, cArgs,
				    pDispParams, pvarResult, puArgErr, bStream);
	...
	for (;;) {

		{
			...
			if (m_bRunning) {
				pos = m_listStream.GetHeadPosition();
				if (NULL != pos) {
					CDeferredCommand *pStrm =
						m_listStream.GetValid(pos);

					CRefTime t = pStrm->GetTime() +
						     m_StreamTimeOffset;
...
HRESULT
CCmdQueue::GetCommandDueFor(REFERENCE_TIME rtStream,
