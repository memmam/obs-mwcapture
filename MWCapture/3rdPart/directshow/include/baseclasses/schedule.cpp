...
{
	CAutoLock lck(
		&m_Serialize); // Need to stop the linked list from changing
...
REFERENCE_TIME CAMSchedule::Advise(const REFERENCE_TIME &rtTime)
{
	...
	DbgLog((LOG_TIMING, 2, TEXT("CAMSchedule::Advise( %lu ms )"),
		ULONG(rtTime / (UNITS / MILLISECONDS))));
	...
	while (rtTime >=
		       (rtNextTime = (pAdvise = head.m_next)->m_rtEventTime) &&
	       !pAdvise->IsZ()) {
	...
	DbgLog((LOG_TIMING, 3,
		...
		DWORD(rtNextTime / (UNITS / MILLISECONDS)),
		pAdvise->m_dwAdviseCookie));
...
{
	ASSERT(pPacket->m_rtEventTime >= 0 &&
	       pPacket->m_rtEventTime < MAX_TIME);
	...
	DbgLog((LOG_TIMING, 2,
		TEXT("Added advise %lu, for thread 0x%02X, scheduled at %lu"),
		pPacket->m_dwAdviseCookie, GetCurrentThreadId(),
...
void CAMSchedule::DumpLinkedList()
{
	...
	int i = 0;
	DbgLog((LOG_TIMING, 1,
		TEXT("CAMSchedule::DumpLinkedList() this = 0x%p"), this));
	for (CAdvisePacket *p = &head; p; p = p->m_next, i++) {
		DbgLog((LOG_TIMING, 1,
			TEXT("Advise List # %lu, Cookie %d,  RefTime %lu"), i,
			p->m_dwAdviseCookie,
