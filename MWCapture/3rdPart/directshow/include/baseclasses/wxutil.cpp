...
BOOL CAMMsgEvent::WaitMsg(DWORD dwTimeout)
{
	...
	do {
		dwWait = MsgWaitForMultipleObjects(1, &m_hEvent, FALSE,
...
DWORD WINAPI CAMThread::InitialThreadProc(__inout LPVOID pv)
{
...
BOOL CAMThread::Create()
{
	...
	m_hThread = CreateThread(NULL, 0, CAMThread::InitialThreadProc, this, 0,
				 &threadid);
...
HRESULT CAMThread::CoInitializeHelper()
{
	...
	if (hOle) {
		...
		PCoInitializeEx pCoInitializeEx =
			(PCoInitializeEx)(GetProcAddress(hOle,
							 "CoInitializeEx"));
		if (pCoInitializeEx) {
...
BOOL CMsgThread::CreateThread()
{
	...
	m_hThread = ::CreateThread(NULL, 0, DefaultThreadProc, (LPVOID)this, 0,
				   &m_ThreadId);
			    ...
			    __in size_t count)
{
	...
	memmoveInternal((BYTE *)dst + cb_dst_offset,
			(BYTE *)src + cb_src_offset, count);
	return S_OK;
}
...
void CCritSec::Lock()
{
	...
	if (currentOwner && (currentOwner != us)) {
		...
		if (m_fTrace) {
			DbgLog((LOG_LOCKING, 2,
				TEXT("Thread %d about to wait for lock %x owned by %d"),
				GetCurrentThreadId(), &m_CritSec,
				currentOwner));
			tracelevel = 2;
	...
	if (0 == m_lockCount++) {
		...
		if (m_fTrace) {
			DbgLog((LOG_LOCKING, tracelevel,
				TEXT("Thread %d now owns lock %x"),
...
void CCritSec::Unlock()
{
	if (0 == --m_lockCount) {
		// about to be unowned
		if (m_fTrace) {
			DbgLog((LOG_LOCKING, 3,
				TEXT("Thread %d releasing lock %x"),
				m_currentOwner, &m_CritSec));
		}
...
DWORD WINAPI WaitDispatchingMessages(HANDLE hObject, DWORD dwWait, HWND hwnd,
				     UINT uMsg, HANDLE hEvent)
{
	BOOL bPeeked = FALSE;
	DWORD dwResult;
	DWORD dwStart;
	...
	for (;;) {
		...
		if (dwResult == WAIT_OBJECT_0 + nCount ||
			...
			MSG msg;
			if (hwnd != NULL) {
				while (PeekMessage(&msg, hwnd, uMsg, uMsg,
						   PM_REMOVE)) {
					DispatchMessage(&msg);
				}
			}
	...
	if (bPeeked) {
		...
		if (HIWORD(GetQueueStatus(QS_POSTMESSAGE)) & QS_POSTMESSAGE) {
			...
			if (uMsgId != 0) {
				MSG msg;
				//  Remove old ones
				while (PeekMessage(&msg, (HWND)-1, uMsgId,
						   uMsgId, PM_REMOVE)) {
				}
			}
				...
				DWORD_PTR dwUser, UINT fuEvent)
{
#if WINVER >= 0x0501
	{
		...
		if (!fCheckedVersion) {
			fTimeKillSynchronousFlagAvailable =
