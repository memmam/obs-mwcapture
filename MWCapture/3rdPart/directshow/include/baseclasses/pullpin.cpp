...
CPullPin::CPullPin() : m_pReader(NULL), m_pAlloc(NULL), m_State(TM_Exit)
{
#ifdef DXMPERF
...
CPullPin::Connect(IUnknown *pUnk, IMemAllocator *pAlloc, BOOL bSync)
{
	...
	HRESULT hr =
		pUnk->QueryInterface(IID_IAsyncReader, (void **)&m_pReader);
	if (FAILED(hr)) {
...
HRESULT
CPullPin::DecideAllocator(IMemAllocator *pAlloc,
...
HRESULT
CPullPin::QueueSample(__inout REFERENCE_TIME &tCurrent,
...
HRESULT
CPullPin::DeliverSample(IMediaSample *pSample, REFERENCE_TIME tStart,
...
void CPullPin::Process(void)
{
	...
	REFERENCE_TIME tStart =
		AlignDown(m_tStart / UNITS, Actual.cbAlign) * UNITS;
	...
	REFERENCE_TIME tAlignStop =
		AlignUp(tStop / UNITS, Actual.cbAlign) * UNITS;
	...
	if (!m_bSync) {
		...
		while (tCurrent < tAlignStop) {
			...
			while (tCurrent < tAlignStop) {
				...
				hr = QueueSample(tCurrent, tAlignStop,
						 bDiscontinuity);
				bDiscontinuity = FALSE;
		...
		while (tCurrent < tAlignStop) {
			...
			LONGLONG tStopThis =
				tCurrent + (pSample->GetSize() * UNITS);
			if (tStopThis > tAlignStop) {
				tStopThis = tAlignStop;
			}
			pSample->SetTime(&tCurrent, &tStopThis);
