...
#define UInt32x32To64(a, b)                       \
	(((ULONGLONG)((ULONG)(a)) & 0xffffffff) * \
	 ((ULONGLONG)((ULONG)(b)) & 0xffffffff))
#endif
...
LONGLONG WINAPI llMulDiv(LONGLONG a, LONGLONG b, LONGLONG c, LONGLONG d)
{
    ...
    */
	...
	x.QuadPart = UInt32x32To64(ua.LowPart, ub.HighPart) +
		     UInt32x32To64(ua.HighPart, ub.LowPart) + p[0].HighPart;
	...
	if (ulic.HighPart == 0) {
		...
		uliResult.LowPart = 0;
		uliResult.QuadPart =
			p[0].QuadPart / dwDivisor + uliResult.QuadPart;
#else
        ...
        */
		if (uliDividend.QuadPart >= (DWORDLONG)dwDivisor) {
			uliResult.HighPart = EnlargedUnsignedDivide(
				uliDividend, dwDivisor, &p[0].HighPart);
		} else {
			uliResult.HighPart = 0;
		}
		uliResult.LowPart =
			EnlargedUnsignedDivide(p[0], dwDivisor, NULL);
#endif
...
LONGLONG WINAPI Int64x32Div32(LONGLONG a, LONG b, LONG c, LONG d)
{
    ...
    */
	if (uliDividend.QuadPart >= (DWORDLONG)dwDivisor) {
		uliResult.HighPart = EnlargedUnsignedDivide(
			uliDividend, dwDivisor, &p0.HighPart);
	} else {
		uliResult.HighPart = 0;
	}
