...
CVideoTransformFilter::CVideoTransformFilter(__in_opt LPCTSTR pName,
...
#define rcT1 ((VIDEOINFOHEADER *)(pmt->pbFormat))->rcTarget
	...
	if (pmt != NULL && pmt->pbFormat != NULL) {
		...
		LONG lStride =
			(HEADER(pmt->pbFormat)->biWidth * lBitCount + 7) / 8;
		lStride = (lStride + 3) & ~3;
		DbgLog((LOG_TRACE, 3,
			TEXT("*Changing input type on the fly to")));
		DbgLog((LOG_TRACE, 3,
			TEXT("FourCC: %lx Compression: %lx BitCount: %ld"),
			fccOut.GetFOURCC(), lCompression, lBitCount));
		DbgLog((LOG_TRACE, 3,
			TEXT("biHeight: %ld rcDst: (%ld, %ld, %ld, %ld)"),
			HEADER(pmt->pbFormat)->biHeight, rcT1.left, rcT1.top,
			rcT1.right, rcT1.bottom));
		DbgLog((LOG_TRACE, 3,
			TEXT("rcSrc: (%ld, %ld, %ld, %ld) Stride: %ld"),
			rcS1.left, rcS1.top, rcS1.right, rcS1.bottom, lStride));
#endif
...
#define rcT ((VIDEOINFOHEADER *)(pmtOut->pbFormat))->rcTarget
	...
	if (pmtOut != NULL && pmtOut->pbFormat != NULL) {
		...
		LONG lStride =
			(HEADER(pmtOut->pbFormat)->biWidth * lBitCount + 7) / 8;
		lStride = (lStride + 3) & ~3;
		DbgLog((LOG_TRACE, 3,
			TEXT("*Changing output type on the fly to")));
		DbgLog((LOG_TRACE, 3,
			TEXT("FourCC: %lx Compression: %lx BitCount: %ld"),
			fccOut.GetFOURCC(), lCompression, lBitCount));
		DbgLog((LOG_TRACE, 3,
			TEXT("biHeight: %ld rcDst: (%ld, %ld, %ld, %ld)"),
			HEADER(pmtOut->pbFormat)->biHeight, rcT.left, rcT.top,
			rcT.right, rcT.bottom));
		DbgLog((LOG_TRACE, 3,
			TEXT("rcSrc: (%ld, %ld, %ld, %ld) Stride: %ld"),
			rcS.left, rcS.top, rcS.right, rcS.bottom, lStride));
#endif
		...
		if (SUCCEEDED(hr)) {
			...
			DbgLog((LOG_TRACE, 3,
				TEXT("Output format change means we must wait for a keyframe")));
			m_nWaitForKey = 30;
	...
	if (pSample->IsDiscontinuity() == S_OK) {
		DbgLog((LOG_TRACE, 3,
			TEXT("Non-key discontinuity - wait for keyframe")));
		m_nWaitForKey = 30;
	}
	...
	if (SUCCEEDED(hr)) {
		...
		m_itrAvgDecode = m_tDecodeStart * (10000 / 16) +
				 15 * (m_itrAvgDecode / 16);
		...
		if (m_nWaitForKey && hr == NOERROR) {
			DbgLog((LOG_TRACE, 3,
				TEXT("still waiting for a keyframe")));
			hr = S_FALSE;
		}
	}
...
BOOL CVideoTransformFilter::ShouldSkipFrame(IMediaSample *pIn)
{
	...
	if (m_itrAvgDecode * 4 > itrFrame) {
		...
		if (m_itrLate > itrFrame) {
			...
			if (m_nKeyFramePeriod > 0) {
				...
				if (m_itrLate / 10000 > it) {
					...
					MSR_INTEGER(
						0,
						777770); // not near enough to next key
#endif
				}
			} else {
#ifdef VTRANSPERF
				MSR_INTEGER(0,
					    777771); // Next key not predictable
#endif
			}
		} else {
#ifdef VTRANSPERF
		...
		MSR_INTEGER(
			0,
			777773); // Decode time short - not not worth skipping
	...
	MSR_INTEGER(m_idLate,
		    (int)m_itrLate / 10000); // Note how late we think we are
	if (m_bSkipping) {
		if (!m_bQualityChanged) {
			m_bQualityChanged = TRUE;
			NotifyEvent(EC_QUALITY_CHANGE, 0, 0);
