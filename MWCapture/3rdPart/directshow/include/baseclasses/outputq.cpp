	  ...
	  m_hr(S_OK)
{
	...
	*phr = pInputPin->QueryInterface(IID_IMemInputPin,
					 (void **)&m_pInputPin);
	if (FAILED(*phr)) {
		return;
	}
	...
	if (bQueue) {
		...
		DWORD dwThreadId;
		m_hThread = CreateThread(NULL, 0, InitialThreadProc,
		...
		DbgLog((LOG_TRACE, 2,
			TEXT("Calling input pin directly - no thread")));
	}
}
...
DWORD COutputQueue::ThreadProc()
{
	while (TRUE) {
		...
		if (lNumberToSend != 0) {
			...
			if (m_hr == S_OK) {
				ASSERT(!m_bFlushed);
				HRESULT hr = m_pInputPin->ReceiveMultiple(
					m_ppSamples, lNumberToSend,
					&nProcessed);
			...
			if (m_hr != S_OK) {
				...
				DbgLog((LOG_ERROR, 2,
					TEXT("ReceiveMultiple returned %8.8X"),
					m_hr));
			}
		}
		...
		if (pSample == EOS_PACKET) {
			...
			if (m_hr == S_OK) {
				DbgLog((LOG_TRACE, 2,
					TEXT("COutputQueue sending EndOfStream()")));
				HRESULT hr = m_pPin->EndOfStream();
				if (FAILED(hr)) {
					DbgLog((LOG_ERROR, 2,
						TEXT("COutputQueue got code 0x%8.8X from EndOfStream()")));
				}
			}
		}
		...
		if (pSample == NEW_SEGMENT) {
			m_pPin->NewSegment(ppacket->tStart, ppacket->tStop,
					   ppacket->dRate);
			delete ppacket;
		}
	}
...
void COutputQueue::NewSegment(REFERENCE_TIME tStart, REFERENCE_TIME tStop,
			      double dRate)
{
	if (!IsQueued()) {
		if (S_OK == m_hr) {
...
HRESULT COutputQueue::ReceiveMultiple(__in_ecount(nSamples)
				      ...
				      __out long *nSamplesProcessed)
{
	...
	if (!IsQueued()) {
		...
		if (S_OK != m_hr) {
			...
			*nSamplesProcessed = 0;
			for (int i = 0; i < nSamples; i++) {
				DbgLog((LOG_TRACE, 3,
					TEXT("COutputQueue (direct) : Discarding %d samples code 0x%8.8X"),
					nSamples, m_hr));
				ppSamples[i]->Release();
			}
		...
		for (iDone = 0;
			...
			if (m_nBatched == m_lBatchSize ||
			    nSamples == 0 &&
				    (m_bSendAnyway || !m_bBatchExact)) {
				LONG nDone;
				DbgLog((LOG_TRACE, 4,
					TEXT("Batching %d samples"),
					m_nBatched));

				if (m_hr == S_OK) {
					m_hr = m_pInputPin->ReceiveMultiple(
						m_ppSamples, m_nBatched,
						&nDone);
				} else {
					nDone = 0;
				}
		...
		*nSamplesProcessed = iDone - iLost;
		...
		if (m_hr != S_OK) {
			*nSamplesProcessed = 0;
			DbgLog((LOG_TRACE, 3,
				TEXT("COutputQueue (queued) : Discarding %d samples code 0x%8.8X"),
				nSamples, m_hr));
			for (int i = 0; i < nSamples; i++) {
				ppSamples[i]->Release();
			}
