...
HRESULT CBaseWindow::PrepareWindow()
{
	...
	m_pClassName = GetClassWindowStyles(&m_ClassStyles, &m_WindowStyles,
					    &m_WindowStylesEx);
...
HRESULT CBaseWindow::DoneWithWindow()
{
	if (!IsWindow(m_hwnd) ||
		...
		if (IsWindow(m_hwnd)) {
			...
			ASSERT(GetWindowThreadProcessId(m_hwnd, NULL) !=
			       GetCurrentThreadId());
			...
			if (m_bDoPostToDestroy) {
				...
				PostMessage(m_hwnd, MsgDestroy,
					    (WPARAM)(HANDLE)m_evDone, 0);
	...
	if (!DestroyWindow(hwnd)) {
		DbgLog((LOG_TRACE, 0,
			TEXT("DestroyWindow %8.8X failed code %d"), hwnd,
			GetLastError()));
		DbgBreak("");
	}
...
HRESULT CBaseWindow::ActivateWindow()
{
	...
	AdjustWindowRectEx(&ClientRect, GetWindowLong(m_hwnd, GWL_STYLE), FALSE,
...
{
	{
		...
		SelectPalette(m_hdc, m_hPalette,
			      m_bBackground || bForceBackground);
		SelectPalette(m_MemoryDC, m_hPalette, m_bBackground);
	}
			 ...
			 LPARAM lParam) // Other parameter
{
	...
	if (pBaseWindow == NULL) {
		...
		if ((uMsg != WM_NCCREATE) ||
		    (NULL ==
		     (pBaseWindow = *(CBaseWindow **)((LPCREATESTRUCT)lParam)
					     ->lpCreateParams))) {
...
void CDrawImage::DisplaySampleTimes(IMediaSample *pSample)
{
	...
	pSample->GetTime((REFERENCE_TIME *)&m_StartSample,
			 (REFERENCE_TIME *)&m_EndSample);
...
void CDrawImage::UpdateColourTable(HDC hdc, __in BITMAPINFOHEADER *pbmi)
{
	...
	UINT uiReturn =
		SetDIBColorTable(hdc, (UINT)0, pbmi->biClrUsed, pColourTable);
...
void CDrawImage::FastRender(IMediaSample *pMediaSample)
{
	...
	if (m_bStretch == FALSE) {
		...
		BitBlt((HDC)m_hdc,        // Target device HDC
		       ...
		       m_TargetRect.right -
			       m_TargetRect.left, // Destination width
		       m_TargetRect.bottom -
			       m_TargetRect.top, // Destination height
...
BOOL CDrawImage::DrawVideoImageHere(HDC hdc, IMediaSample *pMediaSample,
				    __in LPRECT lprcSrc, __in LPRECT lprcDst)
{
	...
	BOOL bRet = (0 != StretchDIBits(hdc, TargetRect.left, TargetRect.top,
					TargetRect.right - TargetRect.left,
					TargetRect.bottom - TargetRect.top,
					SourceRect.left, lAdjustedSourceTop,
					SourceRect.right - SourceRect.left,
					SourceRect.bottom - SourceRect.top,
					pImage, (BITMAPINFO *)pbmi,
...
CImageAllocator::CImageAllocator(__inout CBaseFilter *pFilter,
				 __in_opt LPCTSTR pName, __inout HRESULT *phr)
...
CImageSample *CImageAllocator::CreateImageSample(__in_bcount(Length)
							 LPBYTE pData,
						 LONG Length)
{
	HRESULT hr = NOERROR;
...
CImageSample::CImageSample(__inout CBaseAllocator *pAllocator,
	...
	: CMediaSample(pName, pAllocator, phr, pBuffer, length), m_bInit(FALSE)
{
	ASSERT(pAllocator);
	ASSERT(pBuffer);
}
		...
		if (ContainsPalette(pOldInfo) == TRUE)
			if (pOldInfo->bmiHeader.biClrUsed == VideoEntries)
				if (pOldInfo->bmiHeader.biClrUsed > 0)
					if (memcmp((PVOID)GetBitmapPalette(
							   pNewInfo),
				   ...
				   __out CMediaType *pDest)
{
	...
	pDestInfo->bmiHeader.biClrImportant =
		pSrcInfo->bmiHeader.biClrImportant;
	...
	CopyMemory((PVOID)COLORS(pDestInfo), (PVOID)GetBitmapPalette(pSrcInfo),
...
HRESULT CImagePalette::MakeIdentityPalette(__inout_ecount_full(iColours)
						   PALETTEENTRY *pEntry,
					   INT iColours, __in LPSTR szDevice)
{
	...
	HDC hdc;
	if (szDevice == NULL ||
	    lstrcmpiLocaleIndependentA(szDevice, "DISPLAY") == 0)
	...
	UINT Result =
		GetSystemPaletteEntries(hdc, 0, PalLoCount, SystemEntries);
	...
	Result = GetSystemPaletteEntries(hdc, PalHiStart, PalLoCount,
					 SystemEntries);
	for (UINT Count = 0; Count < Result; Count++) {
		if (INT(Count) + PalHiStart < iColours) {
			if (SystemEntries[Count].peRed !=
				    pEntry[PalHiStart + Count].peRed ||
			    SystemEntries[Count].peGreen !=
				    pEntry[PalHiStart + Count].peGreen ||
			    SystemEntries[Count].peBlue !=
				    pEntry[PalHiStart + Count].peBlue) {
				bIdentityPalette = FALSE;
			}
		}
	}
	...
	for (UINT Count = PalLoCount; INT(Count) < min(PalHiStart, iColours);
	     Count++) {
...
HRESULT CImageDisplay::RefreshDisplayType(__in_opt LPSTR szDeviceName)
{
	...
	if (szDeviceName == NULL ||
	    lstrcmpiLocaleIndependentA(szDeviceName, "DISPLAY") == 0)
	...
	if (hbm) {
		GetDIBits(hdcDisplay, hbm, 0, 1, NULL,
			  (BITMAPINFO *)&m_Display.bmiHeader, DIB_RGB_COLORS);
...
BOOL CImageDisplay::CheckBitFields(const VIDEOINFO *pInput)
{
	...
	for (INT iColour = iRED; iColour <= iBLUE; iColour++) {
		...
		if (TestField & Mask) {
			NOTE1("Bit fields for component %d not contiguous",
			      iColour);
			return FALSE;
		}
	}
...
BOOL CImageDisplay::CheckHeaderValidity(const VIDEOINFO *pInput)
{
	...
	if (pInput->bmiHeader.biSizeImage !=
	    GetBitmapSize(&pInput->bmiHeader)) {
		if (pInput->bmiHeader.biSizeImage) {
...
HRESULT CImageDisplay::UpdateFormat(__inout VIDEOINFO *pVideoInfo)
{
	...
	if (pVideoInfo->bmiHeader.biClrImportant >
	    pVideoInfo->bmiHeader.biClrUsed) {
	...
	if (pVideoInfo->bmiHeader.biSizeImage == 0) {
		pVideoInfo->bmiHeader.biSizeImage =
			GetBitmapSize(&pVideoInfo->bmiHeader);
	}
	return NOERROR;
}
...
STDAPI ConvertVideoInfoToVideoInfo2(__inout AM_MEDIA_TYPE *pmt)
{
	...
	HRESULT hr = DWordAdd(
		pmt->cbFormat,
		sizeof(VIDEOINFOHEADER2) - sizeof(VIDEOINFOHEADER), &dwNewSize);
	if (FAILED(hr)) {
		return hr;
	}
	...
	CopyMemory(pvNew, pmt->pbFormat,
		   FIELD_OFFSET(VIDEOINFOHEADER, bmiHeader));
	...
	pVideoInfo2->dwPictAspectRatioY =
		(DWORD)abs(pVideoInfo2->bmiHeader.biHeight);
...
STDAPI CheckVideoInfo2Type(const AM_MEDIA_TYPE *pmt)
{
	...
	const VIDEOINFOHEADER2 *pHeader =
