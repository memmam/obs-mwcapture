...
CTransformFilter::CTransformFilter(__in_opt LPCTSTR pName,
				   __inout_opt LPUNKNOWN pUnk, REFCLSID clsid)
...
CBasePin *CTransformFilter::GetPin(int n)
{
	...
	if (m_pInput == NULL) {
		...
		m_pOutput = (CTransformOutputPin *)new CTransformOutputPin(
...
HRESULT
CTransformFilter::InitializeOutputSample(IMediaSample *pSample,
					 __deref_out IMediaSample **ppOutSample)
{
	...
	HRESULT hr = m_pOutput->m_pAllocator->GetBuffer(
		&pOutSample,
		pProps->dwSampleFlags & AM_SAMPLE_TIMEVALID ? &pProps->tStart
							    : NULL,
		pProps->dwSampleFlags & AM_SAMPLE_STOPVALID ? &pProps->tStop
							    : NULL,
		dwFlags);
...
CTransformFilter::Receive(IMediaSample *pSample)
{
	...
	if (FAILED(hr)) {
		...
		if (hr == NOERROR) {
			...
			m_bSampleSkipped =
				FALSE; // last thing no longer dropped
		} else {
...
HRESULT
CTransformFilter::NewSegment(REFERENCE_TIME tStart, REFERENCE_TIME tStop,
			     double dRate)
{
	if (m_pOutput != NULL) {
...
CTransformInputPin::CTransformInputPin(
	...
	: CBaseInputPin(pObjectName, pTransformFilter,
			&pTransformFilter->m_csFilter, phr, pName)
{
...
#ifdef UNICODE
CTransformInputPin::CTransformInputPin(
	__in_opt LPCSTR pObjectName, __inout CTransformFilter *pTransformFilter,
...
{
	HRESULT hr =
		m_pTransformFilter->CompleteConnect(PINDIR_INPUT, pReceivePin);
	if (FAILED(hr)) {
		return hr;
	}
...
STDMETHODIMP
CTransformInputPin::NewSegment(REFERENCE_TIME tStart, REFERENCE_TIME tStop,
			       double dRate)
{
...
CTransformOutputPin::CTransformOutputPin(
	...
	: CBaseOutputPin(pObjectName, pTransformFilter,
			 &pTransformFilter->m_csFilter, phr, pPinName),
	  m_pPosition(NULL)
{
	DbgLog((LOG_TRACE, 2,
		TEXT("CTransformOutputPin::CTransformOutputPin")));
	m_pTransformFilter = pTransformFilter;
}
...
STDMETHODIMP
CTransformOutputPin::NonDelegatingQueryInterface(REFIID riid,
...
{
	HRESULT hr =
		m_pTransformFilter->CompleteConnect(PINDIR_OUTPUT, pReceivePin);
	if (FAILED(hr)) {
		return hr;
	}
...
#ifdef DEBUG
	if (FAILED(m_pTransformFilter->CheckTransform(
		    &m_pTransformFilter->m_pInput->CurrentMediaType(),
		    pmtOut))) {
		DbgLog((LOG_ERROR, 0,
			TEXT("*** This filter is accepting an output media type")));
		DbgLog((LOG_ERROR, 0,
			TEXT("    that it can't currently transform to.  I hope")));
		DbgLog((LOG_ERROR, 0,
			TEXT("    it's smart enough to reconnect its input.")));
	}
#endif
...
HRESULT
CTransformOutputPin::DecideBufferSize(IMemAllocator *pAllocator,
