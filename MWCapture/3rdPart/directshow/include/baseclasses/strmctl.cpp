...
STDMETHODIMP CBaseStreamControl::StopAt(const REFERENCE_TIME *ptStop,
					BOOL bSendExtra, DWORD dwCookie)
{
	...
	if (ptStop) {
		if (*ptStop == MAX_TIME) {
			...
			if (m_FilterState == State_Stopped &&
				...
				DbgLog((LOG_TRACE, 2,
					TEXT("graph will begin by DISCARDING")));
			}
			return NOERROR;
		}
		...
		if (m_FilterState == State_Stopped && m_tStartTime > *ptStop) {
			m_StreamState = STREAM_FLOWING;
			DbgLog((LOG_TRACE, 2,
				TEXT("graph will begin by FLOWING")));
		}
...
STDMETHODIMP CBaseStreamControl::StartAt(const REFERENCE_TIME *ptStart,
					 DWORD dwCookie)
{
	CAutoLock lck(&m_CritSec);
	if (ptStart) {
		...
		DbgLog((LOG_TRACE, 2, TEXT("StartAt: %dms"),
			(int)(*ptStart / 10000)));
...
STDMETHODIMP CBaseStreamControl::GetInfo(__out AM_STREAM_INFO *pInfo)
{
	...
	pInfo->dwFlags |=
		m_tStartTime == MAX_TIME ? 0 : AM_STREAM_INFO_START_DEFINED;
	pInfo->dwFlags |= m_tStopTime == MAX_TIME ? 0
...
void CBaseStreamControl::ExecuteStop()
{
	...
	if (m_dwStopCookie && m_pSink) {
		DbgLog((LOG_TRACE, 2,
			TEXT("*sending EC_STREAM_CONTROL_STOPPED (%d)"),
			m_dwStopCookie));
		m_pSink->Notify(EC_STREAM_CONTROL_STOPPED, (LONG_PTR)this,
				m_dwStopCookie);
	}
...
void CBaseStreamControl::ExecuteStart()
{
	...
	if (m_dwStartCookie) {
		DbgLog((LOG_TRACE, 2,
			TEXT("*sending EC_STREAM_CONTROL_STARTED (%d)"),
			m_dwStartCookie));
		m_pSink->Notify(EC_STREAM_CONTROL_STARTED, (LONG_PTR)this,
				m_dwStartCookie);
	}
...
enum CBaseStreamControl::StreamControlState
CBaseStreamControl::CheckSampleTimes(__in const REFERENCE_TIME *pSampleStart,
...
enum CBaseStreamControl::StreamControlState
CBaseStreamControl::CheckStreamState(IMediaSample *pSample)
{
	...
	do {
		...
		if (!m_pRefClock) {
			...
			lWait = LONG((rtBufferStart - rtNow) /
				     10000); // 100ns -> ms
			if (lWait < 10)
				break; // Not worth waiting - discard early
		}

	} while (WaitForSingleObject(GetStreamEventHandle(), lWait) !=
		 WAIT_TIMEOUT);
