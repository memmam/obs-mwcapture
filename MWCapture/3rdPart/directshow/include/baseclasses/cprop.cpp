...
STDMETHODIMP
CBasePropertyPage::NonDelegatingQueryInterface(REFIID riid,
...
INT_PTR CALLBACK CBasePropertyPage::DialogProc(HWND hwnd, UINT uMsg,
...
STDMETHODIMP CBasePropertyPage::Activate(HWND hwndParent, LPCRECT pRect,
					 BOOL fModal)
{
	...
	m_hwnd = CreateDialogParam(g_hInst, MAKEINTRESOURCE(m_DialogId),
...
STDMETHODIMP
CBasePropertyPage::SetPageSite(__in_opt LPPROPERTYPAGESITE pPageSite)
{
	if (pPageSite) {
...
INT_PTR CBasePropertyPage::OnReceiveMessage(HWND hwnd, UINT uMsg, WPARAM wParam,
					    LPARAM lParam)
{
	...
	{
		pPropertyPage =
			_GetWindowLongPtr<CBasePropertyPage *>(hwnd, DWLP_USER);
