...
typedef enum {
	...
	AM_PROPERTY_DVDSUBPIC_COMPOSIT_ON =
		2 // TRUE for subpicture is displayed
...
typedef enum {
	...
	AM_DVDCOPYSTATE_INITIALIZE_TITLE =
		1, // indicates we are starting a title
...
#define AMMPEG2_27MhzTimebase \
	0x00000100 //if set, the PTS,DTS timestamps advance at 27MHz rather than 90KHz
...
#define SIZE_MPEG2VIDEOINFO(pv)                              \
	(FIELD_OFFSET(MPEG2VIDEOINFO, dwSequenceHeader[0]) + \
	 (pv)->cbSequenceHeader)
...
#define AM_VIDEO_FLAG_FIELD_MASK \
	0x0003L // use this mask to check whether the sample is field1 or field2 or frame
