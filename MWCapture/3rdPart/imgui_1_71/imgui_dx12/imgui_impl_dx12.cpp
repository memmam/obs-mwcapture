				   ...
				   ID3D12GraphicsCommandList *ctx)
{
	...
	FrameResources *fr =
		&g_pFrameResources[g_frameIndex % g_numFramesInFlight];
		...
		for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
			...
			if (pcmd->UserCallback != NULL) {
				...
				ctx->SetGraphicsRootDescriptorTable(
					1, *(D3D12_GPU_DESCRIPTOR_HANDLE *)&pcmd
						    ->TextureId);
...
static void ImGui_ImplDX12_CreateFontsTexture()
{
	...
	// Upload texture to graphics system
	{
		...
		hr = g_pd3dDevice->CreateCommandList(
			0, D3D12_COMMAND_LIST_TYPE_DIRECT, cmdAlloc, NULL,
		...
		cmdQueue->ExecuteCommandLists(
			1, (ID3D12CommandList *const *)&cmdList);
...
bool ImGui_ImplDX12_CreateDeviceObjects()
{
	...
	// Create the root signature
	{
		...
		ID3DBlob *blob = NULL;
		if (D3D12SerializeRootSignature(&desc,
	...
	// Create the vertex shader
	{
		...
		D3DCompile(vertexShader, strlen(vertexShader), NULL, NULL, NULL,
			   "main", "vs_5_0", 0, 0, &g_pVertexShaderBlob, NULL);
	...
	// Create the pixel shader
	{
		...
		D3DCompile(pixelShader, strlen(pixelShader), NULL, NULL, NULL,
			   "main", "ps_5_0", 0, 0, &g_pPixelShaderBlob, NULL);
		if (g_pPixelShaderBlob ==
		    NULL) // NB: Pass ID3D10Blob* pErrorBlob to D3DCompile() to get error showing in (const char*)pErrorBlob->GetBufferPointer(). Make sure to Release() the blob!
			return false;
		psoDesc.PS = {g_pPixelShaderBlob->GetBufferPointer(),
	...
	if (g_pd3dDevice->CreateGraphicsPipelineState(
		    &psoDesc, IID_PPV_ARGS(&g_pPipelineState)) != S_OK)
		return false;
