...
static void ImGui_ImplWin32_UpdateGamepads()
{
	...
	if (g_WantUpdateHasGamepad) {
		...
		g_HasGamepad = (XInputGetCapabilities(0, XINPUT_FLAG_GAMEPAD,
						      &caps) == ERROR_SUCCESS);
...
#define MAP_BUTTON(NAV_NO, BUTTON_ENUM)                                 \
	{                                                               \
		io.NavInputs[NAV_NO] =                                  \
			(gamepad.wButtons & BUTTON_ENUM) ? 1.0f : 0.0f; \
	}
#define MAP_ANALOG(NAV_NO, VALUE, V0, V1)                          \
	{                                                          \
		...
		MAP_BUTTON(ImGuiNavInput_Activate,
			   XINPUT_GAMEPAD_A); // Cross / A
		MAP_BUTTON(ImGuiNavInput_Cancel,
			   XINPUT_GAMEPAD_B);                     // Circle / B
						      ...
						      LPARAM lParam)
{
	...
	case WM_XBUTTONDBLCLK: {
		...
		if (msg == WM_XBUTTONDOWN || msg == WM_XBUTTONDBLCLK) {
			button = (GET_XBUTTON_WPARAM(wParam) == XBUTTON1) ? 3
									  : 4;
		}
