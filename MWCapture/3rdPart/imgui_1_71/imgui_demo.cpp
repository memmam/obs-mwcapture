...
static void ShowDemoWindowWidgets()
{
	...
	if (ImGui::TreeNode("Basic")) {
...

		{
			...
			ImGui::SliderFloat("slider float", &f1, 0.0f, 1.0f,
					   "ratio = %.3f");
			ImGui::SliderFloat("slider float (curve)", &f2, -10.0f,
					   10.0f, "%.4f", 2.0f);
	...
	if (ImGui::TreeNode("Plots Widgets")) {
		...
		float (*func)(void *, int) = (func_type == 0) ? Funcs::Sin
							      : Funcs::Saw;
	...
	if (ImGui::TreeNode("Color/Picker Widgets")) {
		...
		if (saved_palette_init) {
			for (int n = 0; n < IM_ARRAYSIZE(saved_palette); n++) {
				ImGui::ColorConvertHSVtoRGB(n / 31.0f, 0.8f,
							    0.8f,
							    saved_palette[n].x,
							    saved_palette[n].y,
							    saved_palette[n].z);
		...
		ImGui::ColorEdit4("HSV shown as HSV##1",
				  ...
				  ImGuiColorEditFlags_DisplayHSV |
					  ImGuiColorEditFlags_InputHSV |
					  ImGuiColorEditFlags_Float);
		ImGui::DragFloat4("Raw HSV values",
				  (float *)&color_stored_as_hsv, 0.01f, 0.0f,
				  1.0f);
	...
	if (ImGui::TreeNode("Range Widgets")) {
		...
		ImGui::DragFloatRange2("range", &begin, &end, 0.25f, 0.0f,
...
// Note that the SliderScalar function has a maximum usable range of half the natural type maximum, hence the /2 below.
		...
		ImGui::DragScalar("drag double", ImGuiDataType_Double, &f64_v,
				  ...
				  1.0f);
		ImGui::DragScalar("drag double ^2", ImGuiDataType_Double,
	...
	if (ImGui::TreeNode("Drag and Drop")) {
...

		{
			...
			static const char *names[9] = {"Bobby", "Beatrice",
						       "Betty", "Brianna",
						       "Barry", "Bernard",
						       "Bibi",  "Blaine",
						       "Bryn"};
...
static void ShowDemoWindowLayout()
{
	...
	if (ImGui::TreeNode("Scrolling")) {
		...
		ImGui::SameLine(140);
		scroll_to_off |= ImGui::DragFloat("##off_y", &scroll_to_off_px,
						  1.00f, 0, 9999, "+%.0f px");

		bool scroll_to_pos = ImGui::Button("Scroll To Pos");
		ImGui::SameLine(140);
		scroll_to_pos |= ImGui::DragFloat("##pos_y", &scroll_to_pos_px,
						  1.00f, 0, 9999,
						  "Y = %.0f px");
	...
	if (ImGui::TreeNode("Horizontal Scrolling")) {
		...
		for (int line = 0; line < lines; line++) {
			...
			for (int n = 0; n < num_buttons; n++) {
				...
				ImGui::Button(
					label,
					ImVec2(40.0f + sinf((float)(line + n)) *
							       20.0f,
					       0.0f));
				ImGui::PopStyleColor(3);
				ImGui::PopID();
...
static void ShowDemoWindowColumns()
{
    ...
    */
	...
	if (ImGui::TreeNode("Tree")) {
		...
		for (int x = 0; x < 3; x++) {
			bool open1 = ImGui::TreeNode((void *)(intptr_t)x,
						     "Node%d", x);
			ImGui::NextColumn();
	...
	if (ImGui::CollapsingHeader("Inputs, Navigation & Focus")) {
		...
		if (ImGui::TreeNode("Dragging")) {
			...
			ImGui::Text(
				...
				value_with_lock_threshold.y, value_raw.x,
...
void ImGui::ShowStyleEditor(ImGuiStyle *ref)
{
	...
	if (ImGui::BeginTabBar("##tabs", ImGuiTabBarFlags_None)) {
		...
		if (ImGui::BeginTabItem("Fonts")) {
			...
			for (int i = 0; i < atlas->Fonts.Size; i++) {
				...
				if (font_details_opened) {
					...
					ImGui::Text(
						...
						font->FallbackChar,
						font->FallbackChar);
					const float surface_sqrt = sqrtf(
						(float)font
							->MetricsTotalSurface);
					ImGui::Text(
			...
			if (ImGui::DragFloat("this window scale", &window_scale,
					     0.005f, 0.3f, 2.0f,
					     "%.2f")) // scale only this window
				ImGui::SetWindowFontScale(window_scale);
			ImGui::DragFloat("global scale", &io.FontGlobalScale,
					 0.005f, 0.3f, 2.0f,
					 "%.2f"); // scale everything
			ImGui::PopItemWidth();
		...
		if (ImGui::BeginTabItem("Rendering")) {
			...
			ImGui::DragFloat(
				"Global Alpha", &style.Alpha, 0.005f, 0.20f,
				1.0f,
		...
		for (int i = 0; i < Items.Size; i++) {
			...
			if (strstr(item, "[error]")) {
				ImGui::PushStyleColor(ImGuiCol_Text,
						      ImVec4(1.0f, 0.4f, 0.4f,
							     1.0f));
				pop_color = true;
			} else if (strncmp(item, "# ", 2) == 0) {
				ImGui::PushStyleColor(ImGuiCol_Text,
						      ImVec4(1.0f, 0.8f, 0.6f,
							     1.0f));
				pop_color = true;
			}
	...
	if (ImGui::BeginTabBar("##TabBar")) {
		...
		if (ImGui::BeginTabItem("Primitives")) {
			...
			for (int n = 0; n < 2; n++) {
				...
				draw_list->AddCircle(ImVec2(x + sz * 0.5f,
							    y + sz * 0.5f),
						     sz * 0.5f, col, 6, th);
			...
			draw_list->AddCircleFilled(ImVec2(x + sz * 0.5f,
							  y + sz * 0.5f),
						   sz * 0.5f, col, 6);
			x += sz + spacing; // Hexagon
			draw_list->AddCircleFilled(ImVec2(x + sz * 0.5f,
			...
			ImGui::Dummy(ImVec2((sz + spacing) * 9.8f,
					    (sz + spacing) * 3));
			ImGui::EndTabItem();
		}
...
struct ExampleAppDocuments {
	...
	ExampleAppDocuments()
	{
		...
		Documents.push_back(MyDocument("Carrot", true,
...
void ShowExampleAppDocuments(bool *p_open)
{
	...
	if (!ImGui::Begin("Example: Documents", p_open,
			  ImGuiWindowFlags_MenuBar)) {
		ImGui::End();
		return;
	}
