			       ...
			       const ImVec2 *text_size_if_known)
{
	...
	if (text_size.x > pos_max.x - pos_min.x) {
		...
		const float ellipsis_width =
			(1.0f + 1.0f) * ellipsis_dot_count - 1.0f;
		const char *text_end_ellipsis = NULL;

		float text_width =
			ImMax((pos_max.x - ellipsis_width) - pos_min.x, 1.0f);
		float text_size_clipped_x =
			font->CalcTextSizeA(font_size, text_width, 0.0f, text,
		...
		RenderTextClippedEx(draw_list, pos_min,
				    ImVec2(clip_max_x, pos_max.y), text,
				    text_end_ellipsis, &text_size,
				    ImVec2(0.0f, 0.0f));
...
void ImGui::UpdateMouseWheel()
{
	...
	if (g.IO.MouseWheel != 0.0f && g.IO.KeyCtrl &&
	    g.IO.FontAllowUserScaling && !window->Collapsed) {
	...
	if (scroll_allowed &&
		...
		if (g.IO.MouseWheelH != 0.0f && !g.IO.KeyShift) {
			float scroll_step = ImFloor(
				ImMin(2 * window->CalcFontSize(), max_step.x));
			SetWindowScrollX(window, window->Scroll.x -
							 g.IO.MouseWheelH *
								 scroll_step);
		}
	}
...
void ImGui::NewFrame()
{
	...
	if (g.ActiveId == 0 && g.FocusTabPressed) {
		...
		if (g.NavId != 0 && g.NavIdTabCounter != INT_MAX)
			g.FocusRequestNextCounterTab = g.NavIdTabCounter + 1 +
						       (g.IO.KeyShift ? -1 : 1);
		else
		...
		if (windows_to_render_front_most[n] &&
		    IsWindowActiveAndVisible(
			    windows_to_render_front_most
				    [n])) // NavWindowingTarget is always temporarily displayed as the front-most window
...
static ImVec2 CalcSizeAutoFit(ImGuiWindow *window, const ImVec2 &size_contents)
{
	...
	ImVec2 size_decorations = ImVec2(0.0f, window->TitleBarHeight() +
						     ...
						     bool snap_on_edges)
{
	...
	if (window->ScrollTarget.x < FLT_MAX) {
		...
		scroll.x = window->ScrollTarget.x -
			   cr_x * window->InnerRect.GetWidth();
	}
				      ...
				      ImU32 resize_grip_col[4])
{
	...
	const int resize_border_count = g.IO.ConfigWindowsResizeFromEdges ? 4
									  : 0;
	...
	ImVec2 size_for_clamping =
		(g.IO.ConfigWindowsMoveFromTitleBarOnly &&
		 !(window->Flags & ImGuiWindowFlags_NoTitleBar))
			...
			: window->Size;
	window->Pos =
		ImMin(rect.Max - padding, ImMax(window->Pos + size_for_clamping,
						rect.Min + padding) -
						  size_for_clamping);
}
...
static void ImGui::RenderWindowOuterBorders(ImGuiWindow *window)
{
	...
	if (border_held != -1) {
		...
		ImRect border_r = GetResizeBorderRect(window, border_held,
						      rounding, 0.0f);
...
bool ImGui::Begin(const char *name, bool *p_open, ImGuiWindowFlags flags)
{
	...
	if (first_begin_of_the_frame) {
		...
		const ImVec2 size_auto_fit =
			CalcSizeAutoFit(window, window->ContentSize);
		...
		const float resize_grip_draw_size = (float)(int)ImMax(
			...
			window->WindowRounding + 1.0f + g.FontSize * 0.2f);
		if (!window->Collapsed)
			if (UpdateManualResize(window, size_auto_fit,
					       &border_held, resize_grip_count,
					       &resize_grip_col[0]))
		...
		window->InnerClipRect.Max.x =
			ImFloor(0.5f + window->InnerRect.Max.x -
				ImMax(ImFloor(window->WindowPadding.y * 0.5f),
				      window->WindowBorderSize));
	...
	if ((g.NavId != id ||
		...
		if ((g.NavMoveRequestFlags &
			...
			if (ImClamp(nav_bb.Max.y, window->ClipRect.Min.y,
				...
				if (NavScoreItem(&g.NavMoveResultLocalVisibleSet,
						 nav_bb)) {
					result =
			...
			if (window->DC.NavLayerActiveMask == 0x00 &&
				...
				if (IsKeyPressed(g.IO.KeyMap[ImGuiKey_PageUp],
					...
					nav_scoring_rect_offset_y =
						-page_offset_y;
					g.NavMoveDir =
						ImGuiDir_Down; // Because our scoring rect is offset, we intentionally request the opposite direction (so we can always land on the last item)
					...
					nav_scoring_rect_offset_y =
						+page_offset_y;
					g.NavMoveDir =
						ImGuiDir_Up; // Because our scoring rect is offset, we intentionally request the opposite direction (so we can always land on the last item)
...
void ImGui::NavUpdateWindowingList()
{
	...
	SetNextWindowSizeConstraints(ImVec2(g.IO.DisplaySize.x * 0.20f,
					    g.IO.DisplaySize.y * 0.20f),
...
void ImGui::ShowMetricsWindow(bool *p_open)
{
	...
	struct Funcs {
					 ...
					 const char *label)
		{
			...
			for (const ImDrawCmd *pcmd =
				...
				ImGui::Text(
					...
					pcmd->ElemCount, pcmd->ElemCount / 3,
					pcmd->VtxOffset, pcmd->IdxOffset);
		...
		{
			if (!ImGui::TreeNode(window, "%s '%s', %d @ 0x%p",
					     label, window->Name,
					     window->Active ||
						     window->WasActive,
					     window))
			...
			ImGui::BulletText(
				...
				window->Size.y, window->ContentSize.x,
				window->ContentSize.y);
			ImGui::BulletText(
	...
	if (ImGui::TreeNode("Tools")) {
		...
		const char *rects_names[RT_Count] = {
			"OuterRect",         "OuterRectClipped", "InnerRect",
			"InnerClipRect",     "WorkRect",         "Contents",
			"ContentsRegionRect"};
		show_windows_rects |= ImGui::Combo("##rects_type",
						   &show_windows_rect_type,
						   rects_names, RT_Count);
