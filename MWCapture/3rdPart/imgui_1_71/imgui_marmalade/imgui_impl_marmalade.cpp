...
void ImGui_Marmalade_RenderDrawData(ImDrawData *draw_data)
{
	...
	for (int n = 0; n < draw_data->CmdListsCount; n++) {
		...
		for (int i = 0; i < nVert; i++) {
			...
			pVertStream[i].x =
				cmd_list->VtxBuffer[i].pos.x * g_RenderScale.x;
			pVertStream[i].y =
				cmd_list->VtxBuffer[i].pos.y * g_RenderScale.y;
		...
		for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
			...
			if (pcmd->UserCallback) {
				...
				CIwMaterial *pCurrentMaterial =
					IW_GX_ALLOC_MATERIAL();
				pCurrentMaterial->SetShadeMode(
					CIwMaterial::SHADE_FLAT);
				pCurrentMaterial->SetCullMode(
					CIwMaterial::CULL_NONE);
				pCurrentMaterial->SetFiltering(false);
				pCurrentMaterial->SetAlphaMode(
				...
				pCurrentMaterial->SetTexture(
					(CIwTexture *)pcmd->TextureId);
				IwGxSetMaterial(pCurrentMaterial);
				IwGxDrawPrims(IW_GX_TRI_LIST,
					      (uint16 *)idx_buffer,
					      pcmd->ElemCount);
			}
...
static void ImGui_Marmalade_SetClipboardText(void * /*user_data*/,
					     const char *text)
{
	if (s3eClipboardAvailable())
...
int32 ImGui_Marmalade_KeyCallback(void *system_data, void *user_data)
{
	...
	io.KeyCtrl =
		s3eKeyboardGetState(s3eKeyLeftControl) == S3E_KEY_STATE_DOWN ||
		s3eKeyboardGetState(s3eKeyRightControl) == S3E_KEY_STATE_DOWN;
	io.KeyShift =
		s3eKeyboardGetState(s3eKeyLeftShift) == S3E_KEY_STATE_DOWN ||
...
{
	ImGuiIO &io = ImGui::GetIO();
	io.BackendPlatformName = io.BackendRendererName =
		"imgui_impl_marmalade";

	io.KeyMap[ImGuiKey_Tab] =
		s3eKeyTab; // Keyboard mapping. ImGui will use those indices to peek into the io.KeysDown[] array.
	...
	if (install_callbacks) {
		...
		s3eKeyboardRegister(S3E_KEYBOARD_KEY_EVENT,
				    ImGui_Marmalade_KeyCallback, 0);
		s3eKeyboardRegister(S3E_KEYBOARD_CHAR_EVENT,
				    ImGui_Marmalade_CharCallback, 0);
	}
...
void ImGui_Marmalade_NewFrame()
{
	...
	if (io.WantTextInput) {
		...
		if (!g_osdKeyboardEnabled) {
			...
			s3eKeyboardSetInt(S3E_KEYBOARD_GET_CHAR,
					  1); // show OSD keyboard
		}
	} else {
