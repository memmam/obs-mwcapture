	...
	ImGui_ImplVulkanH_FrameRenderBuffers *rb, int fb_width, int fb_height)
{
	// Bind pipeline and descriptor sets:
	{
		vkCmdBindPipeline(command_buffer,
				  VK_PIPELINE_BIND_POINT_GRAPHICS, g_Pipeline);
				     ...
				     VkCommandBuffer command_buffer)
{
	...
	if (wrb->FrameRenderBuffers == NULL) {
		...
		wrb->FrameRenderBuffers =
			(ImGui_ImplVulkanH_FrameRenderBuffers *)IM_ALLOC(
				sizeof(ImGui_ImplVulkanH_FrameRenderBuffers) *
				wrb->Count);
	...
	// Upload vertex/index data into a single contiguous GPU buffer
	{
		...
		err = vkMapMemory(v->Device, rb->VertexBufferMemory, 0,
			...
			if (pcmd->UserCallback != NULL) {
				...
				if (clip_rect.x < fb_width &&
					...
					scissor.extent.width =
						(uint32_t)(clip_rect.z -
							   clip_rect.x);
					scissor.extent.height =
						(uint32_t)(clip_rect.w -
							   clip_rect.y);
...
bool ImGui_ImplVulkan_CreateFontsTexture(VkCommandBuffer command_buffer)
{
	...
	// Create the Image:
	{
		...
		err = vkAllocateMemory(v->Device, &alloc_info, v->Allocator,
				       &g_FontMemory);
		check_vk_result(err);
		err = vkBindImageMemory(v->Device, g_FontImage, g_FontMemory,
					0);
		check_vk_result(err);
	}
	...
	// Create the Upload Buffer:
	{
		...
		check_vk_result(err);
		err = vkBindBufferMemory(v->Device, g_UploadBuffer,
					 g_UploadBufferMemory, 0);
		check_vk_result(err);
	}
	...
	{
		char *map = NULL;
		err = vkMapMemory(v->Device, g_UploadBufferMemory, 0,
	...
	// Copy to Image:
	{
		...
		vkCmdCopyBufferToImage(command_buffer, g_UploadBuffer,
				       g_FontImage,
...
bool ImGui_ImplVulkan_CreateDeviceObjects()
{
	...
	// Create The Shader Modules:
	{
		...
		err = vkCreateShaderModule(v->Device, &vert_info, v->Allocator,
					   &vert_module);
		check_vk_result(err);
		...
		err = vkCreateShaderModule(v->Device, &frag_info, v->Allocator,
					   &frag_module);
		check_vk_result(err);
	}
	...
	// Create Descriptor Set:
	{
		...
		err = vkAllocateDescriptorSets(v->Device, &alloc_info,
					       &g_DescriptorSet);
		check_vk_result(err);
	}
...
void ImGui_ImplVulkan_DestroyDeviceObjects()
{
	...
	if (g_PipelineLayout) {
		vkDestroyPipelineLayout(v->Device, g_PipelineLayout,
					v->Allocator);
	...
	int w, int h, uint32_t min_image_count)
{
	...
	// Create Swapchain
	{
		...
		wd->FrameSemaphores =
			(ImGui_ImplVulkanH_FrameSemaphores *)IM_ALLOC(
				sizeof(ImGui_ImplVulkanH_FrameSemaphores) *
				wd->ImageCount);
