...
bool ImGui::MWButton(const char *label, const ImVec2 &size_arg, bool enable)
{
	if (enable)
		return ButtonEx(label, size_arg, 0);
	else
			  ...
			  ImGuiButtonFlags flags)
{
	...
	ItemSize(size,
		 (size.y >= default_size) ? g.Style.FramePadding.y : 0.0f);
	if (!ItemAdd(bb, id))
		return false;
			...
			ImDrawCornerFlags rounding_corners)
{
	...
	ImRect bb = bb_frame;
	bb.Expand(ImVec2(-ImClamp((float)(int)((bb_frame_width - 2.0f) * 0.5f),
				  0.0f, 3.0f),
			 -ImClamp((float)(int)((bb_frame_height - 2.0f) * 0.5f),
				  0.0f, 3.0f)));

	// V denote the main, longer axis of the scrollbar (= height for a vertical scrollbar)
	const float scrollbar_size_v = horizontal ? bb.GetWidth()
						  : bb.GetHeight();
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags)
{
	...
	if (flags & ImGuiSeparatorFlags_Vertical) {
		...
		const ImRect bb(ImVec2(window->DC.CursorPos.x, y1),
				ImVec2(window->DC.CursorPos.x + thickness_draw,
				       y2));
		       ...
		       ImGuiComboFlags flags)
{
	...
	if (!(flags & ImGuiComboFlags_NoPreview))
		window->DrawList->AddRectFilled(
			frame_bb.Min, ImVec2(value_x2, frame_bb.Max.y),
				...
				const char *format)
{
	...
	if (data_type == ImGuiDataType_S16)
		return ImFormatString(buf, buf_size, format,
				      *(const ImS16 *)data_ptr);
	if (data_type == ImGuiDataType_U16)
		return ImFormatString(buf, buf_size, format,
				      *(const ImU16 *)data_ptr);
	IM_ASSERT(0);
	return 0;
}
	...
	case ImGuiDataType_S8:
		if (op == '+') {
			*(ImS8 *)output = ImAddClampOverflow(
				*(const ImS8 *)arg1, *(const ImS8 *)arg2,
				IM_S8_MIN, IM_S8_MAX);
		}
		if (op == '-') {
	...
	case ImGuiDataType_U8:
		if (op == '+') {
			*(ImU8 *)output = ImAddClampOverflow(
				*(const ImU8 *)arg1, *(const ImU8 *)arg2,
				IM_U8_MIN, IM_U8_MAX);
		}
		if (op == '-') {
				    ...
				    const char *format)
{
	...
	if (data_type == ImGuiDataType_S32) {
		...
		if (data_type == ImGuiDataType_S8)
			*(ImS8 *)data_ptr = (ImS8)ImClamp(v32, (int)IM_S8_MIN,
							  (int)IM_S8_MAX);
		else if (data_type == ImGuiDataType_U8)
			*(ImU8 *)data_ptr = (ImU8)ImClamp(v32, (int)IM_U8_MIN,
							  (int)IM_U8_MAX);
		else if (data_type == ImGuiDataType_S16)
			*(ImS16 *)data_ptr = (ImS16)ImClamp(
				v32, (int)IM_S16_MIN, (int)IM_S16_MAX);
		else if (data_type == ImGuiDataType_U16)
			*(ImU16 *)data_ptr = (ImU16)ImClamp(
...
{
	ImGuiContext &g = *GImGui;
	const ImGuiAxis axis = (flags & ImGuiDragFlags_Vertical) ? ImGuiAxis_Y
								 : ImGuiAxis_X;
	...
	if (g.ActiveIdSource == ImGuiInputSource_Mouse && IsMousePosValid() &&
		...
		adjust_delta = GetNavInputAmount2d(
			...
			ImGuiInputReadMode_RepeatFast, 1.0f / 10.0f,
			10.0f)[axis];
	...
	v_cur = RoundScalarWithFormatT<TYPE, SIGNEDTYPE>(format, data_type,
							 v_cur);
			 ...
			 const char *format, float power, ImGuiDragFlags flags)
{
	...
	case ImGuiDataType_S16: {
		...
		bool r = DragBehaviorT<ImS32, ImS32, float>(
			...
			v_max ? *(const ImS16 *)v_max : IM_S16_MAX, format,
			power, flags);
		if (r)
			*(ImS16 *)v = (ImS16)v32;
		return r;
	...
	case ImGuiDataType_U16: {
		...
		bool r = DragBehaviorT<ImU32, ImS32, float>(
			...
			v_max ? *(const ImU16 *)v_max : IM_U16_MAX, format,
			power, flags);
		if (r)
			*(ImU16 *)v = (ImU16)v32;
		return r;
		       ...
		       const char *format, float power)
{
	...
	if (temp_input_is_active || temp_input_start)
		return TempInputTextScalar(frame_bb, id, label, data_type, v,
					   format);
			...
			const void *v_max, const char *format, float power)
{
	...
	for (int i = 0; i < components; i++) {
		...
		value_changed |= DragScalar("", data_type, v, v_speed, v_min,
					    v_max, format, power);
		PopID();
		PopItemWidth();
			    ...
			    ImGuiSliderFlags flags, ImRect *out_grab_bb)
{
	...
	if (g.ActiveId == id) {
		...
		if (g.ActiveIdSource == ImGuiInputSource_Mouse) {
			...
			float delta = (axis == ImGuiAxis_X) ? delta2.x
							    : -delta2.y;
			if (g.NavActivatePressedId == id &&
				...
				clicked_t =
					SliderCalcRatioFromValueT<TYPE,
								  FLOATTYPE>(
			   ...
			   ImGuiSliderFlags flags, ImRect *out_grab_bb)
{
	...
	case ImGuiDataType_S8: {
		...
		bool r = SliderBehaviorT<ImS32, ImS32, float>(
			...
			*(const ImS8 *)v_max, format, power, flags,
			out_grab_bb);
		if (r)
			*(ImS8 *)v = (ImS8)v32;
		return r;
	...
	case ImGuiDataType_U8: {
		...
		bool r = SliderBehaviorT<ImU32, ImS32, float>(
			...
			*(const ImU8 *)v_max, format, power, flags,
			out_grab_bb);
		if (r)
			*(ImU8 *)v = (ImU8)v32;
		return r;
	...
	case ImGuiDataType_S16: {
		...
		bool r = SliderBehaviorT<ImS32, ImS32, float>(
			...
			*(const ImS16 *)v_max, format, power, flags,
			out_grab_bb);
		if (r)
			*(ImS16 *)v = (ImS16)v32;
		return r;
	...
	case ImGuiDataType_U16: {
		...
		bool r = SliderBehaviorT<ImU32, ImS32, float>(
			...
			*(const ImU16 *)v_max, format, power, flags,
			out_grab_bb);
		if (r)
			*(ImU16 *)v = (ImU16)v32;
		return r;
			  ...
			  const char *format, float power)
{
	...
	for (int i = 0; i < components; i++) {
		...
		if (i > 0)
			SameLine(0, g.Style.ItemInnerSpacing.x);
		value_changed |= SliderScalar("", data_type, v, v_min, v_max,
					      format, power);
		PopID();
		PopItemWidth();
		v = (void *)((char *)v + type_size);
				...
				const char *format)
{
	...
	bool value_changed = InputTextEx(label, NULL, data_buf,
					 IM_ARRAYSIZE(data_buf), bb.GetSize(),
					 flags);
	if (init) {
	...
	if (value_changed) {
		value_changed = DataTypeApplyOpFromText(
			data_buf, g.InputTextState.InitialTextA.Data, data_type,
			data_ptr, NULL);
		if (value_changed)
			MarkItemEdited(id);
	}
			...
			const char *format, ImGuiInputTextFlags flags)
{
	...
	if (step != NULL) {
		...
		if (InputText(
			...
			value_changed = DataTypeApplyOpFromText(
				buf, g.InputTextState.InitialTextA.Data,
			 ...
			 ImGuiInputTextFlags flags)
{
	...
	for (int i = 0; i < components; i++) {
		...
		if (i > 0)
			SameLine(0, g.Style.ItemInnerSpacing.x);
		value_changed |= InputScalar("", data_type, v, step, step_fast,
					     format, flags);
		PopID();
		PopItemWidth();
		v = (void *)((char *)v + type_size);
			...
			void *callback_user_data)
{
	...
	if (g.ActiveId != id && init_make_active) {
		...
		if (flags &
			...
			g.ActiveIdBlockNavInputFlags |=
				(1 << ImGuiNavInput_KeyTab_);
		if (!is_multiline &&
		    !(flags & ImGuiInputTextFlags_CallbackHistory))
			g.ActiveIdAllowNavDirFlags =
				((1 << ImGuiDir_Up) | (1 << ImGuiDir_Down));
	...
	const char *buf_display = buf_display_from_state ? state->TextA.Data
							 : buf; //-V595
				       ...
				       ImVec2 half_sz, float bar_w)
{
	...
	ImGui::RenderArrowPointingAt(draw_list,
				     ImVec2(pos.x + half_sz.x, pos.y), half_sz,
			...
			memcpy(&dummy_ref_col, ref_col,
			       sizeof(float) * ((picker_flags &
						 ImGuiColorEditFlags_NoAlpha)
							? 3
							: 4));
			ColorPicker4("##dummypicker", &dummy_ref_col.x,
				     picker_flags);
			PopID();
		}
		PopItemWidth();
			     ...
			     const char *label, const char *label_end)
{
	...
	if (display_frame) {
		...
		frame_bb.Min.x -=
			(float)(int)(window->WindowPadding.x * 0.5f - 1.0f);
	...
	if (is_open && !g.NavIdIsAlive &&
		...
		window->DC.TreeStoreMayJumpToParentOnPop |=
			  ...
			  ImGuiTabBarFlags flags)
{
	...
	if ((flags & ImGuiTabBarFlags_Reorderable) &&
		...
		ImQsort(tab_bar->Tabs.Data, tab_bar->Tabs.Size,
			sizeof(ImGuiTabItem), TabItemComparerByVisibleOffset);
		...
		if (!g.DragDropActive &&
			...
			if (g.IO.MouseDelta.x < 0.0f &&
				...
				if (tab_bar->Flags &
					...
					TabBarQueueChangeTabOrder(tab_bar, tab,
								  -1);
