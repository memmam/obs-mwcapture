...
void ImGui_ImplDX11_RenderDrawData(ImDrawData *draw_data)
{
	...
	if (ctx->Map(g_pVB, 0, D3D11_MAP_WRITE_DISCARD, 0, &vtx_resource) !=
	    S_OK)
		return;
	if (ctx->Map(g_pIB, 0, D3D11_MAP_WRITE_DISCARD, 0, &idx_resource) !=
	    S_OK)
		return;
	...
	// Our visible imgui space lies from draw_data->DisplayPos (top left) to draw_data->DisplayPos+data_data->DisplaySize (bottom right). DisplayPos is (0,0) for single viewport apps.
	{
		...
		if (ctx->Map(g_pVertexConstantBuffer, 0,
	...
	ctx->IAGetVertexBuffers(0, 1, &old.VertexBuffer,
	...
	ctx->IASetVertexBuffers(0, 1, &old.VertexBuffer,
				&old.VertexBufferStride,
				&old.VertexBufferOffset);
	if (old.VertexBuffer)
...
bool ImGui_ImplDX11_CreateDeviceObjects()
{
	...
	// Create the vertex shader
	{
		...
		D3DCompile(vertexShader, strlen(vertexShader), NULL, NULL, NULL,
			   "main", "vs_4_0", 0, 0, &g_pVertexShaderBlob, NULL);
		...
		// Create the constant buffer
		{
			...
			desc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
			desc.MiscFlags = 0;
			g_pd3dDevice->CreateBuffer(&desc, NULL,
	...
	// Create the pixel shader
	{
		...
		D3DCompile(pixelShader, strlen(pixelShader), NULL, NULL, NULL,
			   "main", "ps_4_0", 0, 0, &g_pPixelShaderBlob, NULL);
	...
	// Create depth-stencil State
	{
		...
		g_pd3dDevice->CreateDepthStencilState(&desc,
